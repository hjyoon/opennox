...
typedef int32_t (*ammo_xfer_fn_4F6B20)(nox_object_t*);

_Static_assert(CHAR_BIT == 8, "serialized bytes must remain eight bits");
_Static_assert(sizeof(void*) == 4 || sizeof(void*) == 8,
	"unsupported pointer width");
_Static_assert(sizeof(int32_t) == 4,
	"AmmoXfer result must remain int32");
_Static_assert(
	_Generic(&nox_xxx_XFerAmmo_4F6B20,
		ammo_xfer_fn_4F6B20: 1, default: 0),
	"AmmoXfer must preserve one native pointer and exact int32 result");
...
int main(void) {
	...
	ammo_xfer_fn_4F6B20 const transfer = nox_xxx_XFerAmmo_4F6B20;
