#include <math.h>
#include <stdio.h>
#include <time.h>

#include "client/audio/ail/compat_mss.h"

#include "GAME1.h"
#include "GAME1_1.h"
#include "GAME1_2.h"
#include "GAME1_3.h"
#include "GAME2.h"
#include "GAME2_1.h"
#include "GAME2_2.h"
#include "GAME2_3.h"
#include "GAME3.h"
#include "GAME3_1.h"
#include "GAME3_2.h"
#include "GAME3_3.h"
#include "GAME4.h"
#include "GAME4_1.h"
#include "GAME5_2.h"
#include "client__drawable__drawable.h"
#include "common__system__settings.h"
#include "common__system__team.h"

#include "client__gui__gadgets__listbox.h"
#include "client__gui__gamewin__gamewin.h"
#include "client__gui__gui_ctf.h"
#include "client__gui__guibook.h"
#include "client__gui__guibrief.h"
#include "client__gui__guicon.h"
#include "client__gui__guisave.h"
#include "client__gui__guispell.h"
#include "client__gui__servopts__access.h"
#include "client__gui__servopts__general.h"
#include "client__gui__servopts__guiserv.h"
#include "client__gui__servopts__playrlst.h"
#include "client__gui__window.h"
#include "client__shell__mainmenu.h"
#include "common__gamemech__pausefx.h"
#include "common__net_list.h"
#include "common__strman.h"
#include "common__crypt.h"
#include "operators.h"

#include "client__audio__audevent.h"
#include "client__video__draw_common.h"

#include "client__system__ctrlevnt.h"
#include "common__magic__speltree.h"
#include "input_common.h"

extern uintptr_t dword_8531A0_2576;
extern uint32_t dword_5d4594_1046852;
extern uint32_t dword_5d4594_1047532;
extern void* dword_5d4594_1049692;
extern uint32_t dword_5d4594_1046652;
extern uint32_t dword_5d4594_1047536;
extern uint32_t dword_5d4594_1045420;
extern char* dword_5d4594_831240;
extern uint32_t dword_5d4594_1045640;
extern uint32_t dword_5d4594_831260;
extern uint32_t dword_587000_129656;
extern uint32_t dword_5d4594_832480;
extern void* dword_5d4594_1046360;
extern uint32_t dword_587000_126996;
extern uint32_t dword_5d4594_1045428;
extern uint32_t dword_5d4594_831244;
extern uint32_t dword_5d4594_831076;
extern nox_window* dword_5d4594_1049524;
extern uint32_t dword_5d4594_1047524;
extern void* dword_5d4594_1046356;
extern uint32_t dword_5d4594_1046864;
extern uint32_t dword_5d4594_831256;
extern uint32_t dword_5d4594_1049536;
extern nox_window* dword_5d4594_1046944;
extern uint32_t dword_5d4594_1045424;
extern void* dword_5d4594_1049696;
extern nox_window* dword_5d4594_1046540;
extern uint32_t dword_5d4594_831276;
extern uint32_t dword_5d4594_1046648;
extern uint32_t dword_5d4594_1046640;
extern nox_window* dword_5d4594_1045692;
extern uint32_t dword_5d4594_831084;
extern nox_window* dword_5d4594_1046956;
extern uint32_t dword_5d4594_1047936;
extern float2 obj_5d4594_1046620;
extern nox_window* dword_5d4594_1046536;
extern uint32_t dword_5d4594_1045436;
extern uint32_t dword_5d4594_831220;
extern uint32_t dword_5d4594_1045460;
extern uint32_t dword_587000_122848;
extern uint32_t dword_5d4594_1046932;
extern nox_window* dword_5d4594_1046528;
extern nox_window* dword_5d4594_1046948;
extern uint32_t dword_5d4594_830864;
extern uint32_t dword_5d4594_1047528;
extern uint32_t dword_5d4594_1046636;
extern uint32_t dword_5d4594_832520;
extern uint32_t dword_5d4594_832500;
extern uint32_t dword_5d4594_832528;
extern nox_window* dword_5d4594_1045688;
extern uint32_t dword_5d4594_832524;
extern uint32_t dword_5d4594_832512;
extern uint32_t dword_5d4594_832496;
extern uint32_t nox_xxx_aNox_cfg_0_587000_132136;
extern uint32_t dword_5d4594_832516;
extern nox_window* dword_5d4594_1046520;
extern nox_window* dword_5d4594_1049532;
extern uint32_t dword_5d4594_832508;
extern uint32_t dword_5d4594_832504;
extern uint32_t dword_5d4594_831224;
extern nox_things_imageRef_t* dword_5d4594_1046928;
extern nox_window* dword_5d4594_1045508;
extern nox_window* dword_5d4594_1046504;
extern uint32_t dword_5d4594_832492;
extern uint32_t dword_5d4594_1047512;
extern uint32_t dword_5d4594_832532;
extern uint32_t dword_5d4594_1046656;
extern uint32_t dword_5d4594_1049484;
extern nox_window* dword_5d4594_1046516;
extern uint32_t dword_5d4594_832536;
extern nox_window* dword_5d4594_1046952;
extern void* dword_587000_81128;
extern void* dword_587000_122852;
extern nox_window* dword_5d4594_1046532;
extern uint32_t dword_5d4594_1049496;
extern uint32_t dword_5d4594_1047932;
extern nox_window* dword_5d4594_1049512;
extern nox_window* nox_wnd_briefing_831232;
extern uint32_t dword_5d4594_1045432;
extern nox_things_imageRef_t* dword_5d4594_1046924;
extern void* dword_587000_127004;
extern nox_window* dword_5d4594_1045468;
extern nox_window* dword_5d4594_1046508;
extern nox_window* dword_5d4594_1045480;
extern uint32_t dword_5d4594_1047520;
extern uint32_t nox_xxx_aNox_cfg_0_587000_132132;
extern nox_window* dword_5d4594_1045484;
extern nox_window* dword_5d4594_1045464;
extern nox_window* dword_5d4594_1049520;
extern uint32_t dword_5d4594_1046936;
extern uint32_t dword_5d4594_1047540;
extern nox_window* dword_5d4594_831236;
extern nox_window* dword_5d4594_1046496;
extern uint32_t dword_5d4594_1046872;
extern uint32_t nox_gameDisableMapDraw_5d4594_2650672;
extern nox_window* dword_5d4594_1045636;
static nox_video_bag_image_t* nox_fb_flag_team_border;
extern nox_window* dword_5d4594_1049508;
extern uintptr_t dword_5d4594_1047516;
extern nox_window* dword_5d4594_1049500;
extern nox_window* dword_5d4594_1045604;
extern nox_video_bag_image_t* nox_ctf_flag_team_border;
static nox_video_bag_image_t* nox_book_arrow_images[8];
nox_video_bag_image_t* nox_book_background_image;
nox_video_bag_image_t* nox_book_guide_tab_lit_image;
nox_video_bag_image_t* nox_book_spell_tab_lit_image;
nox_list_item_t nox_gui_server_options_rules = {
	&nox_gui_server_options_rules,
	&nox_gui_server_options_rules,
	&nox_gui_server_options_rules,
};

static nox_image_ref_anim_t* nox_book_anim_data(nox_things_imageRef_t* ref) {
	return ref ? (nox_image_ref_anim_t*)ref->field_24 : NULL;
}

static void nox_book_anim_set_end(nox_things_imageRef_t* ref, void* fnc) {
	nox_image_ref_anim_t* anim = nox_book_anim_data(ref);
	if (anim) {
		anim->on_end = fnc;
	}
}

static void nox_book_anim_start(nox_things_imageRef_t* ref) {
	nox_image_ref_anim_t* anim = nox_book_anim_data(ref);
	if (anim) {
		anim->field_3 = nox_xxx_bookGet_430B40_get_mouse_prev_seq();
	}
}
extern uint32_t dword_5d4594_1046868;
extern nox_window* dword_5d4594_1049504;
extern uint32_t dword_5d4594_832484;
extern nox_window* dword_5d4594_1045684;
extern void* nox_xxx_aClosewoodengat_587000_133480;
extern nox_window* dword_5d4594_1046492;
extern int nox_win_width;
extern int nox_win_height;

extern uint32_t nox_color_white_2523948;
extern uint32_t nox_color_black_2650656;
extern uint32_t nox_color_violet_2598268;
extern uint32_t nox_color_red_2589776;
extern uint32_t nox_color_green_2614268;
extern uint32_t nox_color_cyan_2649820;
extern uint32_t nox_color_blue_2650684;
extern uint32_t nox_color_orange_2614256;
extern uint32_t nox_color_yellow_2589772;
extern uint32_t dword_8531A0_2572;

nox_window* nox_win_unk1 = 0;
nox_window* dword_5d4594_1046512 = 0;

uint32_t dword_587000_122856 = 0x1;
uint32_t dword_5d4594_831092 = 0;
uint32_t nox_player_netCode_85319C = 0;

//----- (0044D040) --------------------------------------------------------
int sub_44D040(int i) {
	nox_thing* obj = nox_get_thing(i);
	if (!obj) {
		return 0;
	}
	return (obj->pri_class >> 22) & 1;
}

//----- (0044D060) --------------------------------------------------------
int sub_44D060(int a1) {
	nox_thing* obj = nox_get_thing(a1);
	return obj && (obj->pri_class & 0x400000) && (obj->sub_class & 0x18);
}

//----- (0044D090) --------------------------------------------------------
int sub_44D090(int a1) {
	nox_thing* obj = nox_get_thing(a1);
	return obj && (obj->pri_class & 0x20400000) != 0;
}

//----- (0044D960) --------------------------------------------------------
void sub_44D960() { dword_587000_122848 = 0; }

//----- (0044D970) --------------------------------------------------------
int sub_44D970() {
	int result; // eax

	result = dword_5d4594_831092;
	if (dword_5d4594_831092) {
		dword_587000_122848 = 1;
	}
	return result;
}

//----- (0044D990) --------------------------------------------------------
int sub_44D990() { return dword_587000_122848; }

//----- (0044E110) --------------------------------------------------------
uint32_t* sub_44E110() {
	uint32_t* v0;     // eax
	int v1;           // eax
	uint32_t* v2;     // eax
	int v3;           // eax
	uint32_t* v4;     // eax
	int v5;           // eax
	uint32_t* v6;     // eax
	int v7;           // eax
	uint32_t* v8;     // eax
	int v9;           // eax
	uint32_t* v10;    // eax
	int v11;          // eax
	uint32_t* v12;    // eax
	int v13;          // eax
	uint32_t* v14;    // eax
	int v15;          // eax
	uint32_t* v16;    // eax
	int v17;          // eax
	uint32_t* v18;    // eax
	int v19;          // eax
	uint32_t* v20;    // eax
	int v21;          // eax
	uint32_t* result; // eax
	int v23;          // eax

	if (!dword_5d4594_832484) {
		dword_5d4594_832484 = nox_xxx_guiFontPtrByName_43F360("default");
	}
	v0 = *(uint32_t**)&dword_5d4594_832496;
	if (!dword_5d4594_832496) {
		v1 = nox_xxx_getTTByNameSpriteMB_44CFC0("GauntletExitB");
		v0 = nox_new_drawable_for_thing(v1);
		dword_5d4594_832496 = v0;
	}
	v0[30] |= 0x1000000u;
	v2 = *(uint32_t**)&dword_5d4594_832492;
	if (!dword_5d4594_832492) {
		v3 = nox_xxx_getTTByNameSpriteMB_44CFC0("BeholderGenerator");
		v2 = nox_new_drawable_for_thing(v3);
		dword_5d4594_832492 = v2;
	}
	v2[30] |= 0x1000000u;
	v4 = *(uint32_t**)&dword_5d4594_832500;
	if (!dword_5d4594_832500) {
		v5 = nox_xxx_getTTByNameSpriteMB_44CFC0("Ankh");
		v4 = nox_new_drawable_for_thing(v5);
		dword_5d4594_832500 = v4;
	}
	v4[30] |= 0x1000000u;
	v6 = *(uint32_t**)&dword_5d4594_832504;
	if (!dword_5d4594_832504) {
		v7 = nox_xxx_getTTByNameSpriteMB_44CFC0("SoulGate");
		v6 = nox_new_drawable_for_thing(v7);
		dword_5d4594_832504 = v6;
	}
	v6[30] |= 0x1000000u;
	v8 = *(uint32_t**)&dword_5d4594_832508;
	if (!dword_5d4594_832508) {
		v9 = nox_xxx_getTTByNameSpriteMB_44CFC0("SilverKey");
		v8 = nox_new_drawable_for_thing(v9);
		dword_5d4594_832508 = v8;
	}
	v8[30] |= 0x1000000u;
	v10 = *(uint32_t**)&dword_5d4594_832512;
	if (!dword_5d4594_832512) {
		v11 = nox_xxx_getTTByNameSpriteMB_44CFC0("GoldKey");
		v10 = nox_new_drawable_for_thing(v11);
		dword_5d4594_832512 = v10;
	}
	v10[30] |= 0x1000000u;
	v12 = *(uint32_t**)&dword_5d4594_832516;
	if (!dword_5d4594_832516) {
		v13 = nox_xxx_getTTByNameSpriteMB_44CFC0("QuestGoldChest");
		v12 = nox_new_drawable_for_thing(v13);
		dword_5d4594_832516 = v12;
	}
	v12[30] |= 0x1000000u;
	v14 = *(uint32_t**)&dword_5d4594_832520;
	if (!dword_5d4594_832520) {
		v15 = nox_xxx_getTTByNameSpriteMB_44CFC0("QuestGoldPile");
		v14 = nox_new_drawable_for_thing(v15);
		dword_5d4594_832520 = v14;
	}
	v14[30] |= 0x1000000u;
	v16 = *(uint32_t**)&dword_5d4594_832524;
	if (!dword_5d4594_832524) {
		v17 = nox_xxx_getTTByNameSpriteMB_44CFC0("DunMirChest4");
		v16 = nox_new_drawable_for_thing(v17);
		dword_5d4594_832524 = v16;
	}
	v16[30] |= 0x1000000u;
	v18 = *(uint32_t**)&dword_5d4594_832528;
	if (!dword_5d4594_832528) {
		v19 = nox_xxx_getTTByNameSpriteMB_44CFC0("WarHammer");
		v18 = nox_new_drawable_for_thing(v19);
		dword_5d4594_832528 = v18;
	}
	v18[30] |= 0x1000000u;
	v20 = *(uint32_t**)&dword_5d4594_832532;
	if (!dword_5d4594_832532) {
		v21 = nox_xxx_getTTByNameSpriteMB_44CFC0("HastePotion");
		v20 = nox_new_drawable_for_thing(v21);
		dword_5d4594_832532 = v20;
	}
	v20[30] |= 0x1000000u;
	result = *(uint32_t**)&dword_5d4594_832536;
	if (!dword_5d4594_832536) {
		v23 = nox_xxx_getTTByNameSpriteMB_44CFC0("ConjurerSpellBook");
		result = nox_new_drawable_for_thing(v23);
		dword_5d4594_832536 = result;
	}
	result[30] |= 0x1000000u;
	return result;
}

//----- (0044E3E0) --------------------------------------------------------
int nox_xxx_playGMCAPsmth_44E3E0() {
	int result; // eax

	result = dword_5d4594_831224;
	*getMemU32Ptr(0x5D4594, 831248) = 1;
	if (dword_5d4594_831224) {
		result = nox_xxx_playDialogFile_44D900((unsigned char*)dword_5d4594_831240, 100);
		dword_5d4594_831244 = 1;
	}
	return result;
}

//----- (0044E560) --------------------------------------------------------
nox_window* sub_44E560() {
	nox_window* result;
	nox_window* v1;

	*getMemU32Ptr(0x5D4594, 831284) = (nox_win_width - NOX_DEFAULT_WIDTH) / 2;
	*getMemU32Ptr(0x5D4594, 831288) = (nox_win_height - NOX_DEFAULT_HEIGHT) / 2;
	result = nox_window_new(0, 56, 0, 0, nox_win_width, nox_win_height, nox_xxx_wndProc_44E6E0);
	dword_5d4594_831236 = result;
	if (result) {
		nox_xxx_wndSetWindowProc_46B300(result, nox_client_wndQuestBriefProc_44E630);
		dword_5d4594_831236->draw_data.bg_color = nox_color_black_2650656;
		nox_wnd_briefing_831232 = nox_new_window_from_file("Briefing.wnd", 0);
		result = nox_wnd_briefing_831232;
		if (result) {
			sub_46B120(result, dword_5d4594_831236);
			nox_window_setPos_46A9B0(nox_wnd_briefing_831232, *getMemIntPtr(0x5D4594, 831284),
									 *getMemIntPtr(0x5D4594, 831288));
			v1 = nox_xxx_wndGetChildByID_46B0C0(nox_wnd_briefing_831232, 1010);
			nox_xxx_wndSetDrawFn_46B340(v1, sub_44E6F0);
			sub_44E410();
			result = dword_5d4594_831236;
		}
	}
	return result;
}

//----- (0044E630) --------------------------------------------------------
int nox_client_wndQuestBriefProc_44E630(int a1, int a2, int a3, int a4) {
	int v2; // eax

	if (*getMemU32Ptr(0x5D4594, 831248) && a2 != 18 && a2 != 17 && sub_41D1B0() != 1) {
		sub_450580();
		if (dword_5d4594_831220) {
			if (dword_5d4594_831220 == 255) {
				nox_gameDisableMapDraw_5d4594_2650672 = 0;
				v2 = nox_client_getIntroScreenDuration_44E3B0();
				nox_video_fadeInScreen_44DAB0(v2, 1, sub_44E320);
				nox_gameDisableMapDraw_5d4594_2650672 = 1;
			}
		} else {
			nox_xxx_wndClearCaptureMain_46ADE0(dword_5d4594_831236);
			nox_xxx_windowFocus_46B500(0);
			dword_5d4594_831256 = 1;
			nox_savegame_sub_46D580();
		}
		nox_xxx_wndSetWindowProc_46B300(dword_5d4594_831236, 0);
	}
	return 1;
}

//----- (0044E6E0) --------------------------------------------------------
int nox_xxx_wndProc_44E6E0(int a1, int a2, int a3, int a4) { return a2 == 23; }

int nox_client_getBriefDuration();
//----- (0044E6F0) --------------------------------------------------------
int sub_44E6F0(nox_window* win, nox_window_data* draw) {
	int v2; // eax
	int v3; // eax

	*(float*)&dword_5d4594_831276 = *(float*)&dword_5d4594_831276 - sub_44E8B0();
	v2 = nox_float2int(*(float*)&dword_5d4594_831276);
	nox_window_setPos_46A9B0(win, 0, v2);
	nox_client_copyRect_49F6F0(*getMemIntPtr(0x5D4594, 831284), *getMemIntPtr(0x5D4594, 831288), NOX_DEFAULT_WIDTH,
							   NOX_DEFAULT_HEIGHT);
	if (getMemByte(0x5D4594, 832472) & 1) {
		sub_44E8E0(win, draw);
	} else if (getMemByte(0x5D4594, 832472) & 2) {
		sub_44F0F0(win, draw);
	} else if (getMemByte(0x5D4594, 832472) & 4) {
		sub_44F300(win, draw);
	} else {
		nox_xxx_wndStaticDrawNoImage_488D00(win, draw);
	}
	nox_client_copyRect_49F6F0(0, 0, nox_getBackbufWidth(), nox_getBackbufHeight());
	if ((long long)*(float*)&dword_5d4594_831276 <= *getMemIntPtr(0x5D4594, 831280) && dword_5d4594_831244 == 1 &&
		!sub_44D930() &&
		(unsigned long long)(nox_platform_get_ticks() - *getMemU64Ptr(0x5D4594, 831292)) >
			nox_client_getBriefDuration() &&
		(*getMemU32Ptr(0x5D4594, 832488) == 1 || !(getMemByte(0x5D4594, 832472) & 5))) {
		if (getMemByte(0x5D4594, 832472) & 2 && dword_5d4594_832480) {
			nox_xxx_clientPlaySoundSpecial_452D80(582, 100);
		}
		sub_450580();
		if (dword_5d4594_831220) {
			nox_gameDisableMapDraw_5d4594_2650672 = 0;
			v3 = nox_client_getIntroScreenDuration_44E3B0();
			nox_video_fadeInScreen_44DAB0(v3, 1, sub_44E320);
			nox_gameDisableMapDraw_5d4594_2650672 = 1;
		} else if (!dword_5d4594_831256) {
			nox_xxx_wndClearCaptureMain_46ADE0(dword_5d4594_831236);
			dword_5d4594_831256 = 1;
			nox_savegame_sub_46D580();
			nox_xxx_wndSetWindowProc_46B300(dword_5d4594_831236, 0);
		}
		nox_xxx_wndSetDrawFn_46B340(win, sub_44E8D0);
	}
	return 1;
}

//----- (0044E8B0) --------------------------------------------------------
double sub_44E8B0() {
	double result; // st7

	if (dword_5d4594_831220 == 255) {
		result = 1.0;
	} else {
		result = 0.0;
	}
	return result;
}

//----- (0044E8D0) --------------------------------------------------------
int sub_44E8D0(nox_window* win, nox_window_data* draw) { return 1; }

// called both for quest and solo games chapter intro/briefing
//----- (00450160) --------------------------------------------------------
int nox_client_lockScreenBriefing_450160(int a1, int a2, char a3) {
	int screen = a1;
	int sound_id;
	nox_playerInfo* player = (nox_playerInfo*)dword_8531A0_2576;
	nox_window* text_window;
	wchar2_t** text;
	int text_offset;
	char* dialog;

	dword_5d4594_831260 = 1;
	dword_5d4594_831244 = 1;
	*getMemU32Ptr(0x5D4594, 832488) = 0;
	*getMemU32Ptr(0x5D4594, 832472) = 0;
	dword_5d4594_831256 = 0;
	*getMemU8Ptr(0x5D4594, 831252) = a1 + 1; // 254/255 for quest, 0 for solo
	int player_class = player ? player->info.playerClass : a2;
	nox_xxx_wndShowModalMB_46A8C0(dword_5d4594_831236);
	nox_xxx_wndSetCaptureMain_46ADC0(dword_5d4594_831236);
	nox_xxx_windowFocus_46B500(dword_5d4594_831236);
	nox_xxx_wndSetWindowProc_46B300(dword_5d4594_831236, nox_client_wndQuestBriefProc_44E630);
	text_window = nox_xxx_wndGetChildByID_46B0C0(nox_wnd_briefing_831232, 1010);
	nox_xxx_wndSetDrawFn_46B340(text_window, sub_44E6F0);
	dword_5d4594_831220 = a2;
	text = (wchar2_t**)text_window->widget_data;
	if (screen == 255) {
		nox_xxx_wndSetIcon_46AE60(nox_wnd_briefing_831232, 0);
		nox_window_call_field_94(text_window, 16385, (uintptr_t)getMemPtr(0x5D4594, 831268), 0);
		nox_xxx_wndClearFlag_46AD80(text_window, 0x2000);
		dword_5d4594_831240 = nox_xxx_GetEndgameDialog_578D80();
		sound_id = 24;
		dword_5d4594_831220 = 255;
	} else if (screen == 254) {
		nox_server_currentMapGetFilename_409B30();
		nox_xxx_wndSetIcon_46AE60(nox_wnd_briefing_831232, nox_xxx_gLoadImg_42F970("GauntletStartMines"));
		nox_window_call_field_94(text_window, 16385, (uintptr_t)getMemAt(0x5D4594, 832540), 0);
		sound_id = 0;
		if (a3 & 1) {
			*getMemU32Ptr(0x5D4594, 832472) = 1;
			nox_xxx_wndSetIcon_46AE60(nox_wnd_briefing_831232, nox_xxx_gLoadImg_42F970("MenuSystemBG"));
			nox_client_resetScreenParticles_431510();
			nox_xxx_bookHideMB_45ACA0(1);
		} else if (a3 & 4) {
			*getMemU32Ptr(0x5D4594, 832472) = 4;
			nox_xxx_wndSetIcon_46AE60(nox_wnd_briefing_831232,
								 nox_xxx_gLoadImg_42F970("GauntletInstructionBackground"));
			nox_client_resetScreenParticles_431510();
			nox_xxx_bookHideMB_45ACA0(1);
		} else {
			*getMemU32Ptr(0x5D4594, 832472) = 2;
			nox_xxx_wndSetIcon_46AE60(nox_wnd_briefing_831232, getMemPtr(0x5D4594, 832460));
			if (getMemPtr(0x5D4594, 832464)) {
				nox_window_call_field_94(text_window, 16385, (uintptr_t)getMemPtr(0x5D4594, 832464), 0);
			}
			nox_client_resetScreenParticles_431510();
			nox_xxx_bookHideMB_45ACA0(1);
		}
		sub_446780();
		dword_5d4594_831240 = 0;
		dword_5d4594_831244 = 1;
		dword_5d4594_831224 = 0;
	} else {
		text_offset = 32 * (screen + player_class + 10 * player_class);
		if (a2) {
			nox_xxx_wndSetIcon_46AE60(nox_wnd_briefing_831232, getMemPtr(0x5D4594, 831300 + text_offset));
			nox_window_call_field_94(text_window, 16385,
							 (uintptr_t)getMemPtr(0x5D4594, 831304 + text_offset), 0);
			dialog = getMemPtr(0x5D4594, 831308 + text_offset);
			sound_id = *getMemU32Ptr(0x5D4594, 831312 + text_offset);
		} else {
			nox_xxx_wndSetIcon_46AE60(nox_wnd_briefing_831232, getMemPtr(0x5D4594, 831316 + text_offset));
			nox_window_call_field_94(text_window, 16385,
							 (uintptr_t)getMemPtr(0x5D4594, 831320 + text_offset), 0);
			dialog = getMemPtr(0x5D4594, 831324 + text_offset);
			sound_id = *getMemU32Ptr(0x5D4594, 831328 + text_offset);
		}
		dword_5d4594_831240 = dialog;
	}
	nox_xxx_drawGetStringSize_43F840(text_window->draw_data.font, *text, 0, getMemAt(0x5D4594, 831280),
								  NOX_DEFAULT_HEIGHT);
	sub_46AB20(text_window, NOX_DEFAULT_WIDTH, *getMemIntPtr(0x5D4594, 831280));
	if (screen == 255) {
		dword_5d4594_831276 = 1140457472;
		*getMemU32Ptr(0x5D4594, 831280) = -20 - *getMemU32Ptr(0x5D4594, 831280);
	} else {
		*getMemU32Ptr(0x5D4594, 831280) =
			(480 - nox_xxx_guiFontHeightMB_43F320(text_window->draw_data.font) -
			 *getMemIntPtr(0x5D4594, 831280)) /
			2;
		*(float*)&dword_5d4594_831276 = (double)*getMemIntPtr(0x5D4594, 831280);
	}
	sub_431290();
	dword_5d4594_831224 = 0;
	if (a2 || screen == 255) {
		if (screen != 254) {
			dword_5d4594_831224 = 1;
		}
	} else if (screen != 254) {
		if (player->field_4408_4576[screen]) {
			nox_xxx_wndClearCaptureMain_46ADE0(dword_5d4594_831236);
			dword_5d4594_831256 = 1;
			nox_savegame_sub_46D580();
			nox_xxx_wndSetWindowProc_46B300(dword_5d4594_831236, 0);
		} else {
			dword_5d4594_831224 = 1;
			player->field_4408_4576[screen] = 1;
		}
	}
	sub_43DD70(sound_id, 50);
	*getMemU64Ptr(0x5D4594, 831292) = nox_platform_get_ticks();
	*getMemU32Ptr(0x5D4594, 831248) = 0;
	int duration = nox_client_getIntroScreenDuration_44E3B0();
	nox_video_fadeOutScreen_44DB30(duration, 1, nox_xxx_playGMCAPsmth_44E3E0);
	if (dword_5d4594_831224) {
		dword_5d4594_831244 = 0;
	}
	return dword_5d4594_831224;
}

//----- (004505E0) --------------------------------------------------------
int sub_4505E0() {
	int result; // eax

	if (dword_5d4594_831236) {
		nox_xxx_windowDestroyMB_46C4E0(dword_5d4594_831236);
		dword_5d4594_831236 = 0;
		nox_wnd_briefing_831232 = 0;
	}
	dword_5d4594_831260 = 0;
	dword_5d4594_832484 = 0;
	if (dword_5d4594_832504) {
		nox_xxx_spriteDelete_45A4B0(*(uint64_t**)&dword_5d4594_832504);
	}
	dword_5d4594_832504 = 0;
	if (dword_5d4594_832492) {
		nox_xxx_spriteDelete_45A4B0(*(uint64_t**)&dword_5d4594_832492);
	}
	dword_5d4594_832492 = 0;
	if (dword_5d4594_832500) {
		nox_xxx_spriteDelete_45A4B0(*(uint64_t**)&dword_5d4594_832500);
	}
	dword_5d4594_832500 = 0;
	if (dword_5d4594_832496) {
		nox_xxx_spriteDelete_45A4B0(*(uint64_t**)&dword_5d4594_832496);
	}
	dword_5d4594_832496 = 0;
	if (dword_5d4594_832508) {
		nox_xxx_spriteDelete_45A4B0(*(uint64_t**)&dword_5d4594_832508);
	}
	dword_5d4594_832508 = 0;
	if (dword_5d4594_832512) {
		nox_xxx_spriteDelete_45A4B0(*(uint64_t**)&dword_5d4594_832512);
	}
	dword_5d4594_832512 = 0;
	if (dword_5d4594_832516) {
		nox_xxx_spriteDelete_45A4B0(*(uint64_t**)&dword_5d4594_832516);
	}
	dword_5d4594_832516 = 0;
	if (dword_5d4594_832520) {
		nox_xxx_spriteDelete_45A4B0(*(uint64_t**)&dword_5d4594_832520);
	}
	dword_5d4594_832520 = 0;
	if (dword_5d4594_832524) {
		nox_xxx_spriteDelete_45A4B0(*(uint64_t**)&dword_5d4594_832524);
	}
	dword_5d4594_832524 = 0;
	if (dword_5d4594_832528) {
		nox_xxx_spriteDelete_45A4B0(*(uint64_t**)&dword_5d4594_832528);
	}
	dword_5d4594_832528 = 0;
	if (dword_5d4594_832532) {
		nox_xxx_spriteDelete_45A4B0(*(uint64_t**)&dword_5d4594_832532);
	}
	result = dword_5d4594_832536;
	dword_5d4594_832532 = 0;
	if (dword_5d4594_832536) {
		result = nox_xxx_spriteDelete_45A4B0(*(uint64_t**)&dword_5d4594_832536);
	}
	dword_5d4594_832536 = 0;
	return result;
}

//----- (00450750) --------------------------------------------------------
unsigned char sub_450750() { return getMemByte(0x5D4594, 831252); }

//----- (00450760) --------------------------------------------------------
char sub_450760(char a1) {
	char result; // al

	result = a1;
	*getMemU8Ptr(0x5D4594, 831252) = a1;
	return result;
}

//----- (00450960) --------------------------------------------------------
int sub_450960(const void* a1, const void* a2) {
	unsigned int v2; // eax
	unsigned int v3; // ecx
	int result;      // eax

	v2 = *((uint32_t*)a1 + 3);
	v3 = *((uint32_t*)a2 + 3);
	if (v2 == v3) {
		result = 0;
	} else {
		result = v2 < v3 ? 1 : -1;
	}
	return result;
}

//----- (00450AD0) --------------------------------------------------------
nox_video_bag_image_t* sub_450AD0(nox_video_bag_image_t* image) {
	nox_video_bag_image_t* result = image;
	if (!image) {
		result = nox_xxx_gLoadImg_42F970("WarriorChapterBegin8");
	}
	setMemPtr(0x5D4594, 832460, result);
	return result;
}

//----- (00450AF0) --------------------------------------------------------
wchar2_t* sub_450AF0(wchar2_t* text) {
	setMemPtr(0x5D4594, 832464, text);
	return text;
}

//----- (00450B00) --------------------------------------------------------
void nox_gui_setQuestStage_450B00(int a1) { *getMemU32Ptr(0x5D4594, 832468) = a1; }

//----- (00450B10) --------------------------------------------------------
int nox_gui_getQuestStage_450B10() { return *getMemU32Ptr(0x5D4594, 832468); }

//----- (00451850) --------------------------------------------------------
int sub_451850(int a2, void* a3p) {
	int a3 = a3p;
	int v2;            // edi
	unsigned char* v3; // esi
	int result;        // eax

	v2 = 0;
	v3 = getMemAt(0x5D4594, 840712);
	do {
		sub_451920((uint32_t*)v3 - 21);
		*(uint32_t*)v3 = nox_xxx_getSndName_40AF80(v2);
		v3 += 200;
		++v2;
	} while ((int)v3 < (int)getMemAt(0x5D4594, 1045312));
	dword_5d4594_1045420 = a3;
	dword_5d4594_1045428 = a2;
	if (a3) {
		dword_5d4594_1045424 = sub_4BD340(a3, 0x100000, 200, 0x2000);
		dword_5d4594_1045436 = sub_4BD280(200, 576);
	}
	if (!dword_5d4594_1045424 || !dword_5d4594_1045420 || !dword_5d4594_1045428 || !dword_5d4594_1045436) {
		return 0;
	}
	nox_common_list_clear_425760(getMemAt(0x5D4594, 840612));
	sub_4864A0(getMemAt(0x5D4594, 1045228));
	result = 1;
	*(uint32_t*)(dword_5d4594_1045428 + 184) = getMemAt(0x5D4594, 1045228);
	dword_5d4594_1045432 = 1;
	return result;
}

//----- (00451920) --------------------------------------------------------
int sub_451920(uint32_t* a2) {
	*a2 = 0;
	a2[1] = 0;
	a2[2] = 0;
	a2[14] = 0;
	a2[15] = 0;
	a2[19] = 0;
	a2[20] = 0;
	a2[12] = 1;
	a2[48] = 0;
	a2[18] = 0;
	a2[17] = 0;
	a2[25] = 0;
	a2[26] = 0;
	a2[16] = 600;
	return sub_4862E0((int)(a2 + 4), 0x4000);
}

//----- (00451970) --------------------------------------------------------
void sub_451970() {
	sub_4521F0();
	sub_452230();
	if (dword_5d4594_1045424) {
		sub_4BD3C0(*(void**)&dword_5d4594_1045424);
		dword_5d4594_1045424 = 0;
	}
	if (dword_5d4594_1045436) {
		sub_4BD2D0(*(void**)&dword_5d4594_1045436);
		dword_5d4594_1045436 = 0;
	}
	dword_5d4594_1045432 = 0;
}

//----- (004519C0) --------------------------------------------------------
void sub_4519C0() {
	int result;        // eax
	int v1;            // esi
	int v2;            // eax
	int v3;            // ebp
	int v4;            // eax
	unsigned char* v5; // edi
	unsigned char* v6; // esi
	unsigned char* v7; // edi
	int v8;            // eax
	int v9;            // eax
	int v10;           // eax

	result = dword_5d4594_1045432;
	if (!dword_5d4594_1045432) {
		return;
	}
	result = *getMemU32Ptr(0x5D4594, 1045448);
	if (*getMemU32Ptr(0x5D4594, 1045448)) {
		return;
	}
	*getMemU32Ptr(0x5D4594, 1045448) = 1;
	sub_486520(*(unsigned int**)&dword_587000_127004);
	v1 = *getMemU32Ptr(0x5D4594, 840612);
	++*getMemU32Ptr(0x5D4594, 1045440);
	if (*(unsigned char**)getMemAt(0x5D4594, 840612) != getMemAt(0x5D4594, 840612)) {
		do {
			v2 = *(uint32_t*)(v1 + 36);
			if (*(uint32_t*)(v2 + 100) != *getMemU32Ptr(0x5D4594, 1045440)) {
				nox_common_list_clear_425760((uint32_t*)(v2 + 88));
				*(uint32_t*)(*(uint32_t*)(v1 + 36) + 52) = 0;
				*(uint32_t*)(*(uint32_t*)(v1 + 36) + 100) = *getMemU32Ptr(0x5D4594, 1045440);
			}
			sub_486520((unsigned int*)(v1 + 184));
			if (*(uint32_t*)(v1 + 28) != 4) {
				sub_451BE0(v1);
			}
			v1 = *(uint32_t*)v1;
		} while ((unsigned char*)v1 != getMemAt(0x5D4594, 840612));
		v1 = *getMemU32Ptr(0x5D4594, 840612);
		if (*(unsigned char**)getMemAt(0x5D4594, 840612) != getMemAt(0x5D4594, 840612)) {
			do {
				sub_452510(v1);
				v1 = *(uint32_t*)v1;
			} while ((unsigned char*)v1 != getMemAt(0x5D4594, 840612));
			v1 = *getMemU32Ptr(0x5D4594, 840612);
		}
	}
	v3 = 0;
	sub_452010();
	if ((unsigned char*)v1 != getMemAt(0x5D4594, 840612)) {
		do {
			v4 = *(uint32_t*)(v1 + 176);
			v5 = *(unsigned char**)v1;
			if (!v4 || v1 != *(uint32_t*)(v4 + 152)) {
				sub_4523D0((uint32_t*)v1);
			}
			if (*(uint8_t*)(v1 + 24) & 1) {
				sub_451FE0(v1);
			} else {
				v3 += (unsigned int)(33 * (*(uint32_t*)(*(uint32_t*)(v1 + 36) + 20) >> 16)) >> 14;
				sub_452050((uint32_t*)v1);
			}
			v1 = (int)v5;
		} while (v5 != getMemAt(0x5D4594, 840612));
	}
	if (v3 <= 100) {
		sub_486350((int)getMemAt(0x5D4594, 1045228), 0x4000);
	} else {
		sub_486350((int)getMemAt(0x5D4594, 1045228), 0x190000u / v3);
	}
	result = sub_486520(getMemUintPtr(0x5D4594, 1045228));
	v6 = *(unsigned char**)getMemAt(0x5D4594, 840612);
	if (*(unsigned char**)getMemAt(0x5D4594, 840612) != getMemAt(0x5D4594, 840612)) {
		do {
			v7 = *(unsigned char**)v6;
			result = *((uint32_t*)v6 + 7);
			if (result == 1) {
				sub_451DC0((int)v6);
				v8 = sub_451CA0(v6);
				*((uint32_t*)v6 + 74) = v8;
				if (!v8) {
					do {
						if (!sub_452120((int)v6)) {
							break;
						}
						v7 = *(unsigned char**)v6;
						sub_451DC0((int)v6);
						v9 = sub_451CA0(v6);
						*((uint32_t*)v6 + 74) = v9;
					} while (!v9);
				}
				v10 = sub_451CA0(v6);
				*((uint32_t*)v6 + 74) = v10;
				if (!v10 || (result = sub_452490(v6)) == 0) {
					sub_4523D0(v6);
					result = sub_451FE0((int)v6);
				}
			}
			v6 = v7;
		} while (v7 != getMemAt(0x5D4594, 840612));
	}
	*getMemU32Ptr(0x5D4594, 1045448) = 0;
}

//----- (00451BE0) --------------------------------------------------------
int sub_451BE0(int a1) {
	int v1;          // eax
	int v2;          // edi
	unsigned int v3; // ebx
	uint32_t* v4;    // esi
	int v5;          // eax
	int v6;          // eax
	uint32_t* v7;    // ebx
	int result;      // eax
	int v9;          // esi
	uint32_t* v10;   // esi

	v1 = a1;
	v2 = *(uint32_t*)(a1 + 36);
	v3 = *(uint32_t*)(a1 + 188) >> 16;
	v4 = *(uint32_t**)(v2 + 88);
	if (v4 != (uint32_t*)(v2 + 88)) {
		do {
			v5 = (v4[44] >> 16) - v3;
			if (v5 < 0) {
				v5 = v3 - (v4[44] >> 16);
			}
			if (v5 >= (*(uint32_t*)(v2 + 20) >> 16) / 10) {
				if (v4[44] >> 16 < v3) {
					break;
				}
			} else {
				v6 = v4[4];
				if (*(uint8_t*)(v2 + 4) & 0x10) {
					if (v6) {
						break;
					}
				} else if (!v6) {
					break;
				}
			}
			v4 = (uint32_t*)*v4;
		} while (v4 != (uint32_t*)(v2 + 88));
		v1 = a1;
	}
	v7 = (uint32_t*)(v1 + 12);
	sub_425770((uint32_t*)(v1 + 12));
	nox_common_list_append_4258E0((int)v4, v7);
	result = *(uint32_t*)(v2 + 56);
	v9 = *(uint32_t*)(v2 + 52) + 1;
	*(uint32_t*)(v2 + 52) = v9;
	if (result) {
		if (v9 > result) {
			v10 = (uint32_t*)(*(uint32_t*)(v2 + 92) - 12);
			nox_common_list_remove_425920(*(uint32_t***)(v2 + 92));
			sub_4523D0(v10);
			result = *(uint32_t*)(v2 + 52) - 1;
			*(uint32_t*)(v2 + 52) = result;
		}
	}
	return result;
}

//----- (00451CA0) --------------------------------------------------------
int sub_451CA0(uint32_t* a1) {
	int v1;       // ecx
	int v3;       // eax
	uint32_t* v4; // ecx

	v1 = a1[42];
	a1[108] = v1;
	if (!v1) {
		return 0;
	}
	v3 = 0;
	if (v1 > 0) {
		v4 = a1 + 76;
		do {
			*v4 = v3++;
			++v4;
		} while (v3 < a1[108]);
	}
	a1[43] = -1;
	return sub_451CF0(a1);
}

//----- (00451F30) --------------------------------------------------------
int sub_451F30(int a1, int a2) {
	int v2;     // edx
	int result; // eax

	*(uint32_t*)(a1 + 4 * *(uint32_t*)(a1 + 168) + 40) =
		sub_4BD470(*(uint32_t***)&dword_5d4594_1045424, *(short*)(*(uint32_t*)(a1 + 36) + 2 * a2 + 128));
	v2 = *(uint32_t*)(a1 + 168);
	result = *(uint32_t*)(a1 + 4 * v2 + 40);
	if (result) {
		sub_4BD650(*(uint32_t*)(a1 + 4 * v2 + 40));
		result = *(uint32_t*)(a1 + 168) + 1;
		*(uint32_t*)(a1 + 168) = result;
	}
	return result;
}

//----- (00451F90) --------------------------------------------------------
int sub_451F90(int a1) {
	int v1;     // edi
	int result; // eax
	int* v3;    // esi

	v1 = 0;
	result = *(uint32_t*)(a1 + 168);
	if (result <= 0) {
		*(uint32_t*)(a1 + 168) = 0;
	} else {
		v3 = (int*)(a1 + 40);
		do {
			sub_4BD660(*v3);
			*v3 = 0;
			result = *(uint32_t*)(a1 + 168);
			++v1;
			++v3;
		} while (v1 < result);
		*(uint32_t*)(a1 + 168) = 0;
	}
	return result;
}

//----- (00451FE0) --------------------------------------------------------
int sub_451FE0(int a1) {
	nox_common_list_remove_425920((uint32_t**)a1);
	*(uint32_t*)(a1 + 280) = 0;
	return sub_4BD300(*(uint32_t**)&dword_5d4594_1045436, a1);
}

//----- (00452010) --------------------------------------------------------
int sub_452010() {
	unsigned char* v0; // esi
	int v1;            // ebx
	int v2;            // edi

	v0 = getMemAt(0x5D4594, 839892);
	v1 = 6;
	do {
		v2 = 10;
		do {
			nox_common_list_clear_425760(v0);
			v0 += 12;
			--v2;
		} while (v2);
		--v1;
	} while (v1);
	return ++*getMemU32Ptr(0x5D4594, 1045444);
}

//----- (00452050) --------------------------------------------------------
void sub_452050(uint32_t* a1) {
	uint32_t* v1;      // esi
	int v2;            // edi
	unsigned int v3;   // ebx
	unsigned char* v4; // ebp
	uint32_t* result;  // eax
	uint32_t** v6;     // esi
	uint32_t** v7;     // esi
	uint32_t* v8;      // esi

	v1 = (uint32_t*)a1[9];
	v2 = v1[12] + a1[75];
	v3 = (a1[47] >> 16) / 0x666u;
	v4 = getMemAt(0x5D4594, 839892 + 120 * v2);
	if (v1[26] == *getMemU32Ptr(0x5D4594, 1045444)) {
		result = (uint32_t*)v1[27];
		if (v2 <= (int)result) {
			if ((uint32_t*)v2 == result && v3 > v1[31]) {
				v1[31] = v3;
				v7 = (uint32_t**)(v1 + 28);
				nox_common_list_remove_425920(v7);
				nox_common_list_append_4258E0((int)&v4[12 * v3], v7);
			}
		} else {
			v1[27] = v2;
			v1[31] = v3;
			v6 = (uint32_t**)(v1 + 28);
			nox_common_list_remove_425920(v6);
			nox_common_list_append_4258E0((int)&v4[12 * v3], v6);
		}
	} else {
		v1[26] = *getMemU32Ptr(0x5D4594, 1045444);
		v1[27] = v2;
		v1[31] = v3;
		v8 = v1 + 28;
		sub_425770(v8);
		nox_common_list_append_4258E0((int)&v4[12 * v3], v8);
	}
}

//----- (00452120) --------------------------------------------------------
int* sub_452120(int a1) {
	int v1;            // ebp
	int* result;       // eax
	int* v3;           // ebx
	unsigned char* v4; // esi
	unsigned char* v5; // edi

	v1 = 0;
	result = sub_4521A0(*(uint32_t*)(a1 + 300) + *(uint32_t*)(*(uint32_t*)(a1 + 36) + 48));
	v3 = result;
	if (result) {
		sub_452190((int)result);
		v4 = *(unsigned char**)getMemAt(0x5D4594, 840612);
		if (*(unsigned char**)getMemAt(0x5D4594, 840612) != getMemAt(0x5D4594, 840612)) {
			do {
				v5 = *(unsigned char**)v4;
				if (*((int**)v4 + 9) == v3) {
					sub_4523D0(v4);
					sub_451FE0((int)v4);
					v1 = 1;
				}
				v4 = v5;
			} while (v5 != getMemAt(0x5D4594, 840612));
		}
		result = (int*)v1;
	}
	return result;
}

//----- (00452190) --------------------------------------------------------
void sub_452190(int a1) { nox_common_list_remove_425920((uint32_t**)(a1 + 112)); }

//----- (004521A0) --------------------------------------------------------
int* sub_4521A0(int a1) {
	int v1;            // ebp
	unsigned char* v2; // ebx
	int v3;            // edi
	int* v4;           // esi
	int* v5;           // eax

	v1 = 0;
	v2 = getMemAt(0x5D4594, 839892);
	if (a1 > 0) {
		while (1) {
			v3 = 0;
			v4 = (int*)v2;
			do {
				v5 = nox_common_list_getFirstSafe_425890(v4);
				if (v5) {
					return v5 - 28;
				}
				++v3;
				v4 += 3;
			} while (v3 < 10);
			++v1;
			v2 += 120;
			if (v1 < a1) {
				continue;
			}
			break;
		}
	}
	return 0;
}

//----- (004521F0) --------------------------------------------------------
int sub_4521F0() {
	int result;        // eax
	unsigned char* v1; // esi
	unsigned char* v2; // edi

	result = dword_5d4594_1045432;
	if (dword_5d4594_1045432) {
		v1 = *(unsigned char**)getMemAt(0x5D4594, 840612);
		if (*(unsigned char**)getMemAt(0x5D4594, 840612) != getMemAt(0x5D4594, 840612)) {
			do {
				v2 = *(unsigned char**)v1;
				sub_4523D0(v1);
				result = sub_451FE0((int)v1);
				v1 = v2;
			} while (v2 != getMemAt(0x5D4594, 840612));
		}
	}
	return result;
}

//----- (00452230) --------------------------------------------------------
int***** sub_452230() {
	int***** result; // eax
	int**** v1;      // esi

	result = *(int******)&dword_5d4594_1045432;
	if (dword_5d4594_1045432) {
		result = *(int******)getMemAt(0x5D4594, 840612);
		if (*(unsigned char**)getMemAt(0x5D4594, 840612) != getMemAt(0x5D4594, 840612)) {
			do {
				v1 = *result;
				if ((uint8_t)result[6] & 1) {
					sub_451FE0((int)result);
				}
				result = (int*****)v1;
			} while (v1 != (int****)getMemAt(0x5D4594, 840612));
		}
	}
	return result;
}

//----- (00452270) --------------------------------------------------------
char* nox_xxx_draw_452270(int a1) {
	char* result; // eax

	if (dword_5d4594_1045432 && a1 >= 0 && a1 < 1023) {
		result = (char*)getMemAt(0x5D4594, 840628 + 200 * a1);
	} else {
		result = 0;
	}
	return result;
}

//----- (004522A0) --------------------------------------------------------
int sub_4522A0(int a1) {
	int result; // eax

	if (dword_5d4594_1045432) {
		result = *(uint32_t*)(a1 + 64);
	} else {
		result = 0;
	}
	return result;
}

//----- (00452300) --------------------------------------------------------
uint32_t* nox_xxx_draw_452300(uint32_t* a1) {
	uint32_t* v1; // esi

	if (!dword_5d4594_1045432) {
		return 0;
	}
	if (!dword_587000_126996) {
		return 0;
	}
	if (!*a1) {
		return 0;
	}
	v1 = sub_4BD2E0(*(uint32_t***)&dword_5d4594_1045436);
	if (!v1) {
		sub_452230();
		v1 = sub_4BD2E0(*(uint32_t***)&dword_5d4594_1045436);
		if (!v1) {
			return 0;
		}
	}
	memset(v1, 0, 0x240u);
	v1[9] = a1;
	sub_425770(v1);
	v1[7] = 0;
	v1[75] = 0;
	v1[142] = 0;
	v1[108] = 0;
	v1[42] = 0;
	sub_4864A0(v1 + 46);
	nox_common_list_append_4258E0((int)getMemAt(0x5D4594, 840612), v1);
	v1[70] = (*getMemU32Ptr(0x587000, 127000))++;
	return v1;
}

//----- (004523D0) --------------------------------------------------------
int sub_4523D0(void* a1p) {
	uint32_t* a1 = a1p;
	int result = 0; // eax

	if (!(a1[6] & 1)) {
		sub_452410((int)a1);
		sub_451F90((int)a1);
		a1[7] = 4;
		a1[70] = 0;
		result = a1[6];
		LOBYTE(result) = result | 1;
		a1[6] = result;
	}
	return result;
}

//----- (00452410) --------------------------------------------------------
int sub_452410(int a1) {
	int result; // eax

	result = *(uint32_t*)(a1 + 176);
	if (result && a1 == *(uint32_t*)(result + 152)) {
		if (*(uint8_t*)(a1 + 24) & 2) {
			sub_4BDA80(*(uint32_t*)(a1 + 176));
		}
		sub_4BDB30(*(uint32_t*)(a1 + 176));
		*(uint32_t*)(*(uint32_t*)(a1 + 176) + 152) = 0;
		*(uint32_t*)(*(uint32_t*)(a1 + 176) + 148) = 0;
		result = *(uint32_t*)(a1 + 176);
		*(uint32_t*)(result + 140) = 0;
		*(uint32_t*)(*(uint32_t*)(a1 + 176) + 144) = 0;
		*(uint32_t*)(*(uint32_t*)(a1 + 176) + 112) = 0;
		*(uint32_t*)(a1 + 176) = 0;
	}
	return result;
}

//----- (00452490) --------------------------------------------------------
int sub_452490(uint32_t* a1) {
	int v1; // eax
	int v3; // edi
	int v4; // eax

	v1 = a1[44];
	if (a1 != *(uint32_t**)(v1 + 152)) {
		return 0;
	}
	v3 = a1[74];
	sub_4BDB90((uint32_t*)v1, (uint32_t*)a1[74]);
	a1[7] = 3;
	v4 = a1[6];
	LOBYTE(v4) = v4 | 2;
	a1[6] = v4;
	a1[74] = 0;
	if (!sub_4BDB40(a1[44])) {
		return 1;
	}
	a1[7] = 1;
	a1[74] = v3;
	a1[6] &= 0xFFFFFFFD;
	return 0;
}

//----- (00452510) --------------------------------------------------------
void sub_452510(int a3) {
	int v1; // eax
	int v2; // eax

	if (!dword_587000_126996) {
		*(uint32_t*)(a3 + 28) = 4;
	}
	while (1) {
		v1 = *(uint32_t*)(a3 + 28);
		if (!v1) {
			break;
		}
		v2 = v1 - 2;
		if (v2) {
			uint32_t v3 = v2 - 2;
			if (!v3) {
				sub_4523D0((uint32_t*)a3);
			}
			return;
		}
		if (nox_platform_get_ticks() <= *(uint64_t*)(a3 + 288)) {
			return;
		}
		*(uint32_t*)(a3 + 28) = *(uint32_t*)(a3 + 32);
	}
	if (!sub_452580((uint32_t*)a3)) {
		sub_4523D0((uint32_t*)a3);
	}
}

//----- (00452690) --------------------------------------------------------
long long sub_452690(int a3, long long a4, int a5) {
	long long result; // rax

	*(uint32_t*)(a3 + 32) = a5;
	result = a4 + nox_platform_get_ticks();
	*(uint64_t*)(a3 + 288) = result;
	*(uint32_t*)(a3 + 28) = 2;
	return result;
}

//----- (004526D0) --------------------------------------------------------
int sub_4526D0(int a1) {
	*(uint32_t*)(*(uint32_t*)(a1 + 152) + 28) = 4;
	return 0;
}

//----- (004526F0) --------------------------------------------------------
int sub_4526F0(int a1) {
	uint32_t* v1; // esi
	int v2;       // eax

	v1 = *(uint32_t**)(a1 + 152);
	v1[6] &= 0xFFFFFFFD;
	v2 = 4;
	if (v1[7] != 4) {
		if (v1[74] || v1[142]) {
			v2 = 1;
		} else {
			v1[71] = 0;
		}
		if (v1[71]) {
			sub_452690((int)v1, (unsigned int)v1[71], v2);
			v1[71] = 0;
			return 0;
		}
		v1[7] = v2;
	}
	return 0;
}

//----- (00452810) --------------------------------------------------------
int* sub_452810(int a1, char a2) {
	int* v2; // esi
	int* v3; // eax

	v2 = 0;
	if (dword_5d4594_1045428) {
		v3 = sub_487810(*(int*)&dword_5d4594_1045428, 1);
		v2 = v3;
		if (v3) {
			if (v3[31] & 0x15 && v3[30] > a1) {
				return 0;
			}
			sub_4BDA80((int)v3);
			v2[29] = dword_587000_127004;
			v2[30] = a1;
			if (a2 & 1) {
				v2[32] = -1;
			} else {
				v2[32] = 0;
			}
			sub_486320(v2 + 4, 0x4000);
		}
	}
	return v2;
}

//----- (00452890) --------------------------------------------------------
#if 0
int nox_thing_read_AVNT_452890_pe32_reference(nox_memfile* a1p, void* a2) {
	int a1 = a1p;
	int v2;             // esi
	unsigned char* v3;  // eax
	int v4;             // eax
	char* v5;           // ebx
	char* v6;           // eax
	char v7;            // dl
	char* v8;           // eax
	int v9;             // ebp
	unsigned char* v10; // eax
	short v11;          // ax
	int v12;            // ecx
	int* v13;           // eax
	int v14;            // ecx
	short v15;          // cx
	short v16;          // cx
	int v17;            // edx
	char* v18;          // eax
	char v19;           // cl
	int result;         // eax
	int v21;            // [esp-4h] [ebp-30h]
	uint16_t* v22;      // [esp+10h] [ebp-1Ch]
	unsigned char v23;  // [esp+18h] [ebp-14h]
	unsigned char v24;  // [esp+1Ch] [ebp-10h]
	unsigned char v25;  // [esp+20h] [ebp-Ch]
	unsigned char v26;  // [esp+24h] [ebp-8h]
	unsigned char v27;  // [esp+28h] [ebp-4h]
	unsigned char v28;  // [esp+30h] [ebp+4h]
	unsigned char v29;  // [esp+30h] [ebp+4h]

	v2 = a1;
	v3 = *(unsigned char**)(a1 + 8);
	v21 = a1;
	v28 = *v3;
	*(uint32_t*)(v21 + 8) = v3 + 1;
	nox_memfile_read(a2, 1u, v28, v21);
	*((uint8_t*)a2 + v28) = 0;
	v4 = nox_xxx_utilFindSound_40AF50(a2);
	if (v4 && (v5 = nox_xxx_draw_452270(v4)) != 0) {
		while (1) {
			v6 = *(char**)(v2 + 8);
			v7 = *v6;
			v8 = v6 + 1;
			*(uint32_t*)(v2 + 8) = v8;
			switch (v7) {
			case 0:
				*(uint32_t*)v5 = 1;
				result = v7 == 0;
				break;
			case 1:
				v23 = *v8;
				*(uint32_t*)(v2 + 8) = v8 + 1;
				*((uint32_t*)v5 + 12) = v23;
				continue;
			case 2:
				v26 = *v8;
				*(uint32_t*)(v2 + 8) = v8 + 1;
				*((uint32_t*)v5 + 1) = v26;
				continue;
			case 3:
				v27 = *v8;
				*(uint32_t*)(v2 + 8) = v8 + 1;
				sub_4862E0((int)(v5 + 16), 163 * v27);
				continue;
			case 4:
				v24 = *v8;
				*(uint32_t*)(v2 + 8) = v8 + 1;
				*((uint32_t*)v5 + 14) = v24;
				continue;
			case 5:
				v25 = *v8;
				*(uint32_t*)(v2 + 8) = v8 + 1;
				*((uint32_t*)v5 + 15) = v25;
				continue;
			case 6:
				v17 = *v8;
				*(uint32_t*)(v2 + 8) = v8 + 1;
				*((uint32_t*)v5 + 19) = v17;
				v18 = *(char**)(v2 + 8);
				v19 = *v18;
				*(uint32_t*)(v2 + 8) = v18 + 1;
				*((uint32_t*)v5 + 20) = v19;
				continue;
			case 7:
				v9 = 0;
				v22 = v5 + 128;
				while (1) {
					v10 = *(unsigned char**)(v2 + 8);
					v29 = *v10;
					*(uint32_t*)(v2 + 8) = v10 + 1;
					if (!v29) {
						break;
					}
					nox_memfile_read(a2, 1u, v29, v2);
					*((uint8_t*)a2 + v29) = 0;
					if (v9 < 32) {
						v11 = sub_486A10(*(int*)&dword_5d4594_1045420, a2);
						*v22 = v11;
						if (v11 != -1) {
							++v9;
							++v22;
						}
					}
				}
				*((uint32_t*)v5 + 48) = v9;
				continue;
			case 8:
				v12 = *(uint32_t*)v8;
				*(uint32_t*)(v2 + 8) = v8 + 4;
				*((uint32_t*)v5 + 17) = v12;
				v13 = *(int**)(v2 + 8);
				v14 = *v13;
				*(uint32_t*)(v2 + 8) = v13 + 1;
				*((uint32_t*)v5 + 18) = v14;
				continue;
			case 9:
				v15 = *(uint16_t*)v8;
				*(uint32_t*)(v2 + 8) = v8 + 2;
				if (v15 > 0) {
					*((uint32_t*)v5 + 16) = 15 * v15;
				}
				continue;
			case 0xA:
				v16 = *(uint16_t*)v8;
				*(uint32_t*)(v2 + 8) = v8 + 2;
				*((uint32_t*)v5 + 2) = v16;
				continue;
			default:
				result = 0;
				break;
			}
			break;
		}
	} else {
		nox_thing_skip_AVNT_inner_452B30(v2);
		result = 1;
	}
	return result;
}
#endif

int nox_thing_read_AVNT_452890(nox_memfile* f, void* bufp) {
	char* buf = bufp;
	uint8_t name_len = nox_memfile_read_u8(f);
	nox_memfile_read(buf, 1, name_len, f);
	buf[name_len] = 0;
	int sound = nox_xxx_utilFindSound_40AF50(buf);
	char* def = sound ? nox_xxx_draw_452270(sound) : NULL;
	if (!def) {
		nox_thing_skip_AVNT_inner_452B30(f);
		return 1;
	}

	for (;;) {
		switch (nox_memfile_read_i8(f)) {
		case 0:
			*(uint32_t*)def = 1;
			return 1;
		case 1:
			((uint32_t*)def)[12] = nox_memfile_read_u8(f);
			break;
		case 2:
			((uint32_t*)def)[1] = nox_memfile_read_u8(f);
			break;
		case 3:
			sub_4862E0(def + 16, 163 * nox_memfile_read_u8(f));
			break;
		case 4:
			((uint32_t*)def)[14] = nox_memfile_read_u8(f);
			break;
		case 5:
			((uint32_t*)def)[15] = nox_memfile_read_u8(f);
			break;
		case 6:
			((uint32_t*)def)[19] = (uint32_t)nox_memfile_read_i8(f);
			((uint32_t*)def)[20] = (uint32_t)nox_memfile_read_i8(f);
			break;
		case 7: {
			int samples = 0;
			uint16_t* ids = (uint16_t*)(def + 128);
			for (;;) {
				uint8_t sample_len = nox_memfile_read_u8(f);
				if (!sample_len) {
					break;
				}
				nox_memfile_read(buf, 1, sample_len, f);
				buf[sample_len] = 0;
				if (samples < 32) {
					int16_t id = (int16_t)sub_486A10(*(int*)&dword_5d4594_1045420, buf);
					*ids = (uint16_t)id;
					if (id != -1) {
						samples++;
						ids++;
					}
				}
			}
			((uint32_t*)def)[48] = (uint32_t)samples;
			break;
		}
		case 8:
			((uint32_t*)def)[17] = nox_memfile_read_u32(f);
			((uint32_t*)def)[18] = nox_memfile_read_u32(f);
			break;
		case 9: {
			int16_t duration = nox_memfile_read_i16(f);
			if (duration > 0) {
				((uint32_t*)def)[16] = 15 * duration;
			}
			break;
		}
		case 0xA:
			((uint32_t*)def)[2] = (uint32_t)nox_memfile_read_i16(f);
			break;
		default:
			return 0;
		}
	}
}

//----- (00452B00) --------------------------------------------------------
int nox_thing_skip_AVNT_452B00(nox_memfile* f) {
	int sz = nox_memfile_read_u8(f);
	nox_memfile_skip(f, sz);
	return nox_thing_skip_AVNT_inner_452B30(f);
}

//----- (00452B30) --------------------------------------------------------
int nox_thing_skip_AVNT_inner_452B30(nox_memfile* f) {
	char v3;          // bl
	unsigned char v5; // dl
	int result;       // eax
	char v8;          // [esp+Ch] [ebp+4h]

	while (1) {
		v3 = nox_memfile_read_i8(f);
		v8 = v3;
		switch (v8) {
		case 0:
			result = v3 == 0;
			break;
		case 1:
		case 2:
		case 3:
		case 4:
		case 5:
			nox_memfile_skip(f, 1);
			continue;
		case 6:
		case 9:
		case 0xA:
			nox_memfile_skip(f, 2);
			continue;
		case 7:
			while (1) {
				v5 = nox_memfile_read_u8(f);
				if (!v5) {
					break;
				}
				nox_memfile_skip(f, v5);
			}
			continue;
		case 8:
			nox_memfile_skip(f, 8);
			continue;
		default:
			result = 0;
			break;
		}
		break;
	}
	return result;
}

//----- (00452BD0) --------------------------------------------------------
int sub_452BD0(nox_memfile* f, char* buf) {
	uint8_t name_len = nox_memfile_read_u8(f);
	nox_memfile_read(buf, 1, name_len, f);
	buf[name_len] = 0;
	int sound = nox_xxx_utilFindSound_40AF50(buf);
	char* def = sound ? nox_xxx_draw_452270(sound) : NULL;
	if (!def) {
		nox_memfile_skip(f, 9);
		for (;;) {
			uint8_t sample_len = nox_memfile_read_u8(f);
			if (!sample_len) {
				return 1;
			}
			nox_memfile_skip(f, sample_len);
		}
	}

	((uint32_t*)def)[1] = 2;
	((uint32_t*)def)[2] = (uint32_t)nox_memfile_read_i16(f);
	sub_4862E0(def + 16, 163 * nox_memfile_read_u8(f));
	int16_t duration = nox_memfile_read_i16(f);
	if (duration > 0) {
		((uint32_t*)def)[16] = 15 * duration;
	}
	((uint32_t*)def)[14] = (uint32_t)nox_memfile_read_i8(f);
	((uint32_t*)def)[19] = (uint32_t)nox_memfile_read_i8(f);
	((uint32_t*)def)[20] = (uint32_t)nox_memfile_read_i8(f);
	int8_t mode = nox_memfile_read_i8(f);
	((uint32_t*)def)[12] = (uint32_t)mode;
	if (mode >= 3) {
		return 0;
	}

	int samples = 0;
	uint16_t* ids = (uint16_t*)(def + 128);
	for (;;) {
		uint8_t sample_len = nox_memfile_read_u8(f);
		if (!sample_len) {
			break;
		}
		nox_memfile_read(buf, 1, sample_len, f);
		buf[sample_len] = 0;
		char* ext = strrchr(buf, '.');
		if (ext) {
			*ext = 0;
		}
		int16_t id = (int16_t)sub_486A10(*(int*)&dword_5d4594_1045420, buf);
		*ids = (uint16_t)id;
		if (id != -1) {
			samples++;
			ids++;
		}
	}
	*(uint32_t*)def = 1;
	((uint32_t*)def)[48] = (uint32_t)samples;
	return 1;
}

//----- (00452D80) --------------------------------------------------------
extern void nox_xxx_clientPlaySoundSpecial_native_452D80(int a1, int a2);

void nox_xxx_clientPlaySoundSpecial_452D80(int a1, int a2) {
	nox_xxx_clientPlaySoundSpecial_native_452D80(a1, a2);
}

//----- (00452DC0) --------------------------------------------------------
void sub_452DC0(int a1, int a2, int a3) {
	uint32_t* result; // eax
	uint32_t* v4;     // esi

	result = nox_xxx_draw_452270(a1);
	if (!result) {
		return;
	}
	result = nox_xxx_draw_452300(result);
	v4 = result;
	if (!result) {
		return;
	}
	sub_452EE0((int)result, a2);
	sub_452F80((int)v4, a3);
	sub_452510((int)v4);
}

//----- (00452E10) --------------------------------------------------------
void sub_452E10(int a1, int a2, int a3) {
	uint32_t* result; // eax
	uint32_t* v4;     // esi

	result = nox_xxx_draw_452270(a1);
	if (!result) {
		return;
	}
	result = nox_xxx_draw_452300(result);
	v4 = result;
	if (!result) {
		return;
	}
	sub_452EE0((int)result, a2);
	sub_452F80((int)v4, a3);
	v4[75] = 2;
	sub_452510((int)v4);
}

//----- (00452E90) --------------------------------------------------------
int sub_452E90(uint32_t* a1, int a2) {
	int result; // eax

	result = a2;
	*a1 = a2;
	if (a2) {
		a1[1] = *(uint32_t*)(a2 + 280);
		result = *(uint32_t*)(a2 + 36);
		a1[2] = result;
	}
	return result;
}

//----- (00452EB0) --------------------------------------------------------
int sub_452EB0(int* a1) {
	int result; // eax

	result = *a1;
	if (*a1 && (a1[2] != *(uint32_t*)(result + 36) || a1[1] != *(uint32_t*)(result + 280))) {
		result = 0;
		*a1 = 0;
	}
	return result;
}

//----- (00452EE0) --------------------------------------------------------
int sub_452EE0(int a1, int a2) {
	int v2; // eax

	v2 = sub_452F10(a1, a2);
	sub_486320((uint32_t*)(a1 + 184), v2);
	return sub_4863B0((unsigned int*)(a1 + 184));
}

//----- (00452F10) --------------------------------------------------------
unsigned int sub_452F10(int a1, int a2) {
	int v2; // ecx

	v2 = a2;
	if (a2 <= 100) {
		if (a2 < 0) {
			v2 = 0;
		}
	} else {
		v2 = 100;
	}
	return (unsigned int)(163 * v2 * (*(uint32_t*)(*(uint32_t*)(a1 + 36) + 20) >> 16)) >> 14;
}

//----- (00452F50) --------------------------------------------------------
int sub_452F50(int a1, int a2) {
	int v2; // eax

	v2 = sub_452F10(a1, a2);
	return sub_486350(a1 + 184, v2);
}

//----- (00452F80) --------------------------------------------------------
uint32_t* sub_452F80(int a1, int a2) {
	int v2; // eax

	v2 = sub_452FA0(a2);
	return sub_486320((uint32_t*)(a1 + 248), v2);
}

//----- (00452FA0) --------------------------------------------------------
int sub_452FA0(int a1) {
	int v1; // eax

	v1 = a1;
	if (a1 <= 50) {
		if (a1 < -50) {
			v1 = -50;
		}
	} else {
		v1 = 50;
	}
	return (v1 * 8192) / 50 + 8192;
}

//----- (00452FE0) --------------------------------------------------------
int sub_452FE0(int a1, int a2) {
	int v2; // eax

	v2 = sub_452FA0(a2);
	return sub_486350(a1 + 248, v2);
}

//----- (00453050) --------------------------------------------------------
void sub_453050() { dword_587000_126996 = 0; }

//----- (00453060) --------------------------------------------------------
void nox_xxx____setargv_9_453060() { dword_587000_126996 = 1; }

//----- (00453070) --------------------------------------------------------
int sub_453070() { return dword_587000_126996; }

//----- (00453080) --------------------------------------------------------
int sub_453080(char a1) {
	int result; // eax

	if (dword_5d4594_1045460) {
		result = sub_453690(1 << a1);
	} else {
		result = sub_453660(1 << a1);
	}
	return result;
}

static int nox_gui_server_list_first_visible(const nox_scrollListBox_data* data) {
	if (!data || !data->items || !data->field_11_0) {
		return 0;
	}
	int scroll = data->field_13_1;
	if ((int)data->items[0].field_0 > scroll) {
		return 0;
	}
	for (int i = 1; i < data->field_11_0; ++i) {
		if ((int)data->items[i].field_0 > scroll) {
			return i;
		}
	}
	return 0;
}

//----- (004532E0) --------------------------------------------------------
nox_window* sub_4532E0(void) {
	if (!dword_5d4594_1045464 || !dword_5d4594_1045468) {
		return NULL;
	}
	int row_height = nox_xxx_guiFontHeightMB_43F320(dword_5d4594_1045464->draw_data.font) + 1;
	sub_46AB20(dword_5d4594_1045464, dword_5d4594_1045464->width, 15 * row_height + 2);
	int y = dword_5d4594_1045464->off_y + row_height + 2;
	nox_window* result = NULL;
	for (int id = 1520; id < 1534; ++id) {
		result = nox_xxx_wndGetChildByID_46B0C0(dword_5d4594_1045468, id);
		if (result) {
			result->off_y = y;
		}
		y += row_height;
	}
	return result;
}

//----- (00453350) --------------------------------------------------------
int sub_453350(nox_window* win, nox_window_data* draw) {
	if (!win || !draw) {
		return 0;
	}
	unsigned int x;
	unsigned int y;
	nox_client_wndGetPosition_46AA60(win, &x, &y);
	if ((int8_t)win->flags >= 0) {
		if (draw->bg_color != 0x80000000u) {
			nox_client_drawRectFilledAlpha_49CF10(x, y, win->width, win->height);
		}
	} else if (draw->bg_image) {
		nox_client_drawImageAt_47D2C0(draw->bg_image, x, y);
	}
	return 1;
}

//----- (004533D0) --------------------------------------------------------
int sub_4533D0(nox_window* win, int event, nox_window* control, uintptr_t event_arg) {
	(void)win;
	(void)event_arg;
	if (!dword_5d4594_1045468 || !dword_5d4594_1045464 || !control) {
		return 0;
	}
	if (event == 0x4000) {
		if (control == nox_xxx_wndGetChildByID_46B0C0(dword_5d4594_1045468, 1513) ||
			control == nox_xxx_wndGetChildByID_46B0C0(dword_5d4594_1045468, 1514)) {
			nox_window_call_field_94(dword_5d4594_1045464, 0x4000, (uintptr_t)control, 0);
			sub_453750();
		}
		return 0;
	}
	if (event != 16391) {
		return 0;
	}
	int id = nox_xxx_wndGetID_46B0A0(control);
	switch (id) {
	case 1513:
	case 1514:
		nox_window_call_field_94(dword_5d4594_1045464, 0x4000, (uintptr_t)control, 0);
		sub_453750();
		return 0;
	case 1515:
		if (dword_5d4594_1045460) {
			*getMemU32Ptr(0x5D4594, 1045456) = -1;
		} else {
			*getMemU32Ptr(0x5D4594, 1045452) = -1;
		}
		sub_453750();
		sub_459D50(1);
		break;
	case 1516:
		if (dword_5d4594_1045460) {
			*getMemU32Ptr(0x5D4594, 1045456) = 0;
		} else {
			*getMemU32Ptr(0x5D4594, 1045452) = 0;
		}
		sub_453750();
		sub_459D50(1);
		break;
	case 1520:
	case 1521:
	case 1522:
	case 1523:
	case 1524:
	case 1525:
	case 1526:
	case 1527:
	case 1528:
	case 1529:
	case 1530:
	case 1531:
	case 1532:
	case 1533: {
		nox_scrollListBox_data* data = dword_5d4594_1045464->widget_data;
		int index = nox_gui_server_list_first_visible(data) + id - 1520;
		if (!data || !data->items || index < 0 || index >= data->field_11_0 || !data->items[index].text[0]) {
			break;
		}
		wchar2_t* title = data->items[index].text;
		int flag = dword_5d4594_1045460 ? sub_415DA0(title) : sub_415960(title);
		if (!flag) {
			break;
		}
		int enabled = (control->draw_data.field_0 & 4) == 0;
		if (dword_5d4594_1045460) {
			sub_453640(getMemAt(0x5D4594, 1045456), flag, enabled);
		} else {
			int byte_index = 0;
			while (flag > 0xFF) {
				flag >>= 8;
				++byte_index;
			}
			sub_453620(getMemAt(0x5D4594, 1045452 + byte_index), flag, enabled);
		}
		sub_459D50(1);
		break;
	}
	default:
		break;
	}
	nox_xxx_clientPlaySoundSpecial_452D80(766, 100);
	return 0;
}

//----- (004535E0) --------------------------------------------------------
int* sub_4535E0(int* a1) {
	int* result; // eax

	result = a1;
	*getMemU32Ptr(0x5D4594, 1045452) = *a1;
	return result;
}

//----- (004535F0) --------------------------------------------------------
int sub_4535F0(int a1) {
	int result; // eax

	result = a1;
	*getMemU32Ptr(0x5D4594, 1045456) = a1;
	return result;
}

//----- (00453600) --------------------------------------------------------
char* sub_453600() { return (char*)getMemAt(0x5D4594, 1045452); }

//----- (00453610) --------------------------------------------------------
int sub_453610() { return *getMemU32Ptr(0x5D4594, 1045456); }

//----- (00453620) --------------------------------------------------------
uint8_t* sub_453620(uint8_t* a1, char a2, int a3) {
	uint8_t* result; // eax

	result = a1;
	if (a3) {
		*a1 |= a2;
	} else {
		*a1 &= ~a2;
	}
	return result;
}

//----- (00453640) --------------------------------------------------------
uint32_t* sub_453640(uint32_t* a1, int a2, int a3) {
	uint32_t* result; // eax

	result = a1;
	if (a3) {
		*a1 |= a2;
	} else {
		*a1 &= ~a2;
	}
	return result;
}

//----- (00453660) --------------------------------------------------------
int sub_453660(int a1) {
	int v1; // ecx
	int v2; // edx
	int v3; // eax

	v1 = a1;
	v2 = 0;
	if (a1 > 0) {
		do {
			v3 = v1 >> 8;
			if (v1 >> 8 > 0) {
				v1 >>= 8;
			}
			++v2;
		} while (v3 > 0);
	}
	return ((unsigned char)v1 & getMemByte(0x5D4594, 1045451 + v2)) != 0;
}

//----- (00453690) --------------------------------------------------------
int sub_453690(int a1) { return (a1 & *getMemU32Ptr(0x5D4594, 1045456)) != 0; }

//----- (004536B0) --------------------------------------------------------
int sub_4536B0(uint32_t* a1) {
	int v1;       // ebx
	int v2;       // edi
	uint32_t* v3; // esi
	int v4;       // ebp
	int result;   // eax

	v1 = 1;
	*a1 = -1;
	v2 = 1;
	v3 = a1;
	v4 = 27;
	do {
		result = sub_415840((char*)v2);
		if (result) {
			result = nox_xxx_getUnitDefDd10_4E3BA0(result);
			if (!result) {
				result = (unsigned char)~(uint8_t)v1;
				*(uint8_t*)v3 &= ~(uint8_t)v1;
			}
		}
		if (v1 == 128) {
			v1 = 1;
			v3 = (uint32_t*)((char*)v3 + 1);
		} else {
			v1 *= 2;
		}
		v2 *= 2;
		--v4;
	} while (v4);
	return result;
}

//----- (00453710) --------------------------------------------------------
int sub_453710() {
	int v0; // edi
	int v1; // esi
	int v2; // ebx
	int v3; // eax

	v0 = -1;
	v1 = 1;
	v2 = 26;
	do {
		v3 = sub_415CD0((char*)v1);
		if (v3 && !nox_xxx_getUnitDefDd10_4E3BA0(v3)) {
			v0 &= ~v1;
		}
		v1 *= 2;
		--v2;
	} while (v2);
	return v0;
}

//----- (00453750) --------------------------------------------------------
int sub_453750(void) {
	if (!dword_5d4594_1045464 || !dword_5d4594_1045468) {
		return 0;
	}
	nox_scrollListBox_data* data = dword_5d4594_1045464->widget_data;
	int index = nox_gui_server_list_first_visible(data);
	int count = *getMemIntPtr(0x5D4594, 1045472 + 4 * dword_5d4594_1045460);
	int result = 0;
	for (int id = 1520; id <= 1533; ++id) {
		nox_window* checkbox = nox_xxx_wndGetChildByID_46B0C0(dword_5d4594_1045468, id);
		while (index >= 0 && index < 32 && ((1u << index) & 0x33u)) {
			++index;
		}
		if (!checkbox) {
			++index;
			continue;
		}
		if (index < 0 || index >= count || index >= 32) {
			result = nox_window_set_hidden(checkbox, 1);
		} else {
			nox_window_set_hidden(checkbox, 0);
			if (sub_453080(index)) {
				checkbox->draw_data.field_0 |= 4u;
			} else {
				checkbox->draw_data.field_0 &= ~4u;
			}
			result = checkbox->draw_data.field_0;
		}
		++index;
	}
	return result;
}

//----- (00453B00) --------------------------------------------------------
nox_window* sub_453B00(void) {
	if (!dword_5d4594_1045480 || !dword_5d4594_1045508 || !dword_5d4594_1045484) {
		return NULL;
	}
	int row_height = nox_xxx_guiFontHeightMB_43F320(dword_5d4594_1045480->draw_data.font) + 1;
	sub_46AB20(dword_5d4594_1045480, dword_5d4594_1045480->width, 15 * row_height + 2);
	sub_46AB20(dword_5d4594_1045508, dword_5d4594_1045508->width, 15 * row_height + 2);
	int y = dword_5d4594_1045480->off_y + row_height + 2;
	nox_window* result = NULL;
	for (int id = 1120; id < 1134; ++id) {
		result = nox_xxx_wndGetChildByID_46B0C0(dword_5d4594_1045484, id);
		if (result) {
			result->off_y = y;
		}
		y += row_height;
	}
	return result;
}

//----- (00453B80) --------------------------------------------------------
int sub_453B80(nox_window* win, nox_window_data* draw) {
	if (!win || !draw) {
		return 0;
	}
	unsigned int x;
	unsigned int y;
	nox_client_wndGetPosition_46AA60(win, &x, &y);
	if ((int8_t)win->flags >= 0) {
		if (draw->bg_color != 0x80000000u) {
			nox_client_drawRectFilledAlpha_49CF10(x, y, win->width, win->height);
		}
	} else if (draw->bg_image) {
		nox_client_drawImageAt_47D2C0(draw->bg_image, x, y);
	}
	return 1;
}

//----- (00453F70) --------------------------------------------------------
void sub_453F70(const void* a1) { memcpy(getMemAt(0x5D4594, 1045488), a1, 0x14u); }

//----- (00453F90) --------------------------------------------------------
char* sub_453F90() { return (char*)getMemAt(0x5D4594, 1045488); }

//----- (00453FA0) --------------------------------------------------------
int sub_453FA0(void* a1, int a2, int a3) {
	bool v3;          // sf
	char v4;          // cl
	int result;       // eax
	unsigned char v6; // [esp+8h] [ebp+8h]

	v4 = a2 & 0x1F;
	v3 = (a2 & 0x8000001F & 0x80000000) != 0;
	v6 = a2 / 32;
	if (v3) {
		v4 = ((v4 - 1) | 0xE0) + 1;
	}
	result = 1 << v4;
	if (a3) {
		*((uint32_t*)a1 + v6) |= result;
	} else {
		result = ~result;
		*((uint32_t*)a1 + v6) &= result;
	}
	return result;
}

//----- (00454000) --------------------------------------------------------
int sub_454000(const void* a1, int a2) {
	return (((const uint32_t*)a1)[(a2 / 32) & 0xFF] & (1u << (a2 % 32))) != 0;
}

//----- (00454040) --------------------------------------------------------
int sub_454040(uint32_t* a1) {
	int v1;           // esi
	int v2;           // edi
	int result;       // eax
	unsigned char v4; // [esp+Ch] [ebp-4h]

	v1 = 1;
	*a1 = -1;
	v4 = 0;
	v2 = 1;
	a1[1] = -1;
	a1[2] = -1;
	a1[3] = -1;
	a1[4] = -1;
	do {
		if (v1 == 0x80000000) {
			v1 = 1;
			++v4;
		} else {
			v1 *= 2;
		}
		result = nox_xxx_spellIsValid_424B50(v2);
		if (result) {
			result = nox_xxx_spellFlags_424A70(v2);
			if (result & 0x7000000) {
				result = nox_xxx_spellIsEnabled_424B70(v2);
				if (!result) {
					result = (int)&a1[v4];
					*(uint32_t*)result = ~v1 & a1[v4];
				}
			}
		}
		++v2;
	} while (v2 < 137);
	return result;
}

//----- (004540E0) --------------------------------------------------------
int sub_4540E0(const void* a1) {
	int v1;     // esi
	int result; // eax

	v1 = 1;
	do {
		if (sub_454000(a1, v1)) {
			result = nox_xxx_spellEnable_424B90(v1);
		} else {
			result = nox_xxx_spellDisable_424BB0(v1);
		}
		++v1;
	} while (v1 < 137);
	return result;
}

//----- (00454120) --------------------------------------------------------
int sub_454120(void) {
	if (!dword_5d4594_1045480 || !dword_5d4594_1045484) {
		return 0;
	}
	nox_scrollListBox_data* data = dword_5d4594_1045480->widget_data;
	int index = nox_gui_server_list_first_visible(data);
	int result = 0;
	for (int id = 1120; id <= 1133; ++id, ++index) {
		nox_window* checkbox = nox_xxx_wndGetChildByID_46B0C0(dword_5d4594_1045484, id);
		if (!checkbox) {
			continue;
		}
		int spell = 0;
		if (data && data->items && index >= 0 && index < data->field_11_0 && data->items[index].text[0]) {
			spell = nox_xxx_spellByTitle_424960(data->items[index].text);
			nox_window_set_hidden(checkbox, 0);
		} else {
			nox_window_set_hidden(checkbox, 1);
		}
		if (spell && sub_454000(getMemAt(0x5D4594, 1045488), spell)) {
			checkbox->draw_data.field_0 |= 4u;
		} else {
			checkbox->draw_data.field_0 &= ~4u;
		}
		result = checkbox->draw_data.field_0;
	}
	return result;
}

//----- (004541D0) --------------------------------------------------------
nox_window* nox_xxx_guiServerAccessLoad_4541D0(nox_window* parent) {
	if (nox_gui_server_access.root) {
		return NULL;
	}
	int lang = nox_strman_get_lang_code();
	if (nox_xxx_guiFontHeightMB_43F320(0) > 10) {
		lang = 2;
	}
	nox_gui_server_access.root = nox_new_window_from_file(
		(const char*)getMemPtr(0x587000, 127824 + 4 * lang), nox_xxx_windowAccessProc_454BA0);
	if (!nox_gui_server_access.root) {
		return NULL;
	}
	nox_draw_setTabWidth_43FE20(100);
	sub_46B120(nox_gui_server_access.root, parent);
	nox_gui_server_access.wnd_10102 = nox_xxx_wndGetChildByID_46B0C0(nox_gui_server_access.root, 10102);
	nox_gui_server_access.wnd_10103 = nox_xxx_wndGetChildByID_46B0C0(nox_gui_server_access.root, 10103);
	nox_gui_server_access.wnd_10104 = nox_xxx_wndGetChildByID_46B0C0(nox_gui_server_access.root, 10104);
	nox_gui_server_access.wnd_10105 = nox_xxx_wndGetChildByID_46B0C0(nox_gui_server_access.root, 10105);
	nox_gui_server_access.wnd_10109 = nox_xxx_wndGetChildByID_46B0C0(nox_gui_server_access.root, 10109);
	nox_gui_server_access.wnd_10111 = nox_xxx_wndGetChildByID_46B0C0(nox_gui_server_access.root, 10111);
	nox_gui_server_access.wnd_10112 = nox_xxx_wndGetChildByID_46B0C0(nox_gui_server_access.root, 10112);
	nox_gui_server_access.wnd_10113 = nox_xxx_wndGetChildByID_46B0C0(nox_gui_server_access.root, 10113);
	nox_gui_server_access.wnd_10123 = nox_xxx_wndGetChildByID_46B0C0(nox_gui_server_access.root, 10123);
	nox_gui_server_access.wnd_10125 = nox_xxx_wndGetChildByID_46B0C0(nox_gui_server_access.root, 10125);
	nox_gui_server_access.wnd_10126 = nox_xxx_wndGetChildByID_46B0C0(nox_gui_server_access.root, 10126);
	nox_gui_server_access.wnd_10127 = nox_xxx_wndGetChildByID_46B0C0(nox_gui_server_access.root, 10127);
	nox_gui_server_access.wnd_10128 = nox_xxx_wndGetChildByID_46B0C0(nox_gui_server_access.root, 10128);
	nox_gui_server_access.wnd_10129 = nox_xxx_wndGetChildByID_46B0C0(nox_gui_server_access.root, 10129);
	nox_gui_server_access.wnd_10130 = nox_xxx_wndGetChildByID_46B0C0(nox_gui_server_access.root, 10130);
	nox_gui_server_access.wnd_10131 = nox_xxx_wndGetChildByID_46B0C0(nox_gui_server_access.root, 10131);
	nox_gui_server_access.wnd_10132 = nox_xxx_wndGetChildByID_46B0C0(nox_gui_server_access.root, 10132);
	nox_gui_server_access.wnd_10133 = nox_xxx_wndGetChildByID_46B0C0(nox_gui_server_access.root, 10133);
	nox_gui_server_access.wnd_10200 = nox_xxx_wndGetChildByID_46B0C0(nox_gui_server_access.root, 10200);

	void* slider = nox_xxx_gLoadImg_42F970("UISlider");
	void* slider_lit = nox_xxx_gLoadImg_42F970("UISliderLit");
	struct {
		nox_window* list;
		int thumb_id;
		int up_id;
		int down_id;
	} scrollbars[] = {
		{nox_gui_server_access.wnd_10109, 10190, 10188, 10189},
		{nox_gui_server_access.wnd_10105, 10187, 10185, 10186},
		{nox_gui_server_access.wnd_10200, 10203, 10201, 10202},
	};
	for (size_t i = 0; i < sizeof(scrollbars) / sizeof(scrollbars[0]); ++i) {
		nox_scrollListBox_data* list_data = scrollbars[i].list->widget_data;
		nox_window* thumb = nox_xxx_wndGetChildByID_46B0C0(nox_gui_server_access.root, scrollbars[i].thumb_id);
		nox_window* up = nox_xxx_wndGetChildByID_46B0C0(nox_gui_server_access.root, scrollbars[i].up_id);
		nox_window* down = nox_xxx_wndGetChildByID_46B0C0(nox_gui_server_access.root, scrollbars[i].down_id);
		if (thumb->field_100) {
			thumb->field_100->width = 16;
			thumb->field_100->height = 10;
		}
		sub_4B5700(thumb, NULL, NULL, slider, slider_lit, slider_lit);
		nox_xxx_wnd_46B280(thumb, scrollbars[i].list);
		nox_xxx_wnd_46B280(up, scrollbars[i].list);
		nox_xxx_wnd_46B280(down, scrollbars[i].list);
		list_data->field_9 = thumb;
		list_data->field_7 = up;
		list_data->field_8 = down;
	}
	sub_454740();
	sub_454640();
	if (nox_common_gameFlags_check_40A5C0(1)) {
		nox_xxx_wndSetDrawFn_46B340(nox_gui_server_access.root, sub_454A90);
	}
	nox_xxx_wndRetNULL_46A8A0();
	return nox_gui_server_access.root;
}

//----- (00454A90) --------------------------------------------------------
int sub_454A90(nox_window* win, nox_window_data* draw) {
	unsigned int x_left;
	unsigned int y_top;

	nox_client_wndGetPosition_46AA60(win, &x_left, &y_top);
	if ((int8_t)win->flags >= 0) {
		if (draw->bg_color != 0x80000000) {
			nox_client_drawRectFilledAlpha_49CF10(x_left, y_top, (int)win->width, (int)win->height);
		}
	} else {
		nox_client_drawImageAt_47D2C0(draw->bg_image, x_left, y_top);
	}
	if (nox_gui_server_access.wnd_10111->flags & 8) {
		const wchar2_t* value =
			(const wchar2_t*)nox_window_call_field_94(nox_gui_server_access.wnd_10111, 16413, 0, 0);
		if (value && *value) {
			if (!(nox_gui_server_access.wnd_10112->flags & 8)) {
				nox_xxx_wnd_46ABB0(nox_gui_server_access.wnd_10112, 1);
			}
		} else if (nox_gui_server_access.wnd_10112->flags & 8) {
			nox_xxx_wnd_46ABB0(nox_gui_server_access.wnd_10112, 0);
		}
	}
	int selection = (int)nox_window_call_field_94(nox_gui_server_access.active_list, 16404, 0, 0);
	if (selection < 0) {
		if (nox_gui_server_access.wnd_10113->flags & 8) {
			nox_xxx_wnd_46ABB0(nox_gui_server_access.wnd_10113, 0);
		}
	} else if (!(nox_gui_server_access.wnd_10113->flags & 8)) {
		nox_xxx_wnd_46ABB0(nox_gui_server_access.wnd_10113, 1);
		return 1;
	}
	return 1;
}

//----- (00454BA0) --------------------------------------------------------
static nox_playerInfo* nox_gui_server_access_player_by_name(const wchar2_t* name) {
	if (!name) {
		return NULL;
	}
	for (nox_playerInfo* player = nox_common_playerInfoGetFirst_416EA0(); player;
		 player = nox_common_playerInfoGetNext_416EE0(player)) {
		if (!_nox_wcsicmp(player->name_final, name)) {
			return player;
		}
	}
	return NULL;
}

int nox_xxx_windowAccessProc_454BA0(nox_window* win, int event, uintptr_t arg1, uintptr_t arg2) {
	(void)win;
	wchar2_t text_buf[8];

	switch (event) {
	case 16387:
		if ((uint16_t)arg1 == 1) {
			return 0;
		}
		nox_window* edit = nox_xxx_wndGetChildByID_46B0C0(nox_gui_server_access.root, (int)arg2);
		if (!edit) {
			return 0;
		}
		char* controls = sub_416630();
		char* settings = sub_416640();
		wchar2_t* text = (wchar2_t*)nox_window_call_field_94(edit, 16413, 0, 0);
		int value = 0;
		if (text && *text) {
			value = nox_wcstol(text, 0, 10);
			if (value < 0) {
				value = 0;
			}
		}
		switch ((int)arg2) {
		case 10104:
			if (text) {
				nox_wcscpy((wchar2_t*)settings + 39, text);
			}
			return 0;
		case 10126:
			if (value > 14) {
				value = 14;
				nox_itow(value, text_buf, 10);
				nox_window_call_field_94(edit, 16414, (uintptr_t)text_buf, UINTPTR_MAX);
			}
			controls[1] = value | controls[1] & 0xF0;
			return 0;
		case 10128:
			if (value > 14) {
				value = 14;
				nox_itow(value, text_buf, 10);
				nox_window_call_field_94(edit, 16414, (uintptr_t)text_buf, UINTPTR_MAX);
			}
			controls[1] = (16 * value) | controls[1] & 0xF;
			return 0;
		case 10130:
			*(uint16_t*)(controls + 5) = value;
			return 0;
		case 10132:
			*(uint16_t*)(controls + 7) = value;
			return 0;
		case 10133:
			if (value < 1) {
				value = 1;
				nox_itow(value, text_buf, 10);
				nox_window_call_field_94(edit, 16414, (uintptr_t)text_buf, UINTPTR_MAX);
			} else if (value > 32) {
				value = 32;
				nox_itow(value, text_buf, 10);
				nox_window_call_field_94(edit, 16414, (uintptr_t)text_buf, UINTPTR_MAX);
			}
			nox_xxx_servSetPlrLimit_409F80(value);
			controls[4] = value;
			return 0;
		case 10136:
			nox_xxx_sysopSetPass_40A610(text);
			return 0;
		default:
			return 0;
		}
	case 16391:
		if (!arg1) {
			return 0;
		}
		nox_window* clicked = (nox_window*)arg1;
		int id = nox_xxx_wndGetID_46B0A0(clicked);
		nox_xxx_clientPlaySoundSpecial_452D80(766, 100);
		switch (id) {
		case 10102:
			controls = sub_416630();
			*controls ^= 0x10;
			if (*controls & 0x10) {
				nox_xxx_wnd_46ABB0(nox_xxx_wndGetChildByID_46B0C0(nox_gui_server_access.root, 10206), 1);
			} else {
				nox_window* banned = nox_xxx_wndGetChildByID_46B0C0(nox_gui_server_access.root, 10206);
				nox_window* allowed = nox_xxx_wndGetChildByID_46B0C0(nox_gui_server_access.root, 10207);
				nox_xxx_wnd_46ABB0(banned, 0);
				banned->draw_data.field_0 &= ~4u;
				allowed->draw_data.field_0 |= 4u;
				nox_window_set_hidden(nox_gui_server_access.wnd_10109, 1);
				sub_46ACE0(nox_gui_server_access.root, 10188, 10190, 1);
				nox_window_set_hidden(nox_gui_server_access.wnd_10105, 0);
				sub_46ACE0(nox_gui_server_access.root, 10185, 10187, 0);
			}
			return 0;
		case 10103:
			controls = sub_416630();
			nox_xxx_wnd_46ABB0(nox_gui_server_access.wnd_10104,
				((~nox_gui_server_access.wnd_10103->draw_data.field_0) >> 2) & 1);
			*controls ^= 0x20u;
			return 0;
		case 10112:
			text = (wchar2_t*)nox_window_call_field_94(nox_gui_server_access.wnd_10111, 16413, 0, 0);
			if (wndIsShown_nox_xxx_wndIsShown_46ACC0(nox_gui_server_access.wnd_10105)) {
				sub_4168A0(text);
			} else {
				sub_416770(0, text, NULL);
			}
			nox_window_call_field_94(nox_gui_server_access.wnd_10111, 16414,
				(uintptr_t)getMemAt(0x5D4594, 1045600), 0);
			return 0;
		case 10113:
			if (nox_gui_server_access.wnd_10102->draw_data.field_0 & 4) {
				int selected = (int)nox_window_call_field_94(nox_gui_server_access.wnd_10109, 16404, 0, 0);
				nox_window_call_field_94(nox_gui_server_access.wnd_10109, 16398, selected, 0);
				sub_416860(selected);
			} else {
				int selected = (int)nox_window_call_field_94(nox_gui_server_access.wnd_10105, 16404, 0, 0);
				nox_window_call_field_94(nox_gui_server_access.wnd_10105, 16398, selected, 0);
				sub_416820(selected);
			}
			return 0;
		case 10124:
			controls = sub_416630();
			controls[2] ^= 0x80u;
			return 0;
		case 10125:
			controls = sub_416630();
			if (nox_gui_server_access.wnd_10125->draw_data.field_0 & 4) {
				nox_xxx_wnd_46ABB0(nox_gui_server_access.wnd_10126, 0);
				controls[1] |= 0xFu;
				return 0;
			}
			nox_xxx_wnd_46ABB0(nox_gui_server_access.wnd_10126, 1);
			text = (wchar2_t*)nox_window_call_field_94(nox_gui_server_access.wnd_10126, 16413, 0, 0);
			if (!text || !*text) {
				return 0;
			}
			controls[1] = controls[1] & 0xF0 | nox_wcstol(text, 0, 10);
			return 0;
		case 10127:
			controls = sub_416630();
			if (nox_gui_server_access.wnd_10127->draw_data.field_0 & 4) {
				nox_xxx_wnd_46ABB0(nox_gui_server_access.wnd_10128, 0);
				controls[1] |= 0xF0u;
				return 0;
			}
			nox_xxx_wnd_46ABB0(nox_gui_server_access.wnd_10128, 1);
			text = (wchar2_t*)nox_window_call_field_94(nox_gui_server_access.wnd_10128, 16413, 0, 0);
			if (!text || !*text) {
				return 0;
			}
			controls[1] = controls[1] & 0xF | nox_wcstol(text, 0, 10);
			return 0;
		case 10129:
			controls = sub_416630();
			if (nox_gui_server_access.wnd_10129->draw_data.field_0 & 4) {
				nox_xxx_wnd_46ABB0(nox_gui_server_access.wnd_10130, 0);
				*(uint16_t*)(controls + 5) = UINT16_MAX;
				return 0;
			}
			nox_xxx_wnd_46ABB0(nox_gui_server_access.wnd_10130, 1);
			text = (wchar2_t*)nox_window_call_field_94(nox_gui_server_access.wnd_10130, 16413, 0, 0);
			if (!text || !*text) {
				return 0;
			}
			*(uint16_t*)(controls + 5) = nox_wcstol(text, 0, 10);
			return 0;
		case 10131:
			controls = sub_416630();
			if (nox_gui_server_access.wnd_10131->draw_data.field_0 & 4) {
				nox_xxx_wnd_46ABB0(nox_gui_server_access.wnd_10132, 0);
				*(uint16_t*)(controls + 7) = UINT16_MAX;
				return 0;
			}
			nox_xxx_wnd_46ABB0(nox_gui_server_access.wnd_10132, 1);
			text = (wchar2_t*)nox_window_call_field_94(nox_gui_server_access.wnd_10132, 16413, 0, 0);
			if (!text || !*text) {
				return 0;
			}
			*(uint16_t*)(controls + 7) = nox_wcstol(text, 0, 10);
			return 0;
		case 10191:
		case 10192: {
			int* selected = (int*)nox_window_call_field_94(nox_gui_server_access.wnd_10200, 16404, 0, 0);
			if (!selected || *selected < 0) {
				return 0;
			}
			for (; *selected >= 0; ++selected) {
				wchar2_t* name = (wchar2_t*)nox_window_call_field_94(
					nox_gui_server_access.wnd_10200, 16406, *selected, 0);
				nox_playerInfo* player = nox_gui_server_access_player_by_name(name);
				if (player && player->playerInd != 31) {
					if (nox_common_gameFlags_check_40A5C0(4096)) {
						sub_4DCFB0(player->playerUnit);
					} else if (id == 10191) {
						nox_xxx_playerCallDisconnect_4DEAB0(player->playerInd, 4);
					} else {
						nox_xxx_playerDisconnByPlrID_4DEB00(player->playerInd);
					}
					if (id == 10192) {
						sub_416770(0, name, player->serial);
					}
				}
			}
			return 0;
		}
		case 10206:
			nox_window_set_hidden(nox_gui_server_access.wnd_10109, 0);
			sub_46ACE0(nox_gui_server_access.root, 10188, 10190, 0);
			nox_window_set_hidden(nox_gui_server_access.wnd_10105, 1);
			sub_46ACE0(nox_gui_server_access.root, 10185, 10187, 1);
			nox_gui_server_access.active_list = nox_gui_server_access.wnd_10109;
			return 0;
		case 10207:
			nox_window_set_hidden(nox_gui_server_access.wnd_10109, 1);
			sub_46ACE0(nox_gui_server_access.root, 10188, 10190, 1);
			nox_window_set_hidden(nox_gui_server_access.wnd_10105, 0);
			sub_46ACE0(nox_gui_server_access.root, 10185, 10187, 0);
			nox_gui_server_access.active_list = nox_gui_server_access.wnd_10105;
			return 0;
		default:
			return 0;
		}
	case 16400:
		if (!arg1) {
			return 0;
		}
		id = nox_xxx_wndGetID_46B0A0((nox_window*)arg1);
		if (id == 10200) {
			if (sub_455770()) {
				sub_46AD20(nox_gui_server_access.root, 10191, 10192, 1);
			} else {
				sub_46AD20(nox_gui_server_access.root, 10191, 10192, 0);
			}
		} else if (id == 10123) {
			settings = sub_416640();
			settings[100] ^= 1 << (int)arg2;
		}
		return 0;
	case 16415:
		if (!arg1) {
			return 0;
		}
		edit = (nox_window*)arg1;
		controls = sub_416630();
		settings = sub_416640();
		text = (wchar2_t*)nox_window_call_field_94(edit, 16413, 0, 0);
		value = 0;
		if (text && *text) {
			value = nox_wcstol(text, 0, 10);
			if (value < 0) {
				value = 0;
			}
		}
		switch (nox_xxx_wndGetID_46B0A0(edit)) {
		case 10104:
			if (text) {
				nox_wcscpy((wchar2_t*)settings + 39, text);
			}
			return 0;
		case 10126:
			if (value > 14) {
				value = 14;
				nox_itow(value, text_buf, 10);
				nox_window_call_field_94(edit, 16414, (uintptr_t)text_buf, UINTPTR_MAX);
			}
			controls[1] = value | controls[1] & 0xF0;
			return 0;
		case 10128:
			if (value > 14) {
				value = 14;
				nox_itow(value, text_buf, 10);
				nox_window_call_field_94(edit, 16414, (uintptr_t)text_buf, UINTPTR_MAX);
			}
			controls[1] = (16 * value) | controls[1] & 0xF;
			return 0;
		case 10130:
			*(uint16_t*)(controls + 5) = value;
			return 0;
		case 10132:
			*(uint16_t*)(controls + 7) = value;
			return 0;
		case 10133:
			if (value < 1) {
				value = 1;
				nox_itow(value, text_buf, 10);
				nox_window_call_field_94(edit, 16414, (uintptr_t)text_buf, UINTPTR_MAX);
			} else if (value > 32) {
				value = 32;
				nox_itow(value, text_buf, 10);
				nox_window_call_field_94(edit, 16414, (uintptr_t)text_buf, UINTPTR_MAX);
			}
			nox_xxx_servSetPlrLimit_409F80(value);
			controls[4] = value;
			return 0;
		case 10136:
			nox_xxx_sysopSetPass_40A610(text);
			return 0;
		default:
			return 0;
		}
	default:
		return 0;
	}
}

//----- (00455770) --------------------------------------------------------
int sub_455770(void) {
	if (!nox_gui_server_access.wnd_10200) {
		return 0;
	}
	int* selected = (int*)nox_window_call_field_94(nox_gui_server_access.wnd_10200, 16404, 0, 0);
	if (!selected) {
		return 0;
	}
	for (; *selected >= 0; ++selected) {
		wchar2_t* name =
			(wchar2_t*)nox_window_call_field_94(nox_gui_server_access.wnd_10200, 16406, *selected, 0);
		nox_playerInfo* player = nox_gui_server_access_player_by_name(name);
		if (player && player->playerInd != 31) {
			return 1;
		}
	}
	return 0;
}

//----- (004557D0) --------------------------------------------------------
int sub_4557D0(int a1) {
	int result = a1;
	if (a1 && nox_gui_server_access.root) {
		result = nox_xxx_windowDestroyMB_46C4E0(nox_gui_server_access.root);
	}
	memset(&nox_gui_server_access, 0, sizeof(nox_gui_server_access));
	return result;
}

//----- (00455800) --------------------------------------------------------
int* sub_455800(void) {
	wchar2_t WideCharStr[10]; // [esp+4h] [ebp-48h]
	wchar2_t v5[26];          // [esp+18h] [ebp-34h]

	if (!nox_gui_server_access.root) {
		return NULL;
	}
	int limit = nox_xxx_servGetPlrLimit_409FA0();
	nox_itow(limit, WideCharStr, 10);
	nox_window_call_field_94(nox_gui_server_access.wnd_10133, 16414, (uintptr_t)WideCharStr, 0);
	if (nox_common_gameFlags_check_40A5C0(1)) {
		nox_window_call_field_94(nox_gui_server_access.wnd_10105, 16399, 0, 0);
		nox_window_call_field_94(nox_gui_server_access.wnd_10109, 16399, 0, 0);
		for (nox_access_allowed_entry_t* entry = sub_4168E0(); entry; entry = sub_4168F0(entry)) {
			nox_window_call_field_94(
				nox_gui_server_access.wnd_10109, 16397, (uintptr_t)entry->name, UINTPTR_MAX);
		}
		for (nox_access_banned_entry_t* entry = sub_416900(); entry; entry = sub_416910(entry)) {
			if (entry->serial[0]) {
				nox_window_call_field_94(
					nox_gui_server_access.wnd_10105, 16397, (uintptr_t)entry->name, UINTPTR_MAX);
			} else {
				nox_swprintf(v5, L"*%s", entry->name);
				nox_window_call_field_94(
					nox_gui_server_access.wnd_10105, 16397, (uintptr_t)v5, UINTPTR_MAX);
			}
		}
	}
	return NULL;
}

//----- (00455920) --------------------------------------------------------
int sub_455920(wchar2_t* a1) {
	if (!nox_gui_server_access.root || !nox_gui_server_access.wnd_10200) {
		return 0;
	}
	return (int)nox_window_call_field_94(nox_gui_server_access.wnd_10200, 16397, (uintptr_t)a1, 3);
}

//----- (00455950) --------------------------------------------------------
void sub_455950(wchar2_t* a1) {
	if (nox_gui_server_access.root) {
		int index = sub_4559B0(a1);
		if (index != -1) {
			nox_window_call_field_94(nox_gui_server_access.wnd_10200, 16398, index, 0);
			if (!sub_455770()) {
				sub_46AD20(nox_gui_server_access.root, 10191, 10192, 0);
			}
		}
	}
}

//----- (004559B0) --------------------------------------------------------
int sub_4559B0(wchar2_t* a1) {
	if (!nox_gui_server_access.wnd_10200) {
		return -1;
	}
	nox_scrollListBox_data* list = nox_gui_server_access.wnd_10200->widget_data;
	if (!list || list->field_11_0 == 0) {
		return -1;
	}
	for (int i = 0; i < list->field_11_0; ++i) {
		const wchar2_t* name =
			(const wchar2_t*)nox_window_call_field_94(nox_gui_server_access.wnd_10200, 16406, i, 0);
		if (name && !nox_wcscmp(name, a1)) {
			return i;
		}
	}
	return -1;
}

//----- (00455A00) --------------------------------------------------------
int sub_455A00(int a1) {
	int result; // eax

	if (a1 && *getMemU32Ptr(0x5D4594, 1045608) &&
		(nox_xxx_wndGetFlags_46ADA0(dword_5d4594_1045604) & NOX_WIN_HIDDEN)) {
		result = nox_xxx_wndShowModalMB_46A8C0(dword_5d4594_1045604);
	} else {
		result = nox_window_set_hidden(dword_5d4594_1045604, 1);
	}
	return result;
}

//----- (00455A50) --------------------------------------------------------
char sub_455A50(char a1) {
	nox_window* v1; // edi
	int v2;       // eax
	int v3;       // esi
	int v4;       // edx

	v1 = 0;
	nox_xxx_objGetTeamByNetCode_418C80(nox_player_netCode_85319C);
	if (!(dword_5d4594_1045604 || (v2 = sub_455C30()) != 0)) {
		return v2;
	}
	int cur_w, cur_h, cur_d;
	nox_xxx_gameGetScreenBoundaries_43BEB0_get_video_mode(&cur_w, &cur_h, &cur_d);

	int max_w, max_h;
	nox_xxx_screenGetSize_430C50_get_video_max(&max_w, &max_h);

	if (cur_w > max_w) {
		cur_w = max_w;
	}
	if (cur_h > max_h) {
		cur_h = max_h;
	}

	*getMemU32Ptr(0x5D4594, 1045612) = 0;
	*getMemU32Ptr(0x5D4594, 1045616) = 0;
	*getMemU32Ptr(0x5D4594, 1045620) = 0;
	*getMemU32Ptr(0x5D4594, 1045608) = 1;
	*getMemU32Ptr(0x5D4594, 1045624) = 0;
	sub_46ACE0((uint32_t*)dword_5d4594_1045604, 8811, 8826, 1);
	v3 = 0;
	*getMemU8Ptr(0x5D4594, 1045628) = a1;
	if (a1) {
		do {
			v1 = nox_xxx_wndGetChildByID_46B0C0(dword_5d4594_1045604, v3 + 8811);
			nox_window_set_hidden(v1, 0);
			++v3;
		} while (v3 < getMemByte(0x5D4594, 1045628));
	}
	if (getMemByte(0x5D4594, 1045628) <= 4u) {
		v4 = cur_w - dword_5d4594_1045604->width / 2;
	} else {
		v4 = cur_w - dword_5d4594_1045604->width;
	}
	dword_5d4594_1045604->off_x = v4 - 91;
	dword_5d4594_1045604->end_x =
		dword_5d4594_1045604->width + dword_5d4594_1045604->off_x;
	LOBYTE(v2) = getMemByte(0x5D4594, 1045628);
	if (getMemByte(0x5D4594, 1045628) <= 8u) {
		if (getMemByte(0x5D4594, 1045628) <= 4u) {
			if (!v1) {
				*getMemU32Ptr(0x5D4594, 1045608) = 0;
				return v2;
			}
			dword_5d4594_1045604->off_y = cur_h - 40 * getMemByte(0x5D4594, 1045628);
		} else {
			dword_5d4594_1045604->off_y = cur_h - dword_5d4594_1045604->height / 2;
		}
	} else {
		dword_5d4594_1045604->off_y = cur_h - dword_5d4594_1045604->height;
	}
	dword_5d4594_1045604->end_y =
		dword_5d4594_1045604->height + dword_5d4594_1045604->off_y;
	LOBYTE(v2) = nox_xxx_wndShowModalMB_46A8C0(dword_5d4594_1045604);
	return v2;
}

//----- (00455C10) --------------------------------------------------------
int sub_455C10() {
	*getMemU32Ptr(0x5D4594, 1045608) = 0;
	return sub_455A00(0);
}

//----- (00455CD0) --------------------------------------------------------
int sub_455CD0(nox_window* win, nox_window_data* draw) {
	nox_video_bag_image_t* image;
	int v6;           // [esp+4h] [ebp-8h]
	int v7;           // [esp+8h] [ebp-4h]
	unsigned char flag_index = win->id - 8811;

	nox_client_wndGetPosition_46AA60(win, &v6, &v7);
	if (getMemByte(0x5D4594, 1045612 + flag_index) == 1) {
		image = draw->en_image;
	} else if (getMemByte(0x5D4594, 1045612 + flag_index) == 2) {
		image = draw->dis_image;
	} else {
		image = draw->bg_image;
	}
	if (image) {
		nox_client_drawImageAt_47D2C0(image, v6, v7);
	}
	if ((win->flags & NOX_WIN_LAYER_FRONT) && nox_ctf_flag_team_border) {
		nox_client_drawImageAt_47D2C0(nox_ctf_flag_team_border, v6 - 4, v7 - 4);
	}
	return 1;
}

//----- (00455E70) --------------------------------------------------------
int sub_455E70(unsigned char a1) {
	nox_window* v2; // eax
	nox_window* v3; // eax

	for (int i = 0; i < getMemByte(0x5D4594, 1045628); i++) {
		v2 = nox_xxx_wndGetChildByID_46B0C0(dword_5d4594_1045604, i + 8811);
		nox_xxx_wndClearFlag_46AD80(v2, 32);
	}
	v3 = nox_xxx_wndGetChildByID_46B0C0(dword_5d4594_1045604, a1 + 8810);
	return nox_xxx_wnd_46AD60(v3, 32);
}

//----- (00455EE0) --------------------------------------------------------
int sub_455EE0() {
	int result; // eax

	if (dword_5d4594_1045604) {
		result = nox_xxx_windowDestroyMB_46C4E0(dword_5d4594_1045604);
	} else {
		result = 0;
	}
	dword_5d4594_1045604 = 0;
	*getMemU32Ptr(0x5D4594, 1045608) = 0;
	return result;
}

//----- (00455F10) --------------------------------------------------------
int sub_455F10(int a1) {
	int result; // eax

	if (a1 && dword_5d4594_1045640 &&
		(nox_xxx_wndGetFlags_46ADA0(dword_5d4594_1045636) & NOX_WIN_HIDDEN)) {
		result = nox_xxx_wndShowModalMB_46A8C0(dword_5d4594_1045636);
	} else {
		result = nox_window_set_hidden(dword_5d4594_1045636, 1);
	}
	return result;
}

//----- (00455F60) --------------------------------------------------------
int sub_455F60() {
	int result; // eax
	int v1;     // ecx
	int v2;     // [esp+0h] [ebp-14h]
	int v3;     // [esp+4h] [ebp-10h]
	int v4;     // [esp+8h] [ebp-Ch]
	int v5;     // [esp+Ch] [ebp-8h]
	int v6;     // [esp+10h] [ebp-4h]

	nox_xxx_objGetTeamByNetCode_418C80(nox_player_netCode_85319C);
	if (dword_5d4594_1045636 || (result = sub_456070()) != 0) {
		nox_xxx_gameGetScreenBoundaries_43BEB0_get_video_mode(&v3, &v2, &v6);
		nox_xxx_screenGetSize_430C50_get_video_max(&v4, &v5);
		v1 = v3;
		if (v3 > v4) {
			v1 = v4;
			v3 = v4;
		}
		if (v2 > v5) {
			v2 = v5;
		}
		dword_5d4594_1045640 = 1;
		dword_5d4594_1045636->off_x = v1 - dword_5d4594_1045636->width / 3 - 91;
		dword_5d4594_1045636->end_x = dword_5d4594_1045636->width + dword_5d4594_1045636->off_x;
		dword_5d4594_1045636->off_y = v2 - 120;
		dword_5d4594_1045636->end_y = dword_5d4594_1045636->height + dword_5d4594_1045636->off_y;
		*getMemU8Ptr(0x5D4594, 1045644) = 0;
		result = nox_xxx_wndShowModalMB_46A8C0(dword_5d4594_1045636);
	}
	return result;
}

//----- (00456050) --------------------------------------------------------
int sub_456050() {
	dword_5d4594_1045640 = 0;
	return sub_455F10(0);
}

//----- (00456070) --------------------------------------------------------
int sub_456070() {
	if (!dword_5d4594_1045636) {
		dword_5d4594_1045636 = nox_new_window_from_file("gui_fb.wnd", 0);
		if (!dword_5d4594_1045636) {
			return 0;
		}
		nox_window_set_all_funcs(dword_5d4594_1045636, 0, sub_4560D0, 0);
		sub_455F10(0);
		nox_fb_flag_team_border = nox_xxx_gLoadImg_42F970("FlagTeamBorder");
	}
	return 1;
}

//----- (004560D0) --------------------------------------------------------
int sub_4560D0(nox_window* win, nox_window_data* draw) {
	int v4; // [esp+4h] [ebp-8h]
	int v5; // [esp+8h] [ebp-4h]

	nox_client_wndGetPosition_46AA60(win, &v4, &v5);
	if (draw->bg_image) {
		nox_client_drawImageAt_47D2C0(draw->bg_image, v4, v5);
	}
	if ((win->flags & NOX_WIN_LAYER_FRONT) && nox_fb_flag_team_border) {
		nox_client_drawImageAt_47D2C0(nox_fb_flag_team_border, v4 - 4, v5 - 4);
	}
	return 1;
}

//----- (00456240) --------------------------------------------------------
int sub_456240() {
	int result; // eax

	if (dword_5d4594_1045636) {
		result = nox_xxx_windowDestroyMB_46C4E0(dword_5d4594_1045636);
	} else {
		result = 0;
	}
	dword_5d4594_1045636 = 0;
	dword_5d4594_1045640 = 0;
	return result;
}

static void nox_gui_server_players_clear_list(nox_list_item_t* head);

//----- (00456500) --------------------------------------------------------
int sub_456500() {
	if (!dword_5d4594_1045684) {
		return 0;
	}
	nox_window* players = nox_xxx_wndGetChildByID_46B0C0(dword_5d4594_1045684, 10501);
	nox_window* teams = nox_xxx_wndGetChildByID_46B0C0(dword_5d4594_1045684, 10502);
	if (!players || !teams) {
		return 0;
	}
	nox_window_call_field_94(players, 16399, 0, 0);
	nox_window_call_field_94(teams, 16399, 0, 0);
	nox_gui_server_players_clear_list(&nox_gui_server_players.players);
	nox_gui_server_players_clear_list(&nox_gui_server_players.teams);
	nox_xxx_wnd_46ABB0(dword_5d4594_1045688, 0);
	nox_xxx_wnd_46ABB0(dword_5d4594_1045692, 0);

	for (nox_team_t* team = nox_server_teamFirst_418B10(); team; team = nox_server_teamNext_418B60(team)) {
		sub_457230(team->name);
	}
	for (nox_playerInfo* player = nox_common_playerInfoGetFirst_416EA0(); player;
		 player = nox_common_playerInfoGetNext_416EE0(player)) {
		if (player->playerInd == 31 && nox_common_getEngineFlag(NOX_ENGINE_FLAG_DISABLE_GRAPHICS_RENDERING)) {
			continue;
		}
		sub_457140(player->netCode, player->name_final);
		nox_object_team_t* value = nox_xxx_objGetTeamByNetCode_418C80(player->netCode);
		if (value && nox_xxx_servObjectHasTeam_419130(value)) {
			sub_4571A0(player->netCode, value->id);
		}
	}
	int visible = !nox_common_gameFlags_check_40A5C0(4096);
	nox_xxx_wnd_46ABB0(players, visible);
	return nox_xxx_wnd_46ABB0(teams, visible);
}

//----- (00456640) --------------------------------------------------------
int sub_456640(nox_window* win, nox_window_data* draw) {
	unsigned int x_left;
	unsigned int y_top;

	if (!win || !draw || !dword_5d4594_1045684) {
		return 0;
	}
	nox_client_wndGetPosition_46AA60(win, &x_left, &y_top);
	if ((int8_t)win->flags >= 0) {
		if (draw->bg_color != 0x80000000) {
			nox_client_drawRectFilledAlpha_49CF10(x_left, y_top, (int)win->width, (int)win->height);
		}
	} else if (draw->bg_image) {
		nox_client_drawImageAt_47D2C0(draw->bg_image, x_left, y_top);
	}
	if (nox_common_gameFlags_check_40A5C0(1) && !nox_common_gameFlags_check_40A5C0(0x8000) && !sub_40A740()) {
		nox_window* teams = nox_xxx_wndGetChildByID_46B0C0(dword_5d4594_1045684, 10502);
		nox_window* assign = nox_xxx_wndGetChildByID_46B0C0(dword_5d4594_1045684, 10503);
		if (!teams || !assign) {
			return 1;
		}
		int team_selected = (int)nox_window_call_field_94(teams, 16404, 0, 0);
		if (team_selected < 0) {
			nox_xxx_wnd_46ABB0(assign, 0);
		} else {
			nox_window* players = nox_xxx_wndGetChildByID_46B0C0(dword_5d4594_1045684, 10501);
			int* selections = players ? (int*)(uintptr_t)nox_window_call_field_94(players, 16404, 0, 0) : NULL;
			nox_xxx_wnd_46ABB0(assign, selections && *selections >= 0);
		}
	}
	nox_window* teams = nox_xxx_wndGetChildByID_46B0C0(dword_5d4594_1045684, 10502);
	if (teams && (int)nox_window_call_field_94(teams, 16404, 0, 0) < 0) {
		nox_xxx_wnd_46ABB0(dword_5d4594_1045688, 0);
		nox_xxx_wnd_46ABB0(dword_5d4594_1045692, 0);
	}
	return 1;
}

//----- (00456BB0) --------------------------------------------------------
void sub_456BB0(nox_team_t* team) {
	nox_playerInfo* local = (nox_playerInfo*)(uintptr_t)dword_8531A0_2576;
	if (!team || (local && ((local->field_4 & 1) || (local->field_3680 & 1)))) {
		return;
	}
	int server = nox_common_gameFlags_check_40A5C0(1);
	nox_object_team_t* value = nox_xxx_objGetTeamByNetCode_418C80(nox_player_netCode_85319C);
	if (!value) {
		return;
	}
	if (nox_xxx_servObjectHasTeam_419130(value)) {
		if (!server) {
			sub_419960(team, value, nox_player_netCode_85319C);
			++*getMemU32Ptr(0x5D4594, 1045696);
			return;
		}
		sub_4196D0(value, team, nox_player_netCode_85319C, 1);
		if (!nox_common_gameFlags_check_40A5C0(128)) {
			nox_playerInfo* player = nox_common_playerInfoGetByID_417040(nox_player_netCode_85319C);
			if (player && player->playerUnit) {
				float2 pos;
				nox_xxx_mapFindPlayerStart_4F7AB0(&pos, player->playerUnit);
				nox_xxx_unitMove_4E7010(player->playerUnit, &pos);
			}
		}
	} else if (server) {
		nox_xxx_createAtImpl_4191D0(team->field_57, value, server, nox_player_netCode_85319C, 1);
	} else {
		sub_419900(team, value, nox_player_netCode_85319C);
	}
	++*getMemU32Ptr(0x5D4594, 1045696);
}

//----- (00456D00) --------------------------------------------------------
wchar2_t* sub_456D00(int a1, wchar2_t* a2) {
	if (!a2 || !dword_5d4594_1045684 || a1 < 0) {
		return NULL;
	}
	nox_window* teams = nox_xxx_wndGetChildByID_46B0C0(dword_5d4594_1045684, 10502);
	if (!teams) {
		return NULL;
	}
	const wchar2_t* line = (const wchar2_t*)nox_window_call_field_94(teams, 16406, a1, 0);
	if (!line) {
		return NULL;
	}
	wchar2_t buf[56];
	nox_wcscpy(buf, line);
	wchar2_t* name = nox_wcstok(buf, L"\t\n\r");
	return name ? nox_wcscpy(a2, name) : NULL;
}

static void nox_gui_server_players_clear_list(nox_list_item_t* head) {
	for (nox_list_item_t* item = nox_common_list_getFirstSafe_425890(head); item;) {
		nox_list_item_t* next = nox_common_list_getNextSafe_4258A0(item);
		nox_common_list_remove_425920(item);
		free(item);
		item = next;
	}
	nox_common_list_clear_425760(head);
}

//----- (00456D60) --------------------------------------------------------
int* sub_456D60(int a1) {
	if (a1 && dword_5d4594_1045684) {
		nox_xxx_windowDestroyMB_46C4E0(dword_5d4594_1045684);
	}
	nox_gui_server_players_clear_list(&nox_gui_server_players.players);
	nox_gui_server_players_clear_list(&nox_gui_server_players.teams);
	dword_5d4594_1045684 = 0;
	dword_5d4594_1045688 = 0;
	dword_5d4594_1045692 = 0;
	return NULL;
}

//----- (00456DF0) --------------------------------------------------------
int sub_456DF0(int a1) {
	if (!dword_5d4594_1045684) {
		return 0;
	}
	int index = sub_456E40(a1, 1);
	if (index < 0) {
		return -1;
	}
	nox_window* players = nox_xxx_wndGetChildByID_46B0C0(dword_5d4594_1045684, 10501);
	if (!players) {
		return 0;
	}
	return (int)nox_window_call_field_94(players, 16398, index, 0);
}

//----- (00456E40) --------------------------------------------------------
int sub_456E40(int a1, int a2) {
	int index = 0;
	for (nox_gui_server_player_entry* entry =
			 (nox_gui_server_player_entry*)nox_common_list_getFirstSafe_425890(&nox_gui_server_players.players);
		 entry; entry = (nox_gui_server_player_entry*)nox_common_list_getNextSafe_4258A0(&entry->list), ++index) {
		if (entry->net_code != (uint32_t)a1) {
			continue;
		}
		if (a2) {
			nox_common_list_remove_425920(&entry->list);
			free(entry);
		}
		return index;
	}
	return -1;
}

//----- (00456EA0) --------------------------------------------------------
int sub_456EA0(wchar2_t* a1) {
	if (!dword_5d4594_1045684) {
		return 0;
	}
	nox_window* teams = nox_xxx_wndGetChildByID_46B0C0(dword_5d4594_1045684, 10502);
	if (!teams) {
		return 0;
	}
	int index = sub_456F10(a1, 1);
	if (index >= 0) {
		nox_window_call_field_94(teams, 16404, 0, 0);
		nox_window_call_field_94(teams, 16398, index, 0);
	}
	return sub_456500();
}

//----- (00456F10) --------------------------------------------------------
int sub_456F10(wchar2_t* a1, int a2) {
	if (!a1) {
		return -1;
	}
	int index = 0;
	for (nox_gui_server_team_entry* entry =
			 (nox_gui_server_team_entry*)nox_common_list_getFirstSafe_425890(&nox_gui_server_players.teams);
		 entry; entry = (nox_gui_server_team_entry*)nox_common_list_getNextSafe_4258A0(&entry->list), ++index) {
		if (_nox_wcsicmp(entry->name, a1)) {
			continue;
		}
		if (a2) {
			nox_common_list_remove_425920(&entry->list);
			free(entry);
		}
		return index;
	}
	return -1;
}

//----- (00456FA0) --------------------------------------------------------
int sub_456FA0() {
	if (!dword_5d4594_1045684) {
		return 0;
	}
	nox_gui_server_players_clear_list(&nox_gui_server_players.teams);
	nox_xxx_wnd_46ABB0(dword_5d4594_1045688, 0);
	nox_xxx_wnd_46ABB0(dword_5d4594_1045692, 0);
	return sub_456500();
}

//----- (00457120) --------------------------------------------------------
unsigned char sub_457120(nox_team_t* team) {
	return team ? getMemByte(0x587000, 128968 + 8 * (team->def_ind % 10)) : 3;
}

//----- (00457140) --------------------------------------------------------
int sub_457140(int a1, wchar2_t* a2) {
	if (!dword_5d4594_1045684 || !a2) {
		return 0;
	}
	nox_window* players = nox_xxx_wndGetChildByID_46B0C0(dword_5d4594_1045684, 10501);
	if (!players) {
		return 0;
	}
	nox_gui_server_player_entry* entry = calloc(1, sizeof(*entry));
	if (!entry) {
		return 0;
	}
	sub_425770(&entry->list);
	nox_common_list_append_4258E0(&nox_gui_server_players.players, &entry->list);
	nox_wcscpy(entry->name, a2);
	entry->net_code = a1;
	return (int)nox_window_call_field_94(players, 16397, (uintptr_t)a2, 3);
}

static uint32_t nox_gui_server_players_color_value(int index) {
	switch (index) {
	case 0: return nox_color_black_2650656;
	case 1: return *getMemU32Ptr(0x852978, 4);
	case 2: return *getMemU32Ptr(0x85B3FC, 956);
	case 3: return *getMemU32Ptr(0x5D4594, 2597996);
	case 4: return nox_color_white_2523948;
	case 5: return nox_color_violet_2598268;
	case 6: return *getMemU32Ptr(0x85B3FC, 940);
	case 7: return nox_color_red_2589776;
	case 8: return *getMemU32Ptr(0x85B3FC, 984);
	case 9: return dword_8531A0_2572;
	case 10: return nox_color_green_2614268;
	case 11: return *getMemU32Ptr(0x85B3FC, 944);
	case 12: return nox_color_cyan_2649820;
	case 13: return nox_color_blue_2650684;
	case 14: return nox_color_orange_2614256;
	case 15: return nox_color_yellow_2589772;
	case 16: return *getMemU32Ptr(0x852978, 0);
	default: return *getMemU32Ptr(0x5D4594, 2597996);
	}
}

//----- (004571A0) --------------------------------------------------------
int sub_4571A0(int a1, int a2) {
	if (!dword_5d4594_1045684) {
		return 0;
	}
	nox_window* players = nox_xxx_wndGetChildByID_46B0C0(dword_5d4594_1045684, 10501);
	nox_scrollListBox_data* data = players ? players->widget_data : NULL;
	int index = sub_456E40(a1, 0);
	if (!data || !data->items || index < 0 || index >= data->field_11_0) {
		return 0;
	}
	int color = 3;
	if (a2) {
		for (nox_gui_server_team_entry* entry =
				 (nox_gui_server_team_entry*)nox_common_list_getFirstSafe_425890(&nox_gui_server_players.teams);
			 entry; entry = (nox_gui_server_team_entry*)nox_common_list_getNextSafe_4258A0(&entry->list)) {
			if (entry->team_id == (uint32_t)a2) {
				color = entry->color;
				break;
			}
		}
	}
	data->items[index].field_129 = nox_gui_server_players_color_value(color);
	return 1;
}

//----- (00457350) --------------------------------------------------------
void sub_457350(int old_id, int new_id) {
	uint32_t old_value = (uint8_t)old_id;
	for (nox_gui_server_team_entry* entry =
			 (nox_gui_server_team_entry*)nox_common_list_getFirstSafe_425890(&nox_gui_server_players.teams);
		 entry; entry = (nox_gui_server_team_entry*)nox_common_list_getNextSafe_4258A0(&entry->list)) {
		if (entry->team_id == old_value) {
			entry->team_id = (uint8_t)new_id;
		}
	}
}

//----- (004573A0) --------------------------------------------------------
int sub_4573A0() {
	return dword_5d4594_1045684 ? sub_456500() : 0;
}

//----- (004573B0) --------------------------------------------------------
void sub_4573B0() { *getMemU32Ptr(0x5D4594, 1045696) = 0; }

//----- (00457460) --------------------------------------------------------
int sub_457460(int a1) {
	wchar2_t WideCharStr[16]; // [esp+4h] [ebp-20h]

	nox_itow(*(unsigned short*)(a1 + 54), WideCharStr, 10);
	nox_window_call_field_94(*(int*)&dword_5d4594_1046516, 16414, (int)WideCharStr, 0);
	nox_itow(*(unsigned char*)(a1 + 56), WideCharStr, 10);
	return nox_window_call_field_94(*(int*)&dword_5d4594_1046520, 16414, (int)WideCharStr, 0);
}

//----- (004574D0) --------------------------------------------------------
int nox_xxx_guiServerOptionsTryHide_4574D0() {
	if (!dword_5d4594_1046492) {
		return 0;
	}
	nox_xxx_guiServerOptionsHide_4597E0(0);
	dword_5d4594_1046492 = 0;
	return 1;
}

//----- (00457B60) --------------------------------------------------------
int sub_457B60(char* a1) {
	char* v1;                // eax
	nox_window* v2;           // eax
	char* v3;                // eax
	unsigned short v4;       // di
	char* v5;                // esi
	nox_window* v6;           // eax
	unsigned short v7;       // ax
	unsigned char v8;        // al
	nox_window* v9;           // eax
	nox_window* v10;          // eax
	nox_window* v11;          // edi
	nox_window* v13;          // edi
	nox_window* v15;          // edi
	wchar2_t* v16;            // eax
	wchar2_t WideCharStr[16]; // [esp+0h] [ebp-E8h]
	wchar2_t v20[100];        // [esp+20h] [ebp-C8h]

	v1 = nox_xxx_serverOptionsGetServername_40A4C0();
	nox_swprintf(v20, L"%S", v1);
	nox_window_call_field_94(dword_5d4594_1046512, 16414, (uintptr_t)v20, 0);
	sub_459CD0();
	if (nox_common_gameFlags_check_40A5C0(128)) {
		v2 = nox_xxx_wndGetChildByID_46B0C0(dword_5d4594_1046492, 10122);
	} else {
		v2 = nox_xxx_wndGetChildByID_46B0C0(dword_5d4594_1046492, 10119);
	}
	nox_window_set_hidden(v2, 1);
	if (nox_common_gameFlags_check_40A5C0(1)) {
		nox_xxx_wnd_46ABB0(dword_5d4594_1046508, 1);
		if (nox_common_gameFlags_check_40A5C0(128)) {
			v3 = nox_xxx_cliGamedataGet_416590(1);
			v4 = *((uint16_t*)v3 + 26) & 0x17F0;
			nox_client_guiserv_updateMapList_458230(v4, v3, 1);
			v5 = sub_4165B0();
			*((uint16_t*)v5 + 26) = v4 | *((uint16_t*)v5 + 26) & 0xE80F;
			if (sub_40A740()) {
				nox_xxx_wnd_46ABB0(dword_5d4594_1046508, 0);
			} else if (nox_xxx_CheckGameplayFlags_417DA0(4)) {
				v6 = nox_xxx_wndGetChildByID_46B0C0(dword_5d4594_1046508, 10330);
				v6->draw_data.field_0 |= 4u;
			} else {
				sub_46AD20(dword_5d4594_1046508, 10331, 10333, 0);
			}
		} else {
			if (nox_xxx_CheckGameplayFlags_417DA0(4)) {
				sub_46AD20(dword_5d4594_1046508, 10330, 10331, 0);
			} else {
				sub_46AD20(dword_5d4594_1046508, 10330, 10333, 0);
			}
			v5 = a1;
			nox_client_guiserv_updateMapList_458230(*(unsigned short*)(a1 + 52), a1, 0);
		}
		v7 = nox_xxx_servGamedataGet_40A020(*((uint16_t*)v5 + 26));
		nox_itow(v7, WideCharStr, 10);
		nox_window_call_field_94(dword_5d4594_1046516, 16414, (uintptr_t)WideCharStr, 0);
		v8 = sub_40A180(*((uint16_t*)v5 + 26));
		nox_itow(v8, WideCharStr, 10);
		nox_window_call_field_94(dword_5d4594_1046520, 16414, (uintptr_t)WideCharStr, 0);
		sub_4580E0(v5);
	} else {
		v9 = nox_xxx_wndGetChildByID_46B0C0(dword_5d4594_1046492, 10161);
		nox_xxx_wnd_46ABB0(v9, 0);
		nox_xxx_wnd_46ABB0(dword_5d4594_1046536, 0);
		dword_5d4594_1046536->flags |= 8u;
		nox_xxx_wnd_46ABB0(dword_5d4594_1046504, 1);
		sub_46AD20(dword_5d4594_1046504, 10134, 10135, 0);
		sub_46ACE0(dword_5d4594_1046492, 10141, 10141, 1);
		v5 = a1;
		nox_client_guiserv_updateMapList_458230(*(unsigned short*)(a1 + 52), a1, 0);
		nox_itow(*(unsigned short*)(a1 + 54), WideCharStr, 10);
		nox_window_call_field_94(dword_5d4594_1046516, 16414, (uintptr_t)WideCharStr, 0);
		nox_itow(*(unsigned char*)(a1 + 56), WideCharStr, 10);
		nox_window_call_field_94(dword_5d4594_1046520, 16414, (uintptr_t)WideCharStr, 0);
		if (0) {
			v10 = nox_xxx_wndGetChildByID_46B0C0(dword_5d4594_1046492, 10210);
			nox_window_set_hidden(v10, 0);
			sub_457FE0();
		}
	}
	v11 = nox_xxx_wndGetChildByID_46B0C0(dword_5d4594_1046508, 10331);
	if (nox_xxx_CheckGameplayFlags_417DA0(2)) {
		v11->draw_data.field_0 |= 4;
	}
	v13 = nox_xxx_wndGetChildByID_46B0C0(dword_5d4594_1046508, 10333);
	if (nox_xxx_CheckGameplayFlags_417DA0(1)) {
		v13->draw_data.field_0 |= 4;
	}
	v15 = nox_xxx_wndGetChildByID_46B0C0(dword_5d4594_1046492, 10119);
	v16 = nox_xxx_guiServerOptionsGetGametypeName_4573C0(*((uint16_t*)v5 + 26));
	nox_window_call_field_94(v15, 16385, (uintptr_t)v16, 0);
	sub_459880(v5);
	return sub_457F30((*((uint16_t*)v5 + 26) >> 12) & 1);
}
// 457F15: variable 'v17' is possibly undefined

//----- (00457F30) --------------------------------------------------------
int sub_457F30(int a1) {
	int v1;       // esi
	nox_window* v2; // eax
	nox_window* v3; // eax
	nox_window* v4; // esi
	nox_window* v5; // eax

	v1 = a1 != 1;
	v2 = nox_xxx_wndGetChildByID_46B0C0(dword_5d4594_1046504, 10152);
	nox_xxx_wnd_46ABB0(v2, v1);
	v3 = nox_xxx_wndGetChildByID_46B0C0(dword_5d4594_1046504, 10141);
	nox_xxx_wnd_46ABB0(v3, v1);
	nox_xxx_wnd_46ABB0(dword_5d4594_1046516, v1);
	nox_xxx_wnd_46ABB0(dword_5d4594_1046520, v1);
	if (a1 == 1) {
		v4 = nox_xxx_wndGetChildByID_46B0C0(dword_5d4594_1046492, 10122);
		if (!wndIsShown_nox_xxx_wndIsShown_46ACC0(v4)) {
			nox_window_set_hidden(v4, 1);
		}
	}
	v5 = nox_xxx_wndGetChildByID_46B0C0(dword_5d4594_1046492, 10183);
	return nox_window_set_hidden(v5, a1);
}

// 458291: variable 'v5' is possibly undefined

//----- (00458500) --------------------------------------------------------
int nox_xxx_windowServerOptionsDrawProc_458500(nox_window* a1, int a2) {
	nox_window* v2; // esi
	int xLeft;    // [esp+8h] [ebp-8h]
	int v5;       // [esp+Ch] [ebp-4h]

	nox_point mpos = nox_client_getMousePos_4309F0();
	nox_client_wndGetPosition_46AA60(a1, &xLeft, &v5);
	nox_client_drawRectFilledAlpha_49CF10(xLeft, v5 + 25, a1->width, a1->height - 25);
	v2 = nox_xxx_wndGetChildByID_46B0C0(dword_5d4594_1046492, 10120);
	if (!wndIsShown_nox_xxx_wndIsShown_46ACC0(v2) && !nox_xxx_wndPointInWnd_46AAB0(v2, mpos.x, mpos.y)) {
		nox_xxx_wndClearCaptureMain_46ADE0(v2);
		nox_window_set_hidden(v2, 1);
	}
	return 1;
}

//----- (00458590) --------------------------------------------------------
int nox_xxx_guiServerOptionsProc_458590(nox_window* win, int event, uintptr_t a3, uintptr_t a4) {
	(void)win;
	if (event == 21) {
		if (a3 != 1) {
			return 0;
		}
		if (a4 == 2) {
			nox_xxx_clientPlaySoundSpecial_452D80(231, 100);
			nox_xxx_guiServerOptionsHide_4597E0(0);
		}
	}
	return 1;
}

// 458F5A: variable 'v34' is possibly undefined

//----- (00459150) --------------------------------------------------------
void sub_4537F0();
char sub_459150() {
	char* v0;          // ebp
	int i;             // edi
	char* v2;          // esi
	int v3;            // ebx
	wchar2_t* v4;       // eax
	char* v5;          // eax
	const char* v6;    // edi
	char* v7;          // eax
	int v8;            // ebx
	char* v9;          // esi
	unsigned char v10; // dl
	char* v11;         // edi
	char* v12;         // eax
	char* v13;         // eax
	int* v14;          // eax
	int* v16;          // [esp-Ch] [ebp-2Ch]
	int* v17;          // [esp-4h] [ebp-24h]
	short v18;         // [esp-4h] [ebp-24h]
	char v19[16];      // [esp+10h] [ebp-10h]

	v0 = sub_4165B0();
	if (v0[52] & 0x60) {
		for (i = nox_xxx_getTeamCounter_417DD0(); i < 2; ++i) {
			v2 = nox_xxx_teamCreate_4186D0(0);
			v3 = (i != 0) + 1;
			if (sub_40A740()) {
				sub_418800((wchar2_t*)v2, (wchar2_t*)getMemAt(0x5D4594, 1046564), 0);
			} else {
				v4 = nox_server_teamTitle_418C20(v3);
				sub_418800((wchar2_t*)v2, v4, 1);
			}
			v2[56] = v3;
			sub_4184D0((wchar2_t*)v2);
		}
		nox_xxx_SetGameplayFlag_417D50(4);
	}
	LOBYTE(v5) = v0[57];
	if ((uint8_t)v5) {
		v5 = sub_409B80();
		v6 = v5;
	} else {
		v6 = v0;
	}
	if (*v6) {
		v7 = nox_xxx_mapGetMapName_409B40();
		v8 = nox_strcmpi(v6, v7);
		sub_4165F0(1, 0);
		v9 = sub_4165D0(0);
		nox_xxx_gameSetServername_40A440(v9 + 9);
		if (!((*((uint16_t*)v9 + 26) >> 8) & 0x10)) {
			sub_409FB0_settings(*((uint16_t*)v9 + 26), *((uint16_t*)v9 + 27));
			sub_40A040_settings(*((uint16_t*)v9 + 26), v9[56]);
		}
		if (v8 && 0) {
			if (!nox_common_gameFlags_check_40A5C0(128)) {
				nox_xxx_net_4263C0();
				sub_4264D0();
				sub_416150(15, 0);
			}
			if (!v9[57]) {
				sub_41D650();
			}
		}
		if (nox_common_gameFlags_check_40A5C0(128)) {
			nox_common_gameFlags_unset_40A540(49152);
			nox_xxx_setGameFlags_40A4D0(*((uint16_t*)v9 + 26) & 0xC000);
		}
		v9[57] = 0;
		if (v8) {
			v19[0] = 0;
			*(uint32_t*)&v19[1] = 0;
			*(uint32_t*)&v19[5] = 0;
			*(uint32_t*)&v19[9] = 0;
			v10 = getMemByte(0x587000, 131672);
			strcpy(v19, v6);
			v11 = &v19[strlen(v19)];
			*(uint32_t*)v11 = *getMemU32Ptr(0x587000, 131668);
			v11[4] = v10;
			nox_xxx_mapLoad_4D2450(v19);
			sub_416690();
			sub_4165D0(1);
			LOBYTE(v5) = (unsigned int)nox_xxx_guiServerOptionsHide_4597E0(0);
		} else {
			nox_xxx_spellEnableAll_424BD0();
			sub_4537F0();
			v17 = (int*)sub_459870();
			v12 = nox_xxx_cliGamedataGet_416590(1);
			sub_57AAA0("user.rul", v12, v17);
			v13 = nox_server_currentMapGetFilename_409B30();
			sub_57A950(v13);
			v18 = nox_common_gameFlags_getVal_40A5B0();
			v16 = (int*)sub_459870();
			v14 = (int*)nox_xxx_cliGamedataGet_416590(0);
			sub_57A1E0(v14, "user.rul", v16, 3, v18);
			LOBYTE(v5) = (unsigned int)nox_xxx_guiServerOptionsHide_4597E0(0);
		}
	}
	return (char)v5;
}

//----- (004593B0) --------------------------------------------------------
int sub_4593B0(int a1) {
	nox_window* v1; // esi
	nox_window* v2; // ebx
	nox_window* v3; // edi
	int result = 0; // eax

	if (!dword_5d4594_1046492) {
		return 0;
	}
	v1 = nox_xxx_wndGetChildByID_46B0C0(dword_5d4594_1046492, 10163);
	v2 = nox_xxx_wndGetChildByID_46B0C0(dword_5d4594_1046492, 10161);
	v3 = nox_xxx_wndGetChildByID_46B0C0(dword_5d4594_1046492, 10162);
	if (!v1 || !v2 || !v3) {
		return 0;
	}
	if (a1) {
		if (a1 == 1) {
			v1->draw_data.bg_image = dword_5d4594_1046360;
			v3->draw_data.bg_image = dword_5d4594_1046356;
			if (dword_5d4594_1046532) {
				sub_456D60(1);
				dword_5d4594_1046532 = 0;
			}
			if (dword_5d4594_1046540) {
				sub_4AD820();
				dword_5d4594_1046540 = 0;
			}
			dword_5d4594_1046528 = nox_xxx_guiServerAccessLoad_4541D0(dword_5d4594_1046492);
			result = sub_459560(10161);
		} else {
			result = a1 - 2;
			if (a1 == 2) {
				v1->draw_data.bg_image = dword_5d4594_1046360;
				if (nox_common_gameFlags_check_40A5C0(1)) {
					v2->draw_data.bg_image = dword_5d4594_1046360;
				} else {
					v2->draw_data.dis_image = dword_5d4594_1046360;
				}
				if (dword_5d4594_1046528) {
					sub_4557D0(1);
					dword_5d4594_1046528 = 0;
				}
				if (dword_5d4594_1046540) {
					sub_4AD820();
					dword_5d4594_1046540 = 0;
				}
				dword_5d4594_1046532 = nox_xxx_guiServerPlayersLoad_456270(dword_5d4594_1046492);
				result = sub_459560(10162);
			}
		}
	} else {
		if (nox_common_gameFlags_check_40A5C0(1)) {
			v2->draw_data.bg_image = dword_5d4594_1046356;
		} else {
			v2->draw_data.dis_image = dword_5d4594_1046356;
		}
		v3->draw_data.bg_image = dword_5d4594_1046356;
		if (dword_5d4594_1046532) {
			sub_456D60(1);
			dword_5d4594_1046532 = 0;
		} else if (dword_5d4594_1046528) {
			sub_4557D0(1);
			dword_5d4594_1046528 = 0;
		}
		dword_5d4594_1046540 = nox_xxx_gui_4AD320(dword_5d4594_1046492);
		result = sub_459560(10163);
	}
	return result;
}

//----- (00459560) --------------------------------------------------------
int sub_459560(int a1) {
	if (!dword_5d4594_1046492) {
		return 0;
	}
	nox_window* selected = nox_xxx_wndGetChildByID_46B0C0(dword_5d4594_1046492, a1);
	if (!selected || !selected->parent) {
		return 0;
	}
	nox_window* parent = selected->parent;
	nox_window* first = NULL;
	nox_window* second = NULL;
	switch (a1) {
	case 10161:
		first = nox_xxx_wndGetChildByID_46B0C0(dword_5d4594_1046492, 10163);
		second = nox_xxx_wndGetChildByID_46B0C0(dword_5d4594_1046492, 10162);
		break;
	case 10162:
		first = nox_xxx_wndGetChildByID_46B0C0(dword_5d4594_1046492, 10163);
		second = nox_xxx_wndGetChildByID_46B0C0(dword_5d4594_1046492, 10161);
		break;
	case 10163:
		first = nox_xxx_wndGetChildByID_46B0C0(dword_5d4594_1046492, 10162);
		second = nox_xxx_wndGetChildByID_46B0C0(dword_5d4594_1046492, 10161);
		break;
	default:
		return 0;
	}
	if (!first || !second) {
		return 0;
	}
	sub_46B120(first, NULL);
	sub_46B120(first, parent);
	sub_46B120(second, NULL);
	sub_46B120(second, parent);
	sub_46B120(selected, NULL);
	return sub_46B120(selected, parent);
}

//----- (00459700) --------------------------------------------------------
int sub_459700() {
	char* v0;          // esi
	int v1;            // eax
	int v2;            // edi
	const wchar2_t* v3; // eax
	wchar2_t v5[256] = {0};   // [esp+8h] [ebp-200h]

	sub_416580();
	v0 = sub_4165B0();
	v1 = nox_window_call_field_94(*(int*)&dword_5d4594_1046496, 16404, 0, 0);
	v2 = v1;
	v3 = (const wchar2_t*)nox_window_call_field_94(*(int*)&dword_5d4594_1046496, 16406, v1, 0);
	if (v3) {
		nox_wcsncpy(v5, v3, 255);
		v5[255] = 0;
	} else {
		v5[0] = 0;
	}
	nox_window_call_field_94(*(int*)&dword_5d4594_1046496, 16398, v2, 0);
	nox_window_call_field_94(*(int*)&dword_5d4594_1046496, 16402, v2, 0);
	nox_window_call_field_94(*(int*)&dword_5d4594_1046496, 16397, (int)v5, -1);
	nox_window_call_field_94(*(int*)&dword_5d4594_1046496, 16403, v2, 0);
	sub_57A9F0(v0, "user.rul");
	sub_57A1E0((int*)v0, "user.rul", 0, 5, *((uint16_t*)v0 + 26));
	sub_459880((int)v0);
	return sub_459D50(1);
}

//----- (004597E0) --------------------------------------------------------
int* nox_xxx_guiServerOptionsHide_4597E0(int a1) {
	nox_window* v1; // eax
	int* result; // eax

	if (dword_5d4594_1046492) {
		v1 = nox_xxx_wndGetFocus_46B4F0();
		if (nox_window_is_child(dword_5d4594_1046492, v1)) {
			nox_xxx_windowFocus_46B500(0);
		}
		nox_xxx_windowDestroyMB_46C4E0(dword_5d4594_1046492);
		dword_5d4594_1046492 = 0;
		sub_456D60(0);
		sub_4BE610();
		dword_5d4594_1046532 = 0;
		sub_4557D0(0);
		dword_5d4594_1046528 = 0;
		sub_4AD820();
		dword_5d4594_1046536 = 0;
	}
	result = (int*)a1;
	if (a1) {
		result = sub_57ADF0((int*)&nox_gui_server_options_rules);
		dword_587000_129656 = 1;
	}
	return result;
}

//----- (00459870) --------------------------------------------------------
char* sub_459870() { return (char*)&nox_gui_server_options_rules; }

//----- (00459A40) --------------------------------------------------------
int sub_459A40(char* a1) {
	char v2[100];    // [esp+0h] [ebp-12Ch]
	wchar2_t v3[100]; // [esp+64h] [ebp-C8h]

	strncpy(v2, a1, 0xFu);
	v2[15] = 0;
	nox_swprintf(v3, L"%S", v2);
	return nox_window_call_field_94(dword_5d4594_1046512, 16414, (uintptr_t)v3, 0);
}

//----- (00459AA0) --------------------------------------------------------
char* sub_459AA0(void* a1p) {
	uint8_t* a1 = a1p;
	uintptr_t v1;      // eax
	const wchar2_t* v2; // eax
	const wchar2_t* v3; // eax
	intptr_t result;   // eax
	uintptr_t v5;      // eax
	const char* v6;    // edi
	char v7[100];      // [esp+Ch] [ebp-64h]

	v1 = nox_window_call_field_94(dword_5d4594_1046512, 16413, 0, 0);
	nox_sprintf(v7, "%S", v1);
	strncpy((char*)a1 + 9, v7, 0xFu);
	*(uint16_t*)(a1 + 52) = sub_459C10();
	memcpy(a1 + 24, sub_453F90(), 0x14u);
	*(uint32_t*)(a1 + 44) = *(uint32_t*)sub_453600();
	*(uint32_t*)(a1 + 48) = sub_453610();
	v2 = (const wchar2_t*)nox_window_call_field_94(dword_5d4594_1046516, 16413, 0, 0);
	if (*v2) {
		*(uint16_t*)(a1 + 54) = nox_wcstol(v2, 0, 10);
	}
	v3 = (const wchar2_t*)nox_window_call_field_94(dword_5d4594_1046520, 16413, 0, 0);
	if (*v3) {
		*(uint8_t*)(a1 + 56) = nox_wcstol(v3, 0, 10);
	}
	*(uint8_t*)(a1 + 57) =
		(nox_xxx_wndGetChildByID_46B0C0(dword_5d4594_1046492, 10122)->draw_data.field_0 >> 2) & 1;
	result = (intptr_t)nox_window_call_field_94(dword_5d4594_1046496, 16404, 0, 0);
	if (result >= 0 && (v5 = nox_window_call_field_94(dword_5d4594_1046496, 16406, (uintptr_t)result, 0),
						 nox_sprintf(v7, "%S", v5), (result = (intptr_t)strtok(v7, "\t")) != 0)) {
		v6 = (const char*)result;
		result = 0;
		strcpy((char*)a1, v6);
	} else {
		*(uint8_t*)a1 = 0;
	}
	return (char*)result;
}

//----- (00459C30) --------------------------------------------------------
int sub_459C30() {
	char* v0;     // esi
	int result;   // eax
	uint32_t* v2; // edi
	int v3;       // eax

	v0 = nox_xxx_cliGamedataGet_416590(1);
	sub_453F70(v0 + 24);
	sub_4535E0((int*)v0 + 11);
	sub_4535F0(*((uint32_t*)v0 + 12));
	result = dword_5d4594_1046492;
	if (dword_5d4594_1046492) {
		sub_459880((int)v0);
		sub_4BDF70((int*)v0);
		nox_client_guiserv_updateMapList_458230(*((unsigned short*)v0 + 26), v0, 0);
		sub_457460((int)v0);
		if (0) {
			sub_457FE0();
		}
		v2 = nox_xxx_wndGetChildByID_46B0C0(*(uint32_t**)&dword_5d4594_1046492, 10119);
		v3 = nox_xxx_guiServerOptionsGetGametypeName_4573C0(*((uint16_t*)v0 + 26));
		result = nox_window_call_field_94((int)v2, 16385, v3, 0);
	}
	return result;
}

//----- (00459D50) --------------------------------------------------------
int sub_459D50(int a1) {
	int result; // eax

	result = a1;
	*getMemU32Ptr(0x5D4594, 1046544) = a1;
	return result;
}

//----- (00459D60) --------------------------------------------------------
int sub_459D60() { return *getMemU32Ptr(0x5D4594, 1046544); }

//----- (00459D80) --------------------------------------------------------
int sub_459D80(int a1) {
	int result = 0; // eax

	if (dword_5d4594_1046492) {
		result = nox_window_set_hidden(dword_5d4594_1046492, a1);
	}
	return result;
}

//----- (00459DA0) --------------------------------------------------------
int sub_459DA0() { return dword_5d4594_1046492 != 0; }

//----- (00459DB0) --------------------------------------------------------
int sub_459DB0(nox_drawable* dr) {
	return dr && (dr->flags28 & 0x400000u) != 0 && (dr->flags29 & 8u) != 0;
}

//----- (00459EC0) --------------------------------------------------------
nox_drawable* nox_xxx_cliNextMinimapObj_459EC0(nox_drawable* dr) {
	nox_drawable* next = dr ? dr->field_102 : NULL;
	if (dr && dr == next) {
		printf("nox_xxx_cliNextMinimapObj_459EC0: infinite loop!\n");
		abort();
		return NULL;
	}
	return next;
}

//----- (0045A010) --------------------------------------------------------
nox_drawable* sub_45A010(nox_drawable* dr) { return dr->field_104; }

//----- (0045A070) --------------------------------------------------------
nox_drawable* nox_drawable_next_45A070(nox_drawable* a1) {
	return a1 ? a1->field_92 : NULL;
}

//----- (0045A990) --------------------------------------------------------
nox_drawable* nox_xxx_spriteSetActiveMB_45A990_drawable(nox_drawable* dr) {
	dr->flags30 |= 4u;
	return dr;
}

//----- (0045A9B0) --------------------------------------------------------
void sub_45A9B0(nox_drawable* source, nox_drawable* listener) {
	if (!source || !listener) {
		return;
	}
	char* sound_def = nox_xxx_draw_452270(source->field_123);
	nox_draw_viewport_t* vp = nox_draw_getViewport_437250();
	if (!sound_def || !vp) {
		return;
	}

	int volume = 0;
	int pan = 0;
	if ((source->flags30 & 0x1000000u) != 0 && (source->flags70 & 0xCu) == 0) {
		const int dx = (int)(listener->pos.x - source->pos.x);
		const int dy = (int)(listener->pos.y - source->pos.y);
		const int max_distance = *(uint32_t*)(sound_def + 64);
		if (dx < max_distance && dy < max_distance && max_distance > 0) {
			const int distance = (int)sqrt((double)dx * dx + (double)dy * dy + 1.0);
			if (distance < max_distance) {
				volume = 100 * (max_distance - distance) / max_distance;
				if (volume < 0) {
					volume = 0;
				} else if (volume > 100) {
					volume = 100;
				}
				if (nox_win_width >= 2) {
					pan = (int)(50 * (source->pos.x - vp->field_6 - vp->x1) / (nox_win_width / 2));
				}
			}
		}
	}

#if UINTPTR_MAX == UINT32_MAX
	// The original audio-channel record embeds 32-bit pointers. Keep the
	// exact channel lifecycle on 32-bit builds; 64-bit builds use the native
	// OpenAL effects path and must never reinterpret this legacy record.
	int* handle = (int*)&source->field_124;
	uint32_t* channel = (uint32_t*)sub_452EB0(handle);
	if (volume != 0) {
		if (channel) {
			sub_452FE0((int)(intptr_t)channel, pan);
			sub_452F50((int)(intptr_t)channel, volume);
		} else {
			channel = nox_xxx_draw_452300((uint32_t*)sound_def);
			if (channel) {
				sub_452EE0((int)(intptr_t)channel, volume);
				sub_452F80((int)(intptr_t)channel, pan);
				sub_452E90(handle, (int)(intptr_t)channel);
			}
		}
	} else if (channel) {
		sub_4523D0(channel);
	}
#else
	(void)volume;
	(void)pan;
	// A stale legacy channel cannot be valid after widening Drawable. Native
	// 64-bit positional loop ownership will be attached here independently.
	source->field_124 = NULL;
	source->field_125 = 0;
	source->field_126 = 0;
#endif
}

//----- (0045AB80) --------------------------------------------------------
nox_drawable* nox_xxx_spriteSetFrameMB_45AB80_drawable(nox_drawable* dr, int frame) {
	if (!(dr->flags28 & 2) || !(dr->flags29 & 0x40000) || dr->field_69 != 8) {
		dr->field_78 = dr->field_77;
		dr->field_77 = frame;
	}
	return dr;
}

int nox_xxx_spriteSetFrameMB_45AB80(int a1, int a2) {
	return (int)(intptr_t)nox_xxx_spriteSetFrameMB_45AB80_drawable((nox_drawable*)(intptr_t)a1, a2);
}

//----- (0045ABC0) --------------------------------------------------------
int nox_xxx_guiSpellSortFn_45ABC0(const void* a1, const void* a2) {
	int v2;            // edi
	const wchar2_t* v3; // esi
	const wchar2_t* v4; // eax
	int result;        // eax
	int v6;            // [esp-4h] [ebp-Ch]
	nox_playerInfo* player = (nox_playerInfo*)(uintptr_t)dword_5d4594_1047516;

	v2 = *(uint32_t*)a2;
	if (dword_5d4594_1046868 == 1) {
		v3 = (const wchar2_t*)nox_xxx_guiCreatureGetName_427240(*(uint32_t*)a1);
		v4 = (const wchar2_t*)nox_xxx_guiCreatureGetName_427240(v2);
	} else {
		v6 = *(uint32_t*)a1;
		if (player->info.playerClass) {
			v3 = (const wchar2_t*)nox_xxx_spellTitle_424930(v6);
			v4 = (const wchar2_t*)nox_xxx_spellTitle_424930(v2);
		} else {
			v3 = (const wchar2_t*)nox_xxx_abilityGetName_0_425260(v6);
			v4 = (const wchar2_t*)nox_xxx_abilityGetName_0_425260(v2);
		}
	}
	if (v3 && v4) {
		result = _nox_wcsicmp(v3, v4);
	} else {
		result = 0;
	}
	return result;
}

//----- (0045AC40) --------------------------------------------------------
int nox_xxx_bookSetColor_45AC40() {
	int result; // eax

	*getMemU32Ptr(0x5D4594, 1046880) = nox_color_rgb_4344A0(15, 15, 15);
	result = nox_color_rgb_4344A0(115, 100, 100);
	*getMemU32Ptr(0x5D4594, 1046884) = result;
	return result;
}

//----- (0045AC70) --------------------------------------------------------
void nox_client_toggleSpellbook_45AC70() {
	if (wndIsShown_nox_xxx_wndIsShown_46ACC0(nox_win_unk1)) {
		nox_xxx_bookShowMB_45AD70(0);
	} else {
		nox_xxx_bookHideMB_45ACA0(0);
	}
}

//----- (0045ACA0) --------------------------------------------------------
int nox_xxx_bookHideMB_45ACA0(int a1) {
	nox_window* v1; // eax
	nox_window* v2; // ecx

	if (wndIsShown_nox_xxx_wndIsShown_46ACC0(nox_win_unk1) || dword_5d4594_1047520 == 1) {
		return 0;
	}
	dword_5d4594_1046864 = 0;
	set_dword_5d4594_3799468(1);
	dword_5d4594_1046868 = dword_5d4594_1046872 != 0;
	v1 = nox_xxx_wndGetCaptureMain_46AE00();
	v2 = nox_win_unk1;
	if (v1 == nox_win_unk1) {
		nox_xxx_wndClearCaptureMain_46ADE0(nox_win_unk1);
		v2 = nox_win_unk1;
	}
	nox_window_set_hidden(v2, 1);
	nox_xxx_clientPlaySoundSpecial_452D80(787, 100);
	if (a1) {
		nox_xxx_aNox_cfg_0_587000_132132 = 1;
		dword_5d4594_1046936 = 0;
		nox_window_set_hidden(dword_5d4594_1046952, 1);
	}
	if (nox_xxx_bookGetSpellDnDType_477670() == 1) {
		nox_xxx_bookSpellDnDclear_477660();
	}
	return 1;
}

//----- (0045ADF0) --------------------------------------------------------
int nox_xxx_guiSpellSortList_45ADF0(int a1) {
	int v1; // edi
	int i;  // esi
	int v3; // eax
	int k;  // esi
	int v5; // eax
	int j;  // esi
	int v7; // eax
	int v9; // [esp+Ch] [ebp-4h]
	nox_playerInfo* player = (nox_playerInfo*)(uintptr_t)dword_5d4594_1047516;

	v1 = 0;
	dword_5d4594_1046656 = nox_xxx_guiFontHeightMB_43F320(0) + 2;
	*getMemU32Ptr(0x5D4594, 1047508) = 0;
	dword_5d4594_1047512 = 0;
	v9 = 2 * (141 / dword_5d4594_1046656) - 2;
	if (dword_5d4594_1046868 == 1) {
		for (i = nox_xxx_bookGetFirstCreMB_427300(); i; i = nox_xxx_bookGetNextCre_427320(i)) {
			if (nox_common_gameFlags_check_40A5C0(0x2000) && !nox_common_gameFlags_check_40A5C0(4096) ||
				player->beast_scroll_lvl[i]) {
				if (nox_xxx_bookCreatureTest_4D70C0(i)) {
					v3 = *getMemU32Ptr(0x5D4594, 1047508);
					v1 = 1;
					*getMemU32Ptr(0x5D4594, 1046960 + 4 * *getMemU32Ptr(0x5D4594, 1047508)) = i;
					*getMemU32Ptr(0x5D4594, 1047508) = v3 + 1;
				}
			}
		}
	} else if (a1) {
		for (j = nox_xxx_spellFirstValid_424AD0(); j; j = nox_xxx_spellNextValid_424AF0(j)) {
			if (j != 34 && !nox_xxx_playerCheckSpellClass_57AEA0(a1, j) &&
				(nox_common_gameFlags_check_40A5C0(0x2000) && !nox_common_gameFlags_check_40A5C0(4096) ||
				 player->spell_lvl[j])) {
				if (nox_xxx_spellHasFlags_424A50(j, 0x15000)) {
					++dword_5d4594_1047512;
				}
				if (!nox_xxx_spellHasFlags_424A50(j, 0x2000)) {
					v7 = *getMemU32Ptr(0x5D4594, 1047508);
					v1 = 1;
					*getMemU32Ptr(0x5D4594, 1046960 + 4 * *getMemU32Ptr(0x5D4594, 1047508)) = j;
					*getMemU32Ptr(0x5D4594, 1047508) = v7 + 1;
				}
			}
		}
	} else {
		for (k = nox_xxx_bookFirstKnownAbil_425330(); k; k = nox_xxx_bookNextKnownAbil_425350(k)) {
			if (nox_common_gameFlags_check_40A5C0(0x2000) && !nox_common_gameFlags_check_40A5C0(4096) ||
				player->spell_lvl[k]) {
				v5 = *getMemU32Ptr(0x5D4594, 1047508);
				v1 = 1;
				*getMemU32Ptr(0x5D4594, 1046960 + 4 * *getMemU32Ptr(0x5D4594, 1047508)) = k;
				*getMemU32Ptr(0x5D4594, 1047508) = v5 + 1;
			}
		}
	}
	*getMemU32Ptr(0x5D4594, 1046940) = (*getMemU32Ptr(0x5D4594, 1047508) - dword_5d4594_1047512) / v9 + 1;
	qsort(getMemAt(0x5D4594, 1046960), *getMemU32Ptr(0x5D4594, 1047508) - dword_5d4594_1047512, 4u,
		  nox_xxx_guiSpellSortFn_45ABC0);
	return v1;
}

//----- (0045B010) --------------------------------------------------------
void nox_xxx_book_45B010(int a1) {
	unsigned int result; // eax

	result = nox_gui_xxx_check_446360();
	if (result) {
		return;
	}
	result = sub_4AE3D0();
	if (result) {
		return;
	}
	if (a1 || (nox_xxx_get_57AF20() == 0)) {
		dword_5d4594_1046864 = 1;
		dword_5d4594_1046868 = dword_5d4594_1046872 != 0;
		nox_xxx_wndShowModalMB_46A8C0(nox_win_unk1);
		nox_xxx_clientPlaySoundSpecial_452D80(786, 100);
	}
}

//----- (0045B070) --------------------------------------------------------
int nox_xxx_bookWndProc_45B070(int a1, int a2) {
	if (dword_5d4594_1047520 == 1) {
		return 1;
	}
	if (a2 == 10) {
		nox_client_toggleSpellbook_45AC70();
		return 1;
	}
	if (*getMemU32Ptr(0x852978, 8) && !sub_478030() && !sub_47A260() &&
		(*(uint8_t*)(*getMemU32Ptr(0x852978, 8) + 120) & 2) == 2) {
		return 1;
	}
	if (a2 != 5) {
		return 0;
	}
	nox_window_set_hidden(dword_5d4594_1046944, 0);
	if (nox_xxx_aNox_cfg_0_587000_132132) {
		if (dword_5d4594_1046936 + 1 < *getMemIntPtr(0x5D4594, 1046940)) {
			++dword_5d4594_1046936;
		} else {
			dword_5d4594_1046932 = 0;
			nox_xxx_aNox_cfg_0_587000_132132 = 0;
			nox_window_set_hidden(dword_5d4594_1046952, 0);
		}
		if (dword_5d4594_1046872) {
			nox_book_anim_set_end(dword_5d4594_1046924, nox_xxx_bookClickCreature_45B200);
		} else {
			nox_book_anim_set_end(dword_5d4594_1046924, nox_xxx_bookClickSpell_45B1F0);
		}
		dword_5d4594_1046868 = 2;
		nox_book_anim_start(dword_5d4594_1046924);
		nox_xxx_clientPlaySoundSpecial_452D80(788, 100);
		return 1;
	}
	if (dword_5d4594_1046932 < *getMemIntPtr(0x5D4594, 1047508) - dword_5d4594_1047512 - 1) {
		++dword_5d4594_1046932;
		if (dword_5d4594_1046872) {
			nox_book_anim_set_end(dword_5d4594_1046924, nox_xxx_bookClickCreature_45B200);
		} else {
			nox_book_anim_set_end(dword_5d4594_1046924, nox_xxx_bookClickSpell_45B1F0);
		}
		dword_5d4594_1046868 = 2;
		nox_book_anim_start(dword_5d4594_1046924);
		nox_xxx_clientPlaySoundSpecial_452D80(788, 100);
		return 1;
	}
	return 1;
}

//----- (0045B1F0) --------------------------------------------------------
int nox_xxx_bookClickSpell_45B1F0() {
	int result; // eax

	result = 0;
	dword_5d4594_1046868 = 0;
	dword_5d4594_1046872 = 0;
	return result;
}

//----- (0045B200) --------------------------------------------------------
int nox_xxx_bookClickCreature_45B200() {
	int result; // eax

	result = 1;
	dword_5d4594_1046868 = 1;
	dword_5d4594_1046872 = 1;
	return result;
}

//----- (0045B210) --------------------------------------------------------
int nox_xxx_book_45B210(int a1, int a2) {
	if (dword_5d4594_1047520 == 1) {
		return 1;
	}
	if (a2 == 10) {
		nox_client_toggleSpellbook_45AC70();
		return 1;
	}
	if (*getMemU32Ptr(0x852978, 8) && !sub_478030() && !sub_47A260() &&
		(*(uint8_t*)(*getMemU32Ptr(0x852978, 8) + 120) & 2) == 2) {
		return 1;
	}
	if (a2 != 5) {
		return 0;
	}
	nox_window_set_hidden(dword_5d4594_1046948, 0);
	if (!nox_xxx_aNox_cfg_0_587000_132132) {
		if (*(int*)&dword_5d4594_1046932 < *getMemIntPtr(0x5D4594, 1047508) - *(int*)&dword_5d4594_1047512) {
			if (*(int*)&dword_5d4594_1046932 <= 0) {
				nox_xxx_aNox_cfg_0_587000_132132 = 1;
				dword_5d4594_1046936 = *getMemU32Ptr(0x5D4594, 1046940) - 1;
				nox_window_set_hidden(dword_5d4594_1046952, 1);
			} else {
				--dword_5d4594_1046932;
			}
		} else {
			nox_xxx_aNox_cfg_0_587000_132132 = 1;
			dword_5d4594_1046936 = 0;
			nox_window_set_hidden(dword_5d4594_1046952, 1);
		}
	} else {
		if (!dword_5d4594_1046936) {
			return 1;
		}
		--dword_5d4594_1046936;
	}
	if (dword_5d4594_1046872) {
		nox_book_anim_set_end(dword_5d4594_1046928, nox_xxx_bookClickCreature_45B200);
	} else {
		nox_book_anim_set_end(dword_5d4594_1046928, nox_xxx_bookClickSpell_45B1F0);
	}
	dword_5d4594_1046868 = 3;
	nox_book_anim_start(dword_5d4594_1046928);
	nox_xxx_clientPlaySoundSpecial_452D80(788, 100);
	return 1;
}

//----- (0045B360) --------------------------------------------------------
int nox_xxx_bookChildWndProcMB_45B360(uint32_t* a1, unsigned int a2) {
	int v2; // esi
	nox_playerInfo* player = (nox_playerInfo*)dword_5d4594_1047516;

	v2 = 0;
	if (dword_5d4594_1047520 == 1 || *getMemU32Ptr(0x852978, 8) && !sub_478030() && !sub_47A260() &&
										 (*(uint8_t*)(*getMemU32Ptr(0x852978, 8) + 120) & 2) == 2) {
		return 1;
	}
	if (a2 != 5) {
		if (a2 <= 5 || a2 > 7) {
			return 0;
		}
		return 1;
	}
	if (*a1 == 1310) {
		if (dword_5d4594_1046872) {
			dword_5d4594_1046868 = 0;
			dword_5d4594_1046872 = 0;
			if (!nox_xxx_guiSpellSortList_45ADF0(player->info.playerClass)) {
				dword_5d4594_1046868 = 1;
				dword_5d4594_1046872 = 1;
				nox_xxx_guiSpellSortList_45ADF0(player->info.playerClass);
				nox_xxx_clientPlaySoundSpecial_452D80(925, 100);
				return 1;
			}
			v2 = 1;
		}
		if (!nox_xxx_aNox_cfg_0_587000_132132 || dword_5d4594_1046936 || v2 == 1) {
			nox_xxx_aNox_cfg_0_587000_132132 = 1;
			dword_5d4594_1046936 = 0;
			nox_xxx_clientPlaySoundSpecial_452D80(788, 100);
			dword_5d4594_1046868 = 3;
			nox_book_anim_start(dword_5d4594_1046928);
			nox_book_anim_set_end(dword_5d4594_1046928, nox_xxx_bookClickSpell_45B1F0);
		nox_window_set_hidden(dword_5d4594_1046952, 1);
		nox_window_set_hidden(dword_5d4594_1046948, 0);
			nox_xxx_wndClearCaptureMain_46ADE0(nox_win_unk1);
			nox_xxx_bookSpellDnDclear_477660();
		}
		return 1;
	}
	if (*a1 != 1320) {
		return 1;
	}
	if (dword_5d4594_1046872 != 1) {
		dword_5d4594_1046868 = 1;
		dword_5d4594_1046872 = 1;
		if (!nox_xxx_guiSpellSortList_45ADF0(player->info.playerClass)) {
			dword_5d4594_1046868 = 0;
			dword_5d4594_1046872 = 0;
			nox_xxx_guiSpellSortList_45ADF0(player->info.playerClass);
			nox_xxx_clientPlaySoundSpecial_452D80(925, 100);
			return 1;
		}
		v2 = 1;
	}
	if (nox_xxx_aNox_cfg_0_587000_132132 && !dword_5d4594_1046936 && v2 != 1) {
		return 1;
	}
	nox_xxx_aNox_cfg_0_587000_132132 = 1;
	dword_5d4594_1046936 = 0;
	nox_xxx_clientPlaySoundSpecial_452D80(788, 100);
	if (v2 == 1) {
		dword_5d4594_1046868 = 2;
		nox_book_anim_start(dword_5d4594_1046924);
		nox_book_anim_set_end(dword_5d4594_1046924, nox_xxx_bookClickCreature_45B200);
	} else {
		dword_5d4594_1046868 = 3;
		nox_book_anim_start(dword_5d4594_1046928);
		nox_book_anim_set_end(dword_5d4594_1046928, nox_xxx_bookClickCreature_45B200);
	}
	nox_window_set_hidden(dword_5d4594_1046952, 1);
	nox_window_set_hidden(dword_5d4594_1046948, 0);
	nox_xxx_wndClearCaptureMain_46ADE0(nox_win_unk1);
	nox_xxx_bookSpellDnDclear_477660();
	return 1;
}

//----- (0045B5F0) --------------------------------------------------------
int nox_xxx_bookListWndProc_45B5F0(nox_window* a1, unsigned int a2, unsigned int a3) {
	int v3;        // ebp
	int v4;        // edi
	int v5;        // esi
	int result;    // eax
	bool v7;       // sf
	signed int v8; // ebp
	int v9;        // eax
	int v10;       // ecx
	char v11;      // al
	int v12;       // eax
	int v13;       // eax
	int v14;       // [esp+10h] [ebp-4h]
	nox_playerInfo* player = (nox_playerInfo*)dword_5d4594_1047516;

	v3 = a3 >> 16;
	v4 = (unsigned short)a3;
	if (dword_5d4594_1047520 == 1 || *getMemU32Ptr(0x852978, 8) && !sub_478030() && !sub_47A260() &&
										 (*(uint8_t*)(*getMemU32Ptr(0x852978, 8) + 120) & 2) == 2) {
		return 1;
	}
	if (!nox_xxx_aNox_cfg_0_587000_132132) {
		if (a2 >= 5) {
			if (a2 <= 8) {
				return 1;
			}
			if (a2 == 11) {
				nox_client_toggleSpellbook_45AC70();
				return 1;
			}
		}
		return 0;
	}
	dword_5d4594_1046656 = nox_xxx_guiFontHeightMB_43F320(0) + 2;
	v5 = 141 / dword_5d4594_1046656 - 1;
	nox_gui_getWindowOffs_46AA20(a1, &v14, &a3);
	switch (a2) {
	case 5u:
		if (*getMemU32Ptr(0x852978, 8) && (*(uint8_t*)(*getMemU32Ptr(0x852978, 8) + 120) & 2) == 2) {
			return 1;
		}
		dword_5d4594_1047536 = v3;
		v7 = (int)(-19 - a3 + v3) < 0;
		v8 = -19 - a3 + v3;
		dword_5d4594_1047532 = v4;
		if (v7) {
			nox_xxx_aNox_cfg_0_587000_132136 = -1;
			return 1;
		}
		v9 = v8 / *(int*)&dword_5d4594_1046656;
		if (v8 / *(int*)&dword_5d4594_1046656 >= v5) {
			nox_xxx_aNox_cfg_0_587000_132136 = -1;
			return 1;
		}
		if (v4 - v14 > 145) {
			v9 += v5;
		}
		nox_xxx_aNox_cfg_0_587000_132136 = 2 * v5 * dword_5d4594_1046936 + v9;
		nox_xxx_wndSetCaptureMain_46ADC0(a1);
		v10 = nox_xxx_aNox_cfg_0_587000_132136;
		if (nox_xxx_aNox_cfg_0_587000_132136 >= *getMemIntPtr(0x5D4594, 1047508) - dword_5d4594_1047512) {
			nox_xxx_aNox_cfg_0_587000_132136 = -1;
			return 1;
		}
		v11 = player->info.playerClass;
		if (!v11 && !dword_5d4594_1046868) {
			dword_5d4594_1047528 = *getMemU32Ptr(0x5D4594, 1046960 + 4 * nox_xxx_aNox_cfg_0_587000_132136);
			nox_xxx_bookSaveSpellForDragDrop_477640(*(int*)&dword_5d4594_1047528, 1);
			nox_xxx_clientPlaySoundSpecial_452D80(793, 100);
			return 1;
		}
		if (v11 != 2 || dword_5d4594_1046868 != 1) {
			if (dword_5d4594_1046868) {
				return 1;
			}
			dword_5d4594_1047528 = *getMemU32Ptr(0x5D4594, 1046960 + 4 * nox_xxx_aNox_cfg_0_587000_132136);
			nox_xxx_bookSaveSpellForDragDrop_477640(*(int*)&dword_5d4594_1047528, 1);
			nox_xxx_clientPlaySoundSpecial_452D80(793, 100);
			result = 1;
		} else {
			if (!player->spell_lvl[134]) {
				if (!nox_common_gameFlags_check_40A5C0(0x2000) || nox_common_gameFlags_check_40A5C0(4096)) {
					nox_xxx_wndClearCaptureMain_46ADE0(a1);
					nox_xxx_bookMoveToPage_45B930(*(int*)&nox_xxx_aNox_cfg_0_587000_132136);
					dword_5d4594_1047528 = 0;
					nox_xxx_aNox_cfg_0_587000_132136 = -1;
					return 1;
				}
				v10 = nox_xxx_aNox_cfg_0_587000_132136;
			}
			dword_5d4594_1047528 = *getMemU32Ptr(0x5D4594, 1046960 + 4 * v10) + 74;
			nox_xxx_bookSaveSpellForDragDrop_477640(*(int*)&dword_5d4594_1047528, 1);
			nox_xxx_clientPlaySoundSpecial_452D80(793, 100);
			result = 1;
		}
		break;
	case 6u:
	case 7u:
		if ((int)nox_xxx_aNox_cfg_0_587000_132136 >= 0) {
			v12 = dword_5d4594_1047532 - v4;
			if (*(int*)&dword_5d4594_1047532 - v4 < 0) {
				v12 = v4 - dword_5d4594_1047532;
			}
			if (v12 >= 5) {
				nox_xxx_bookSpellDrop_45DCA0(*(int*)&dword_5d4594_1047528, 0, v4, v3, 0);
			} else {
				v13 = dword_5d4594_1047536 - v3;
				if (*(int*)&dword_5d4594_1047536 - v3 < 0) {
					v13 = v3 - dword_5d4594_1047536;
				}
				if (v13 >= 5) {
					nox_xxx_bookSpellDrop_45DCA0(*(int*)&dword_5d4594_1047528, 0, v4, v3, 0);
				} else {
					nox_xxx_bookMoveToPage_45B930(*(int*)&nox_xxx_aNox_cfg_0_587000_132136);
				}
			}
		}
		if (nox_xxx_wndGetCaptureMain_46AE00()) {
			nox_xxx_wndClearCaptureMain_46ADE0(a1);
		}
		nox_xxx_bookSpellDnDclear_477660();
		return 1;
	case 8u:
		return 1;
	case 0xBu:
		nox_client_toggleSpellbook_45AC70();
		return 1;
	default:
		return 0;
	}
	return result;
}

//----- (0045B930) --------------------------------------------------------
int nox_xxx_bookMoveToPage_45B930(int a1) {
	nox_xxx_aNox_cfg_0_587000_132132 = 0;
	dword_5d4594_1046932 = a1;
	dword_5d4594_1046936 = 99;
	if (dword_5d4594_1046872) {
		nox_book_anim_set_end(dword_5d4594_1046924, nox_xxx_bookClickCreature_45B200);
	} else {
		nox_book_anim_set_end(dword_5d4594_1046924, nox_xxx_bookClickSpell_45B1F0);
	}
	dword_5d4594_1046868 = 2;
	nox_book_anim_start(dword_5d4594_1046924);
	nox_xxx_clientPlaySoundSpecial_452D80(788, 100);
	nox_window_set_hidden(dword_5d4594_1046952, 0);
	nox_window_set_hidden(dword_5d4594_1046944, 0);
	return nox_window_set_hidden(dword_5d4594_1046948, 0);
}

int nox_xxx_book_45BD30(int a1, int a2) { return 1; }

//----- (0045B9D0) --------------------------------------------------------
int nox_xxx_bookInit_45B9D0() {
	nox_window* v1;
	nox_window* v2;
	nox_video_bag_image_t* v3;
	nox_video_bag_image_t* v4;
	nox_video_bag_image_t* image;

	dword_5d4594_1047516 = dword_8531A0_2576;
	image = nox_xxx_gLoadImg_42F970("ArrowNW");
	nox_book_arrow_images[0] = image;
	if (!image) {
		return 0;
	}

	image = nox_xxx_gLoadImg_42F970("ArrowN");
	nox_book_arrow_images[1] = image;
	if (!image) {
		return 0;
	}

	image = nox_xxx_gLoadImg_42F970("ArrowNE");
	nox_book_arrow_images[2] = image;
	if (!image) {
		return 0;
	}

	image = nox_xxx_gLoadImg_42F970("ArrowW");
	nox_book_arrow_images[3] = image;
	if (!image) {
		return 0;
	}

	image = nox_xxx_gLoadImg_42F970("ArrowE");
	nox_book_arrow_images[4] = image;
	if (!image) {
		return 0;
	}

	image = nox_xxx_gLoadImg_42F970("ArrowSW");
	nox_book_arrow_images[5] = image;
	if (!image) {
		return 0;
	}

	image = nox_xxx_gLoadImg_42F970("ArrowS");
	nox_book_arrow_images[6] = image;
	if (!image) {
		return 0;
	}

	image = nox_xxx_gLoadImg_42F970("ArrowSE");
	nox_book_arrow_images[7] = image;
	if (!image) {
		return 0;
	}

	nox_book_background_image = nox_xxx_gLoadImg_42F970("BookOfKnowledge");
	if (!nox_book_background_image) {
		return 0;
	}

	nox_book_guide_tab_lit_image = nox_xxx_gLoadImg_42F970("GuideTabLit");
	if (!nox_book_guide_tab_lit_image) {
		return 0;
	}

	nox_book_spell_tab_lit_image = nox_xxx_gLoadImg_42F970("SpellTabLit");
	if (!nox_book_spell_tab_lit_image) {
		return 0;
	}

	dword_5d4594_1046924 = nox_xxx_gLoadAnim_42FA20("BookPageForward");
	if (!dword_5d4594_1046924) {
		return 0;
	}

	nox_book_anim_set_end(dword_5d4594_1046924, nox_xxx_bookClickSpell_45B1F0);
	dword_5d4594_1046928 = nox_xxx_gLoadAnim_42FA20("BookPageBackward");
	if (!dword_5d4594_1046928) {
		return 0;
	}

	nox_book_anim_set_end(dword_5d4594_1046928, nox_xxx_bookClickSpell_45B1F0);
	nox_win_unk1 = nox_window_new(0, 1196, 5, nox_win_height - 323, 285, 168, 0);
	nox_window_set_all_funcs(nox_win_unk1, nox_xxx_bookListWndProc_45B5F0, nox_xxx_bookDrawList_45BD40, 0);
	nox_window_set_hidden(nox_win_unk1, 1);
	v1 = nox_window_new(nox_win_unk1, 8, 257, 15, 27, 40, 0);
	if (!v1) {
		return 0;
	}

	nox_window_set_all_funcs(v1, nox_xxx_bookChildWndProcMB_45B360, nox_xxx_book_45BD30,
								 nox_xxx_book_45CF00);
	v1->id = 1320;
	v2 = nox_window_new(nox_win_unk1, 8, 253, 61, 27, 40, 0);
	if (!v2) {
		return 0;
	}

	nox_window_set_all_funcs(v2, nox_xxx_bookChildWndProcMB_45B360, nox_xxx_book_45BD30,
								 nox_xxx_book_45CF00);
	v2->id = 1310;
	dword_5d4594_1046944 = nox_window_new(nox_win_unk1, 136, 24, 138, 20, 20, 0);
	nox_window_set_all_funcs(dword_5d4594_1046944, nox_xxx_book_45B210, 0, 0);
	v3 = nox_xxx_gLoadImg_42F970("ArrowW");
	nox_xxx_wndSetIcon_46AE60(dword_5d4594_1046944, v3);
	dword_5d4594_1046948 = nox_window_new(nox_win_unk1, 136, 233, 138, 20, 20, 0);
	nox_window_set_all_funcs(dword_5d4594_1046948, nox_xxx_bookWndProc_45B070, 0, 0);
	v4 = nox_xxx_gLoadImg_42F970("ArrowE");
	nox_xxx_wndSetIcon_46AE60(dword_5d4594_1046948, v4);
	dword_5d4594_1046952 = nox_window_new(nox_win_unk1, 8, 63, 19, 30, 30, 0);
	nox_window_set_all_funcs(dword_5d4594_1046952, nox_xxx_bookWndFn_45CC10,
								 nox_xxx_bookDrawIconFn_45CB30, 0);
	nox_window_set_hidden(dword_5d4594_1046952, 1);
	dword_5d4594_1046956 = nox_window_new(0, 40, 0, 0, 30, 30, 0);
	nox_window_set_all_funcs(dword_5d4594_1046956, 0, nox_xxx_bookDrawFn_45C7D0, 0);
	nox_window_set_hidden(dword_5d4594_1046956, 1);
	dword_5d4594_1046868 = 0;
	dword_5d4594_1046872 = 0;
	nox_xxx_aNox_cfg_0_587000_132132 = 1;
	dword_5d4594_1046936 = 0;

	return 1;
}

//----- (0045CB30) --------------------------------------------------------
int nox_xxx_bookDrawIconFn_45CB30(uint32_t* a1) {
	nox_video_bag_image_t* v1; // eax
	nox_video_bag_image_t* v2; // esi
	int v4; // [esp+4h] [ebp-8h]
	int v5; // [esp+8h] [ebp-4h]
	nox_playerInfo* player = (nox_playerInfo*)dword_5d4594_1047516;

	if (dword_5d4594_1046868) {
		if (dword_5d4594_1046868 != 1 || player->info.playerClass != 2 ||
			!player->spell_lvl[134] &&
				(!nox_common_gameFlags_check_40A5C0(0x2000) || nox_common_gameFlags_check_40A5C0(4096))) {
			return 1;
		}
		v1 = nox_xxx_spellIcon_424A90(*getMemU32Ptr(0x5D4594, 1046960 + 4 * dword_5d4594_1046932) + 74);
	} else if (player->info.playerClass) {
		v1 = nox_xxx_spellIcon_424A90(*getMemU32Ptr(0x5D4594, 1046960 + 4 * dword_5d4594_1046932));
	} else {
		v1 = nox_xxx_spellGetAbilityIcon_425310(*getMemU32Ptr(0x5D4594, 1046960 + 4 * dword_5d4594_1046932), 0);
	}
	v2 = v1;
	if (v1) {
		nox_client_wndGetPosition_46AA60(a1, &v5, &v4);
		nox_client_drawImageAt_47D2C0(v2, v5, v4);
	}
	return 1;
}

//----- (0045CC10) --------------------------------------------------------
int nox_xxx_bookWndFn_45CC10(uint32_t* a1, int a2, unsigned int a3) {
	int result; // eax
	int v4;     // ebp
	char v5;    // al
	char v6;    // al
	nox_playerInfo* player = (nox_playerInfo*)dword_5d4594_1047516;

	if (*getMemU32Ptr(0x852978, 8) && (*(uint8_t*)(*getMemU32Ptr(0x852978, 8) + 120) & 2) == 2) {
		return 1;
	}
	v4 = a3 >> 16;
	if (dword_5d4594_1046868 == 1) {
		v5 = player->info.playerClass;
		if (v5 == 1 || !v5 ||
			!nox_common_gameFlags_check_40A5C0(0x2000) && !nox_common_gameFlags_check_40A5C0(4096) &&
				!player->spell_lvl[134]) {
			return 0;
		}
	}
	switch (a2) {
	case 5:
		if (nox_xxx_wndGetCaptureMain_46AE00() || dword_5d4594_1047540) {
			return 1;
		}
		v6 = player->info.playerClass;
		if (!v6 && !dword_5d4594_1046868) {
			dword_5d4594_1047540 = *getMemU32Ptr(0x5D4594, 1046960 + 4 * dword_5d4594_1046932);
			nox_xxx_bookSaveSpellForDragDrop_477640(*(int*)&dword_5d4594_1047540, 1);
			nox_xxx_wndSetCaptureMain_46ADC0((nox_window*)a1);
			nox_xxx_clientPlaySoundSpecial_452D80(793, 100);
			return 1;
		}
		if (v6 == 2 && dword_5d4594_1046868 == 1) {
			dword_5d4594_1047540 = *getMemU32Ptr(0x5D4594, 1046960 + 4 * dword_5d4594_1046932) + 74;
			nox_xxx_bookSaveSpellForDragDrop_477640(*(int*)&dword_5d4594_1047540, 1);
			nox_xxx_wndSetCaptureMain_46ADC0((nox_window*)a1);
			nox_xxx_clientPlaySoundSpecial_452D80(793, 100);
			return 1;
		}
		if (dword_5d4594_1046868) {
			return 1;
		}
		dword_5d4594_1047540 = *getMemU32Ptr(0x5D4594, 1046960 + 4 * dword_5d4594_1046932);
		if (!nox_xxx_spellHasFlags_424A50(*(int*)&dword_5d4594_1047540, 0x15000)) {
			nox_xxx_bookSaveSpellForDragDrop_477640(*(int*)&dword_5d4594_1047540, 1);
			nox_xxx_wndSetCaptureMain_46ADC0((nox_window*)a1);
			nox_xxx_clientPlaySoundSpecial_452D80(793, 100);
			return 1;
		}
		dword_5d4594_1047540 = 0;
		return 1;
	case 6:
	case 7:
		if (!dword_5d4594_1047540) {
			return 1;
		}
		if (!nox_xxx_wndPointInWnd_46AAB0(a1, (unsigned short)a3, v4)) {
			if (nox_xxx_guiSpell_460650()) {
				nox_xxx_guiSpellTargetClickSet_45D9D0(*(int*)&dword_5d4594_1047540);
			} else if (sub_4611A0()) {
				sub_4611B0();
			} else {
				nox_xxx_bookSpellDrop_45DCA0(*(int*)&dword_5d4594_1047540, 0, (unsigned short)a3, v4, 0);
			}
			dword_5d4594_1047540 = 0;
			nox_xxx_wndClearCaptureMain_46ADE0((nox_window*)a1);
			nox_xxx_bookSpellDnDclear_477660();
			return 1;
		}
		if (player->info.playerClass) {
			if (nox_xxx_guiSpell_460650()) {
				nox_xxx_guiSpellSetCursor_45DF60(0, 0);
				dword_5d4594_1047540 = 0;
				nox_xxx_wndClearCaptureMain_46ADE0((nox_window*)a1);
				nox_xxx_bookSpellDnDclear_477660();
				return 1;
			}
			if (!nox_xxx_spellHasFlags_424A50(*(int*)&dword_5d4594_1047540, 0x600)) {
				nox_xxx_guiSpellSetCursor_45DF60(*(int*)&dword_5d4594_1047540, 0);
				nox_xxx_bookSpellDnDclear_477660();
				return 1;
			}
			if (nox_xxx_guiSpellSetCursor_45DF60(*(int*)&dword_5d4594_1047540, 1)) {
				dword_5d4594_1047540 = 0;
				nox_xxx_wndClearCaptureMain_46ADE0((nox_window*)a1);
				nox_xxx_bookSpellDnDclear_477660();
				return 1;
			}
		} else {
			if (sub_4611A0()) {
				sub_45DFC0(0);
				dword_5d4594_1047540 = 0;
				nox_xxx_wndClearCaptureMain_46ADE0((nox_window*)a1);
				nox_xxx_bookSpellDnDclear_477660();
				return 1;
			}
			if (sub_45DFC0(*(int*)&dword_5d4594_1047540)) {
				dword_5d4594_1047540 = 0;
				nox_xxx_wndClearCaptureMain_46ADE0((nox_window*)a1);
				nox_xxx_bookSpellDnDclear_477660();
				return 1;
			}
		}
		nox_xxx_bookSpellDnDclear_477660();
		return 1;
	case 8:
		return 1;
	default:
		return 0;
	}
	return result;
}

//----- (0045CFC0) --------------------------------------------------------
int sub_45CFC0() { return ((unsigned char)~nox_xxx_wndGetFlags_46ADA0(nox_win_unk1) >> 4) & 1; }

//----- (0045CFE0) --------------------------------------------------------
void nox_xxx_netSpellRewardCli_45CFE0(int a1, int a2, int a3, int a4) {
	int v5;             // ebp
	int v6;             // esi
	uint32_t* v7;       // edi
	int v8;             // eax
	int v9;             // esi
	uint32_t* v10;      // eax
	int v11;            // [esp+14h] [ebp+4h]

	nox_playerInfo* player = (nox_playerInfo*)dword_8531A0_2576;
	if (!player) {
		return;
	}
	v5 = a1;
	if (nox_xxx_playerCheckSpellClass_57AEA0(player->info.playerClass, a1) == 9) {
		return;
	}
	player->spell_lvl[a1] = a2;
	if (nox_xxx_spellHasFlags_424A50(a1, 0x1000)) {
		v11 = 0x2000;
		goto LABEL_9;
	} else if (nox_xxx_spellHasFlags_424A50(a1, 0x4000)) {
		v11 = 0x8000;
		goto LABEL_9;
	} else if (nox_xxx_spellHasFlags_424A50(a1, 0x10000)) {
		v11 = 0x20000;
		goto LABEL_9;
	} else {
		goto LABEL_A;
	}
LABEL_9:
	v6 = 1;
	v7 = &player->spell_lvl[1];
	do {
		if (nox_xxx_spellHasFlags_424A50(v6, v11) && nox_xxx_spellIsValid_424B50(v6)) {
			*v7 = a2;
		}
		++v6;
		++v7;
	} while (v6 < 137);
LABEL_A:
	dword_5d4594_1046868 = 0;
	dword_5d4594_1046872 = 0;
	nox_xxx_guiSpellSortList_45ADF0(player->info.playerClass);
	if (v5 == 34) {
		v8 = 1;
		if (player->spell_lvl[34] != 1 || !a3) {
			v8 = 0;
		}
		nox_xxx_quickbarAddTrap_460EC0(v8);
	} else if (a3) {
		v9 = 0;
		v10 = getMemU32Ptr(0x5D4594, 1046960);
		do {
			if (*v10 == v5) {
				break;
			}
			++v10;
			++v9;
		} while (v10 < getMemU32Ptr(0x5D4594, 1047508));
		if (v9 != 137) {
			nox_xxx_bookHideMB_45ACA0(0);
			nox_xxx_bookMoveToPage_45B930(v9);
			nox_xxx_book_45B010(0);
			nox_xxx_bookRewardCli_499CF0((int*)2, v5, a4);
		}
	}
}

//----- (0045D140) --------------------------------------------------------
#if 0 // PE32-only: native-width client guide awards are implemented in client_guide_reward_45d140_export.go.
void nox_xxx_netGuideRewardCli_45D140(int a1, int a2) {
	int v2;            // edx
	unsigned char* v3; // esi
	uint32_t* v4;      // ecx
	int v5;            // eax
	int* v6;           // ecx
	int v7;            // eax
	int v8;            // esi
	unsigned char* v9; // eax

	v2 = dword_8531A0_2576;
	if (dword_8531A0_2576) {
		v3 = getMemAt(0x587000, 132124);
		*(uint32_t*)(dword_8531A0_2576 + 4 * a1 + 4244) = 1;
		if (*getMemU32Ptr(0x587000, 132124)) {
			do {
				v4 = *(uint32_t**)v3;
				if (a1 == **(uint32_t**)v3) {
					v5 = v4[1];
					v6 = v4 + 1;
					if (v5) {
						do {
							++v6;
							*(uint32_t*)(v2 + 4 * v5 + 4244) = 1;
							v5 = *v6;
						} while (*v6);
					}
				}
				v7 = *((uint32_t*)v3 + 1);
				v3 += 4;
			} while (v7);
		}
		dword_5d4594_1046868 = 1;
		dword_5d4594_1046872 = 1;
		nox_xxx_guiSpellSortList_45ADF0(*(unsigned char*)(v2 + 2251));
		if (a2) {
			v8 = 0;
			v9 = getMemAt(0x5D4594, 1046960);
			do {
				if (*(uint32_t*)v9 == a1) {
					break;
				}
				v9 += 4;
				++v8;
			} while ((int)v9 < (int)getMemAt(0x5D4594, 1047124));
			if (v8 != 41) {
				nox_xxx_bookHideMB_45ACA0(0);
				nox_xxx_bookMoveToPage_45B930(v8);
				nox_xxx_book_45B010(0);
				nox_xxx_bookRewardCli_499CF0((int*)4, a1, 0);
			}
		}
	}
}
#endif

void nox_xxx_bookGuideRewardCli_native_45D140(int guide) {
	nox_xxx_bookRewardCli_499CF0((int*)4, guide, 0);
}

//----- (0045D200) --------------------------------------------------------
int* nox_xxx_bookSetForward_45D200(int* a1, int a2, int2* a3) {
	int* result; // eax
	int v4;      // ecx
	int v5;      // ecx

	nox_xxx_bookShowMB_45AD70(1);
	nox_window_setPos_46A9B0(nox_win_unk1, a3->field_0, a3->field_4);
	result = a1;
	if (a1 == (int*)2 || a1 == (int*)4) {
		v5 = 0;
		result = getMemIntPtr(0x5D4594, 1046960);
		do {
			if (*result == a2) {
				break;
			}
			++result;
			++v5;
		} while (result < getMemIntPtr(0x5D4594, 1047508));
		if (v5 != 137) {
			result = (int*)nox_xxx_bookMoveToPage_45B930(v5);
		}
	} else if (a1 == (int*)3) {
		v4 = 0;
		result = getMemIntPtr(0x5D4594, 1046960);
		do {
			if (*result == a2) {
				break;
			}
			++result;
			++v4;
		} while (result < getMemIntPtr(0x5D4594, 1046984));
		if (v4 != 6) {
			result = (int*)nox_xxx_bookMoveToPage_45B930(v4);
		}
	}
	return result;
}

//----- (0045D290) --------------------------------------------------------
void nox_xxx_abilityReward_45D290(int a1, char* a2, int a3) {
	nox_playerInfo* player; // eax
	int v4;                // esi
	int32_t* entry;         // eax

	player = nox_common_playerInfoGetByID_417040(nox_player_netCode_85319C);
	if (player) {
		dword_5d4594_1046868 = 0;
		dword_5d4594_1046872 = 0;
		nox_xxx_guiSpellSortList_45ADF0(player->info.playerClass);
		if (a2) {
			v4 = 0;
			entry = getMemI32Ptr(0x5D4594, 1046960);
			do {
				if (*entry == a1) {
					break;
				}
				++entry;
				++v4;
			} while (entry < getMemI32Ptr(0x5D4594, 1046984));
			if (v4 != 6) {
				nox_xxx_bookHideMB_45ACA0(0);
				nox_xxx_bookMoveToPage_45B930(v4);
				nox_xxx_book_45B010(0);
				if (a3) {
					nox_xxx_bookRewardCli_499CF0((int*)3, a1, a3);
				}
			}
		}
	}
}

//----- (0045D320) --------------------------------------------------------
int sub_45D320(int a1) {
	nox_playerInfo* player; // ebx
	int result;            // eax
	int v3;                // ebp
	int v4;                // esi
	uint32_t* v5;          // edi

	player = (nox_playerInfo*)dword_8531A0_2576;
	result = nox_xxx_bookHideMB_45ACA0(1);
	if (player) {
		player->spell_lvl[a1] = 0;
		sub_461360(a1);
		if (nox_xxx_spellHasFlags_424A50(a1, 0x1000)) {
			v3 = 0x2000;
		} else if (nox_xxx_spellHasFlags_424A50(a1, 0x4000)) {
			v3 = 0x8000;
		} else {
			if (!nox_xxx_spellHasFlags_424A50(a1, 0x10000)) {
				return nox_xxx_guiSpellSortList_45ADF0(player->info.playerClass);
			}
			v3 = 0x20000;
		}
		v4 = 1;
		v5 = &player->spell_lvl[1];
		do {
			if (nox_xxx_spellHasFlags_424A50(v4, v3)) {
				if (nox_xxx_spellIsValid_424B50(v4)) {
					*v5 = 0;
					sub_461360(v4);
				}
			}
			++v4;
			++v5;
		} while (v4 < 137);
		return nox_xxx_guiSpellSortList_45ADF0(player->info.playerClass);
	}
	return result;
}

//----- (0045D400) --------------------------------------------------------
int sub_45D400(int a1) {
	nox_playerInfo* player; // edi
	int result;            // eax

	player = (nox_playerInfo*)dword_8531A0_2576;
	result = nox_xxx_bookHideMB_45ACA0(1);
	if (player) {
		player->beast_scroll_lvl[a1] = 0;
		sub_461360(a1 + 74);
		for (int group_index = 0; group_index < 41; ++group_index) {
			uint32_t* group = getMemPtr(0x587000, 132124 + 4 * group_index);
			if (!group) {
				break;
			}
			if (a1 == group[0]) {
				for (uint32_t* member = &group[1]; *member; ++member) {
					player->beast_scroll_lvl[*member] = 0;
					sub_461360(*member + 74);
				}
			}
		}
		result = nox_xxx_guiSpellSortList_45ADF0(player->info.playerClass);
	}
	return result;
}

//----- (0045D4A0) --------------------------------------------------------
char* nox_xxx_clientQuestDisableAbility_45D4A0(int a1) {
	char* result; // eax

	nox_xxx_bookHideMB_45ACA0(1);
	nox_xxx_netAbilityRewardCli_4611E0(a1, 0, 0);
	nox_playerInfo* local_player = (nox_playerInfo*)dword_5d4594_1047516;
	if (local_player) {
		local_player->spell_lvl[a1] = 0;
	}
	sub_461360(a1);
	nox_playerInfo* player = nox_common_playerInfoGetByID_417040(nox_player_netCode_85319C);
	result = (char*)player;
	if (player) {
		result = (char*)(intptr_t)nox_xxx_guiSpellSortList_45ADF0(player->info.playerClass);
	}
	return result;
}

//----- (0045D500) --------------------------------------------------------
int sub_45D500(int a1) {
	int result; // eax

	result = dword_5d4594_1046864;
	if (a1) {
		if (dword_5d4594_1046864) {
			result = nox_window_set_hidden(nox_win_unk1, 0);
		}
	} else if (dword_5d4594_1046864) {
		result = sub_47A260();
		if (!result) {
			result = nox_window_set_hidden(nox_win_unk1, 1);
		}
	}
	return result;
}

//----- (0045D550) --------------------------------------------------------
uint32_t* sub_45D550(uint32_t* a1) {
	uint32_t* result; // eax

	result = a1;
	if (a1) {
		result = (uint32_t*)nox_client_wndGetPosition_46AA60(dword_5d4594_1046952, a1, a1 + 1);
	}
	return result;
}

//----- (0045D570) --------------------------------------------------------
void nox_xxx_bookFillAll_45D570(int a1, int a2) {
	int result; // eax
	double v3;  // st7
	int2 a4;    // [esp+Ch] [ebp-10h]
	int2 a3;    // [esp+14h] [ebp-8h]

	result = dword_5d4594_1047520;
	if (dword_5d4594_1047520 != 1) {
		nox_xxx_quickBarClose_4606B0();
		*getMemU32Ptr(0x5D4594, 1046612) = nox_xxx_buttonsGetSelectedRow_45E180();
		if (a1 != 2 || (result = nox_xxx_spellHasFlags_424A50(a2, 0x15000)) == 0) {
			result = nox_xxx_buttonHaveSpellInBarMB_4612D0(a2);
			if (result != 1) {
				if (a1 == 2 || a1 == 4) {
					result = nox_xxx_buttonFindFirstEmptySlot_461250();
				} else {
					if (a1 != 3) {
						return;
					}
					result = sub_4612A0();
				}
				dword_5d4594_1046852 = result;
				if (result != -1) {
					dword_5d4594_1047520 = 1;
					if (a1 == 2 || a1 == 3) {
						dword_5d4594_1046868 = 0;
						dword_5d4594_1046872 = 0;
					} else {
						dword_5d4594_1046868 = 1;
						dword_5d4594_1046872 = 1;
					}
					sub_45D550(getMemAt(0x5D4594, 1046844));
					*(float*)&dword_5d4594_1046636 = (double)*getMemIntPtr(0x5D4594, 1046844);
					*(float*)&dword_5d4594_1046640 = (double)*getMemIntPtr(0x5D4594, 1046848);
					nox_window_setPos_46A9B0(dword_5d4594_1046956, *getMemIntPtr(0x5D4594, 1046844),
											 *getMemIntPtr(0x5D4594, 1046848));
					nox_xxx_buttonSetImgMB_461320(*(int*)&dword_5d4594_1046852, getMemAt(0x5D4594, 1046668));
					a3.field_0 = 400;
					a4.field_4 = 400;
					dword_5d4594_1047524 = a2;
					dword_5d4594_1046652 = a1 == 3;
					*getMemU32Ptr(0x5D4594, 1046676) = a1;
					nox_client_setRenderGUI(1);
					a3.field_4 = -500;
					a4.field_0 = 350;
					*getMemU32Ptr(0x5D4594, 1046680) = 0;
					sub_4BEDE0((int2*)getMemAt(0x5D4594, 1046844), (int2*)getMemAt(0x5D4594, 1046668), &a3, &a4, 19,
							   0.0, (uintptr_t)sub_45D7D0, NULL);
					*getMemU32Ptr(0x5D4594, 1046628) = 0;
					obj_5d4594_1046620.field_0 =
						*getMemFloatPtr(0x5D4594, 1046692) - *getMemFloatPtr(0x5D4594, 1046684);
					obj_5d4594_1046620.field_4 =
						*getMemFloatPtr(0x5D4594, 1046696) - *getMemFloatPtr(0x5D4594, 1046688);
					nox_xxx_utilNormalizeVector_509F20(&obj_5d4594_1046620);
					if (nox_win_width < 1000) {
						if (nox_win_width < 750) {
							v3 = 6.0;
						} else {
							v3 = 8.0;
						}
					} else {
						v3 = 10.0;
					}
					obj_5d4594_1046620.field_0 = obj_5d4594_1046620.field_0 * v3;
					obj_5d4594_1046620.field_4 = obj_5d4594_1046620.field_4 * v3;
					nox_window_set_hidden(dword_5d4594_1046956, 0);
					nox_xxx_wndShowModalMB_46A8C0(dword_5d4594_1046956);
					if (nox_common_gameFlags_check_40A5C0(2048)) {
						sub_57AF30(0, a1);
					}
					dword_5d4594_1046648 = nox_xxx_bookGet_430B40_get_mouse_prev_seq();
					if (!nox_common_gameFlags_check_40A5C0(2048) || nox_gui_xxx_check_446360() == 1 ||
						(result = nox_xxx_gameGet_4DB1B0(), result == 1)) {
						sub_45D870();
					}
				}
			}
		}
	}
}

//----- (0045D7D0) --------------------------------------------------------
int sub_45D7D0(int* a1, int* a2) {
	int result; // eax
	double v3;  // st7

	result = *getMemU32Ptr(0x5D4594, 1046680);
	if (*getMemIntPtr(0x5D4594, 1046680) < 20) {
		result = *getMemU32Ptr(0x5D4594, 1046680) + 1;
		*getMemFloatPtr(0x5D4594, 1046676 + 8 * result) = (double)*a1;
		*getMemFloatPtr(0x5D4594, 1046680 + 8 * result) = (double)a1[1];
		*getMemFloatPtr(0x5D4594, 1046684 + 8 * result) = (double)*a2;
		v3 = (double)a2[1];
		*getMemU32Ptr(0x5D4594, 1046680) = result;
		*getMemFloatPtr(0x5D4594, 1046688 + 8 * result) = v3;
	}
	return result;
}

//----- (0045D810) --------------------------------------------------------
void sub_45D810() {
	if (dword_5d4594_1047520) {
		dword_5d4594_1047520 = 0;
		nox_window_set_hidden(dword_5d4594_1046956, 1);
		dword_5d4594_1046648 = 0;
		nox_xxx_clientUpdateButtonRow_45E110(*getMemIntPtr(0x5D4594, 1046612));
		if (nox_common_gameFlags_check_40A5C0(2048)) {
			sub_57B0A0();
			sub_413A00(0);
		}
	}
}

//----- (0045D9B0) --------------------------------------------------------
int sub_45D9B0() { return dword_5d4594_1047520; }

//----- (0045D9D0) --------------------------------------------------------
int nox_xxx_guiSpellTargetClickSet_45D9D0(int a1) {
	if (*getMemU32Ptr(0x5D4594, 1047916)) {
		return 0;
	}
	*getMemU32Ptr(0x5D4594, 1047916) = a1;
	*getMemU8Ptr(0x5D4594, 1047920) = 0;
	nox_xxx_setKeybTimeout_4160D0(5);
	*getMemU32Ptr(0x5D4594, 1047928) = 0;
	*getMemU32Ptr(0x5D4594, 1047924) = 0;
	return 1;
}

//----- (0045DA10) --------------------------------------------------------
int nox_xxx_guiSpell_45DA10(int a1) {
	int result; // eax

	if (*getMemU32Ptr(0x5D4594, 1047916)) {
		return 0;
	}
	*getMemU32Ptr(0x5D4594, 1047916) = a1;
	*getMemU8Ptr(0x5D4594, 1047920) = 0;
	nox_xxx_setKeybTimeout_4160D0(5);
	result = 1;
	*getMemU32Ptr(0x5D4594, 1047924) = 1;
	return result;
}

//----- (0045DA50) --------------------------------------------------------
void nox_client_invokeSpellSlot_45DA50(int a1) {
	nox_playerInfo* player = (nox_playerInfo*)dword_8531A0_2576;
	if (!player || nox_xxx_playerAnimCheck_4372B0()) {
		return;
	}
	unsigned char* slot = nox_quickbar_selected_slot(nox_xxx_aClosewoodengat_587000_133480, a1);
	if (!slot) {
		return;
	}
	if (player->info.playerClass) {
		nox_xxx_clientSendSpell_45DB20((char*)slot, 1, slot[4] & 1);
	} else {
		nox_xxx_clientSendAbil_45DAF0(*(uint32_t*)slot);
	}
	nox_xxx_clientStoreLastButton_45DAD0(a1);
}

//----- (0045DAD0) --------------------------------------------------------
void nox_xxx_clientStoreLastButton_45DAD0(int a1) {
	*getMemU32Ptr(0x587000, 133484) = a1;
	*getMemU32Ptr(0x5D4594, 1049540) = gameFrame();
}

//----- (0045DAF0) --------------------------------------------------------
int nox_xxx_clientSendAbil_45DAF0(int a1) {
	int result; // eax
	short v3;   // [esp+0h] [ebp-2h]

	result = nox_xxx_guiCursor_477600();
	if (!result) {
		result = a1;
		if (a1) {
			HIBYTE(v3) = a1;
			LOBYTE(v3) = 122;
			result = nox_netlist_addToMsgListCli_40EBC0(31, 0, &v3, 2);
		}
	}
	return result;
}

//----- (0045DB20) --------------------------------------------------------
int nox_xxx_clientSendSpell_45DB20(char* a1, int a2, char a3) {
	int result;  // eax
	int v4;      // ecx
	char* v5;    // eax
	char v6[22]; // [esp+0h] [ebp-18h]

	result = nox_xxx_guiCursor_477600();
	if (!result && *(uint32_t*)a1) {
		v4 = 0;
		v6[0] = 121;
		v5 = &v6[1];
		do {
			if (v4 >= a2) {
				*(uint32_t*)v5 = 0;
			} else {
				*(uint32_t*)v5 = *(uint32_t*)&v5[a1 - &v6[1]];
			}
			++v4;
			v5 += 4;
		} while (v4 < 5);
		v6[21] = a3;
		result = nox_netlist_addToMsgListCli_40EBC0(31, 0, v6, 22);
	}
	return result;
}

//----- (0045DB90) --------------------------------------------------------
int sub_45DB90() {
	int result; // eax

	result = 0;
	memset(getMemAt(0x5D4594, 1049544), 0, 0x88u);
	*getMemU8Ptr(0x5D4594, 1049680) = 0;
	return result;
}

//----- (0045DBB0) --------------------------------------------------------
void nox_xxx_guiSpellTargetClickCheckSend_45DBB0() {
	if (*getMemU32Ptr(0x5D4594, 1047916)) {
		nox_xxx_clientSendSpell_45DB20((char*)getMemAt(0x5D4594, 1047916), 1, getMemByte(0x5D4594, 1047924));
		*getMemU32Ptr(0x5D4594, 1047916) = 0;
	}
}

//----- (0045DBE0) --------------------------------------------------------
unsigned char* nox_quickbar_selected_row(void* base) {
	if (!base) {
		return 0;
	}
	unsigned char* data = base;
	unsigned int row = data[200];
	if (row >= 5) {
		return 0;
	}
	return data + 40 * row;
}

unsigned char* nox_quickbar_trap_selected_row(void) {
	unsigned int row = getMemByte(0x5D4594, 1048140);
	if (row >= 3) {
		return 0;
	}
	return getMemAt(0x5D4594, 1047940 + 40 * row);
}

unsigned char* nox_quickbar_selected_slot(void* base, int slot) {
	if (slot < 0 || slot >= 5) {
		return 0;
	}
	unsigned char* row = nox_quickbar_selected_row(base);
	return row ? row + 8 * slot : 0;
}

void* nox_xxx_book_45DBE0(void* a1, int a2, int a3) {
	void* result; // eax

	nox_xxx_spellKeyPackSetSpell_45DC40(nox_xxx_aClosewoodengat_587000_133480, a2, a3);
	result = a1;
	if (a1 == (void*)2) {
		unsigned char* row = nox_quickbar_selected_row(nox_xxx_aClosewoodengat_587000_133480);
		if (!row || a3 < 0 || a3 >= 5) {
			return result;
		}
		if (a2 && (result = (void*)nox_xxx_spellHasFlags_424A50(a2, 0x600)) != 0) {
			row[8 * a3 + 4] = 1;
		} else {
			result = *(void**)&nox_xxx_aClosewoodengat_587000_133480;
			row[8 * a3 + 4] = 0;
		}
	}
	return result;
}

//----- (0045DC40) --------------------------------------------------------
void nox_xxx_spellKeyPackSetSpell_45DC40(void* a1, int a2, int a3) {
	wchar2_t* v3; // eax

	unsigned char* row = nox_quickbar_selected_row(a1);
	if (!row || a3 < 0 || a3 >= 5) {
		return;
	}
	*(uint32_t*)(row + 8 * a3) = a2;
	nox_window* win = nox_quickbar_button(a1, a3);
	if (win) {
		v3 = (wchar2_t*)nox_xxx_spellTitle_424930(a2);
		nox_xxx_wndWddSetTooltip_46B000(&win->draw_data, v3);
	}
	if (dword_8531A0_2576) {
		nox_xxx_clientPlaySoundSpecial_452D80(794, 100);
	}
}

//----- (0045DCA0) --------------------------------------------------------
int nox_xxx_bookSpellDrop_45DCA0(int a1, char a2, int a3, int a4, void* a5) {
	unsigned short old_spell = 137;
	unsigned char old_flags = 0;
	unsigned char* trap_data = getMemAt(0x5D4594, 1047940);
	nox_window* trap_root = nox_quickbar_root(trap_data);
	int trap_shown = trap_root && wndIsShown_nox_xxx_wndIsShown_46ACC0(trap_root);
	if ((trap_shown || !nox_xxx_spellPutInBox_45DEB0(trap_data, a1, a3, a4)) && a5 != trap_data) {
		int first_row = *getMemU32Ptr(0x5D4594, 1049476) ? 0 : 4;
		for (int row_index = first_row; row_index < 5; ++row_index) {
			unsigned char* data = getMemAt(0x5D4594, 1048196 + 256 * row_index);
			int slot = nox_xxx_spellBoxPointToWnd_45DE60(data, a3, a4);
			unsigned char* row = nox_quickbar_selected_row(data);
			if (slot >= 0 && row) {
				old_spell = *(uint16_t*)(row + 8 * slot);
				old_flags = row[8 * slot + 4];
			}
			if (!nox_xxx_spellPutInBox_45DEB0(data, a1, a3, a4)) {
				continue;
			}
			if (slot >= 0 && row) {
				row[8 * slot + 4] = nox_xxx_bookGetSpellDnDType_477670() == 1
					? nox_xxx_spellHasFlags_424A50(a1, 0x600)
					: a2;
			}
			nox_xxx_updateSpellIcons_45DDF0(data);
			break;
		}
	}
	return old_flags | (old_spell << 16);
}

//----- (0045DDF0) --------------------------------------------------------
int nox_xxx_updateSpellIcons_45DDF0(void* data) {
	int i;      // esi
	int result; // eax

	for (i = 0; i < 5; ++i) {
		result = nox_xxx_updateSpellIconDir_45DE10(i, data);
	}
	return result;
}

//----- (0045DE10) --------------------------------------------------------
int nox_xxx_updateSpellIconDir_45DE10(int index, void* data) {
	unsigned char* row = nox_quickbar_selected_row(data);
	if (!row || index < 0 || index >= 5) {
		return 0;
	}
	uint32_t spell = *(uint32_t*)(row + 8 * index);
	unsigned char flags = row[8 * index + 4];
	return sub_46AE10(nox_quickbar_nugget(data, index), (flags & 1) && spell != 0);
}

//----- (0045DE60) --------------------------------------------------------
int nox_xxx_spellBoxPointToWnd_45DE60(void* data, int x, int y) {
	for (int i = 0; i < 5; ++i) {
		nox_window* win = nox_quickbar_button(data, i);
		if (win && nox_xxx_wndPointInWnd_46AAB0(win, x, y)) {
			return i;
		}
	}
	return -1;
}

//----- (0045DF60) --------------------------------------------------------
int nox_xxx_guiSpellSetCursor_45DF60(int a1, char a2) {
	int result; // eax

	if (a1) {
		nox_xxx_clientPlaySoundSpecial_452D80(766, 100);
		if (!(a2 & 1) || nox_xxx_spellHasFlags_424A50(a1, 0x2000)) {
			*getMemU32Ptr(0x5D4594, 1047556) = a1;
			*getMemU32Ptr(0x5D4594, 1047928) = 1;
			result = 0;
		} else {
			nox_xxx_guiSpell_45DA10(a1);
			result = 1;
		}
	} else {
		*getMemU32Ptr(0x5D4594, 1047928) = 0;
		result = 0;
	}
	return result;
}

//----- (0045DFC0) --------------------------------------------------------
int sub_45DFC0(int a1) {
	int result; // eax
	int v2;     // esi
	int v3;     // eax

	if (a1) {
		v2 = *getMemU32Ptr(0x5D4594, 1047764 + 24*a1 + 4);
		if (v2) {
			dword_5d4594_1047932 = 0;
			dword_5d4594_1047936 = 0;
			nox_xxx_clientSendAbil_45DAF0(a1);
			nox_xxx_clientPlaySoundSpecial_452D80(766, 100);
			result = v2;
		} else {
			v3 = *getMemU32Ptr(0x5D4594, 1047764 + 24*a1);
			dword_5d4594_1047932 = 1;
			dword_5d4594_1047936 = v3;
			nox_xxx_clientPlaySoundSpecial_452D80(766, 100);
			result = 0;
		}
	} else {
		dword_5d4594_1047932 = 0;
		dword_5d4594_1047936 = 0;
		result = 0;
	}
	return result;
}

// 45E040: using guessed type int var_14[5];

//----- (0045E110) --------------------------------------------------------
int nox_xxx_clientUpdateButtonRow_45E110(int a1) {
	int result; // eax

	result = a1;
	if (a1 >= 0 && a1 < 5 && !*getMemU32Ptr(0x5D4594, 1049476) && !dword_5d4594_1049496) {
		unsigned char* data = nox_xxx_aClosewoodengat_587000_133480;
		if (!data) {
			return result;
		}
		data[200] = a1;
		if (dword_8531A0_2576) {
			nox_xxx_clientPlaySoundSpecial_452D80(798, 100);
		}
		// Preserve the original PE32 selected-row slot for 32-bit builds. Native
		// code derives it from data[200], so the packed data layout stays intact.
		if (sizeof(void*) == 4) {
			*(uint32_t*)(data + 204) = (uint32_t)(uintptr_t)(data + 40 * data[200]);
		}
		result = nox_xxx_updateSpellIcons_45DDF0(data);
	}
	return result;
}

//----- (0045E180) --------------------------------------------------------
int nox_xxx_buttonsGetSelectedRow_45E180() {
	if (!nox_xxx_aClosewoodengat_587000_133480) {
		return 0;
	}
	return *((unsigned char*)nox_xxx_aClosewoodengat_587000_133480 + 200);
}

//----- (0045EF30) --------------------------------------------------------
int nox_xxx_quickbarButtonBookDraw_45EF30() { return 1; }

//----- (0045EF40) --------------------------------------------------------
int sub_45EF40() { return 0; }

//----- (0045EF50) --------------------------------------------------------
int nox_xxx_quickBarWnd_45EF50(nox_window* win, int event, unsigned int pos) {
	void* data = nox_quickbar_data_for_window(win);
	nox_playerInfo* player = (nox_playerInfo*)dword_8531A0_2576;
	if (!data) {
		return 0;
	}
	if (*getMemU32Ptr(0x852978, 8) && (*(uint8_t*)(*getMemU32Ptr(0x852978, 8) + 120) & 2) == 2) {
		return 1;
	}
	int slot = -1;
	for (int i = 0; i < 5; ++i) {
		if (nox_quickbar_button(data, i) == win) {
			slot = i;
			break;
		}
	}
	if (slot < 0) {
		return 0;
	}
	unsigned char* row = nox_quickbar_selected_row(data);
	if (!row) {
		return 0;
	}
	uint32_t* spell = (uint32_t*)(row + 8 * slot);
	int x = (unsigned short)pos;
	int y = pos >> 16;
	switch (event) {
	case 5:
		if (!*spell || *getMemU32Ptr(0x5D4594, 1047928) || dword_5d4594_1047932 || nox_xxx_get_57AF20() ||
			(player && player->info.playerClass && !nox_xxx_spellIsEnabled_424B70(*spell))) {
			return 1;
		}
		nox_xxx_wndSetCaptureMain_46ADC0(win);
		nox_xxx_bookSaveSpellForDragDrop_477640(*spell, 2);
		dword_5d4594_1049692 = data;
		dword_5d4594_1049696 = row;
		nox_xxx_clientPlaySoundSpecial_452D80(793, 100);
		return 1;
	case 6:
	case 7: {
		if (!nox_xxx_wndGetCaptureMain_46AE00()) {
			return 1;
		}
		int target = nox_xxx_spellBoxPointToWnd_45DE60(data, x, y);
		if (!nox_xxx_get_57AF20() && !*getMemU32Ptr(0x5D4594, 1047928) && !dword_5d4594_1047932) {
			if (target == slot) {
				dword_5d4594_1049532 = win;
				if (data != getMemAt(0x5D4594, 1047940)) {
					if (player && player->info.playerClass) {
						if (nox_xxx_spellIsEnabled_424B70(*spell)) {
							nox_xxx_guiSpellSetCursor_45DF60(*spell, row[8 * slot + 4]);
							nox_xxx_clientStoreLastButton_45DAD0(slot);
						}
					} else {
						sub_45DFC0(*spell);
						nox_xxx_clientStoreLastButton_45DAD0(slot);
					}
				}
			} else if (target >= 0) {
				nox_xxx_clientSwapQuickbarKeys_45F300(data, slot, target);
				nox_xxx_clientPlaySoundSpecial_452D80(794, 100);
				nox_xxx_updateSpellIcons_45DDF0(data);
			} else {
				unsigned int dropped = nox_xxx_bookSpellDrop_45DCA0(*spell, row[8 * slot + 4], x, y, data);
				if ((dropped >> 16) != 137 && data != getMemAt(0x5D4594, 1047940)) {
					*spell = dropped >> 16;
					row[8 * slot + 4] = dropped;
					nox_xxx_updateSpellIcons_45DDF0(data);
				}
			}
		} else if (dword_5d4594_1047932 && target < 0) {
			sub_4611B0();
			dword_5d4594_1047932 = 0;
		} else if (*getMemU32Ptr(0x5D4594, 1047928)) {
			if (target < 0) {
				nox_xxx_guiSpellTargetClickSet_45D9D0(*getMemIntPtr(0x5D4594, 1047556));
			}
			*getMemU32Ptr(0x5D4594, 1047928) = 0;
		}
		if (!*getMemU32Ptr(0x5D4594, 1047928) && !dword_5d4594_1047932) {
			nox_xxx_wndClearCaptureMain_46ADE0(win);
			dword_5d4594_1049532 = 0;
		}
		nox_xxx_bookSpellDnDclear_477660();
		return 1;
	}
	case 8:
	case 12:
	case 16:
		return 0;
	default:
		return 1;
	}
}
// 45F261: variable 'v13' is possibly undefined

//----- (0045F300) --------------------------------------------------------
void nox_xxx_clientSwapQuickbarKeys_45F300(void* data, int a2, int a3) {
	unsigned char* row = nox_quickbar_selected_row(data);
	if (!row || a2 < 0 || a2 >= 5 || a3 < 0 || a3 >= 5) {
		return;
	}
	uint32_t pair[2] = {*(uint32_t*)(row + 8 * a2), *(uint32_t*)(row + 8 * a2 + 4)};
	memcpy(row + 8 * a2, row + 8 * a3, 8);
	memcpy(row + 8 * a3, pair, 8);
	nox_window* w2 = nox_quickbar_button(data, a2);
	nox_window* w3 = nox_quickbar_button(data, a3);
	if (w2) {
		nox_xxx_wndWddSetTooltip_46B000(&w2->draw_data,
			(wchar2_t*)nox_xxx_spellTitle_424930(*(uint32_t*)(row + 8 * a2)));
	}
	if (w3) {
		nox_xxx_wndWddSetTooltip_46B000(&w3->draw_data,
			(wchar2_t*)nox_xxx_spellTitle_424930(*(uint32_t*)(row + 8 * a3)));
	}
}

//----- (0045F390) --------------------------------------------------------
void sub_45F390(void* a1, void* a2, int a3, int a4) {
	unsigned char* row1 = a1;
	unsigned char* row2 = a2;
	uint32_t v4;
	uint32_t v5;
	wchar2_t* v6; // eax

	v4 = *(uint32_t*)(row2 + 8 * a4);
	v5 = *(uint32_t*)(row2 + 8 * a4 + 4);
	memcpy(row2 + 8 * a4, row1 + 8 * a3, 8);
	*(uint32_t*)(row1 + 8 * a3) = v4;
	*(uint32_t*)(row1 + 8 * a3 + 4) = v5;
	v6 = (wchar2_t*)nox_xxx_spellTitle_424930(*(uint32_t*)(row2 + 8 * a4));
	nox_window* win = nox_quickbar_button(nox_xxx_aClosewoodengat_587000_133480, a4);
	if (win) {
		nox_xxx_wndWddSetTooltip_46B000(&win->draw_data, v6);
	}
}

//----- (0045F450) --------------------------------------------------------
int nox_xxx_quickbarButtonBookWnd_45F450(nox_window* win, unsigned int event) {
	(void)win;
	if (event >= 5) {
		if (event <= 6) {
			return 1;
		}
		if (event == 7) {
			nox_client_toggleSpellbook_45AC70();
			return 1;
		}
	}
	return 0;
}

//----- (0045F500) --------------------------------------------------------
int sub_45F500(int slot, void* data) {
	nox_window* nugget = nox_quickbar_nugget(data, slot);
	return nugget ? (nugget->draw_data.field_0 >> 1) & 1 : 0;
}

//----- (0045F520) --------------------------------------------------------
int sub_45F520(nox_window* win, unsigned int event) {
	if (!win) {
		return 0;
	}
	unsigned int row_index = win->field_92 >> 16;
	unsigned int slot = win->field_92 & 0xffff;
	if (row_index >= 5 || slot >= 5) {
		return 0;
	}
	unsigned char* data = getMemAt(0x5D4594, 1048196 + 256 * row_index);
	unsigned char* row = nox_quickbar_selected_row(data);
	if (!row) {
		return 0;
	}
	unsigned char* entry = row + 8 * slot;
	uint32_t spell = *(uint32_t*)entry;
	if (!spell) {
		return 0;
	}
	if (event != 5) {
		if (event > 5 && event <= 7) {
			return 1;
		}
		return 0;
	}
	if (nox_xxx_spellHasFlags_424A50(spell, 0x200400)) {
		nox_xxx_clientPlaySoundSpecial_452D80(925, 100);
	} else {
		entry[4] ^= 1u;
		nox_xxx_updateSpellIconDir_45DE10(slot, data);
		nox_xxx_clientPlaySoundSpecial_452D80(921, 100);
	}
	return 1;
}

//----- (0045F5D0) --------------------------------------------------------
int sub_45F5D0(nox_window* win, nox_window_data* draw_data) {
	short* v2; // esi
	unsigned int x;
	unsigned int y;

	(void)draw_data;
	if (!win) {
		return 0;
	}
	nox_client_wndGetPosition_46AA60(win, &x, &y);
	v2 = (short*)sub_42E8E0(win->field_92 + 28, 1);
	if (v2) {
		nox_xxx_drawSetTextColor_434390(nox_color_white_2523948);
		nox_xxx_drawString_43F6E0(nox_quickbar_font, v2, x, y);
	}
	return 1;
}

//----- (0045F630) --------------------------------------------------------
int nox_xxx_quickbarTrapUpDownProc_45F630(nox_window* win, unsigned int event) {
	if (!win) {
		return 0;
	}
	unsigned int control = win->field_92;
	if ((!*getMemU32Ptr(0x852978, 8) || (*(uint8_t*)(*getMemU32Ptr(0x852978, 8) + 120) & 2) != 2) && !sub_4AE3D0()) {
		if (event == 5) {
			if (control <= 2 && dword_5d4594_1049508) {
				dword_5d4594_1049508->draw_data.bg_image = 0;
			}
			switch (control) {
			case 0:
				nox_client_spellSetPrev_460540();
				break;
			case 1:
				nox_client_spellSetNext_4604F0();
				break;
			case 2:
				sub_460920();
				break;
			case 3:
				nox_client_trapSetPrev_4603F0();
				break;
			case 4:
				nox_client_trapSetNext_4603A0();
				break;
			default:
				break;
			}
			*getMemU32Ptr(0x5D4594, 1049700) = 2;
			*getMemU32Ptr(0x5D4594, 1049704) = control;
		} else if (event <= 5 || event > 7) {
			return 0;
		}
	}
	return 1;
}

//----- (0045F6F0) --------------------------------------------------------
int nox_xxx_quickbarTrapUpDownDraw_45F6F0(nox_window* win, nox_window_data* draw_data) {
	if (!win) {
		return 0;
	}
	nox_window_data* data = draw_data ? draw_data : &win->draw_data;
	unsigned int x;
	unsigned int y;
	nox_client_wndGetPosition_46AA60(win, &x, &y);
	int draw_x = (int)x + (int)data->img_px;
	int draw_y = (int)y + (int)data->img_py;
	if (*getMemIntPtr(0x5D4594, 1049700) <= 0 ||
		*getMemIntPtr(0x5D4594, 1049704) != (int)win->field_92) {
		if (data->bg_image) {
			nox_client_drawImageAt_47D2C0(data->bg_image, draw_x, draw_y);
		}
	} else {
		if (data->hl_image) {
			nox_client_drawImageAt_47D2C0(data->hl_image, draw_x, draw_y);
		}
		if (!--*getMemU32Ptr(0x5D4594, 1049700) && *getMemU32Ptr(0x5D4594, 1049704) <= 2) {
			if (dword_5d4594_1049508) {
				dword_5d4594_1049508->draw_data.bg_image = dword_5d4594_1049508->draw_data.hl_image;
			}
		}
	}
	sub_46AE10(dword_5d4594_1049524, sub_45CFC0());
	return 1;
}

//----- (0045F7A0) --------------------------------------------------------
int nox_xxx_quickbarTrapButtonProc_45F7A0(nox_window* win, unsigned int event, unsigned int pos) {
	if (!win) {
		return 0;
	}
	if (event < 5) {
		sub_46AE10(dword_5d4594_1049504, 0);
		return 0;
	}
	if (event <= 6) {
		return 1;
	}
	if (event != 7) {
		sub_46AE10(dword_5d4594_1049504, 0);
		return 0;
	}
	if (*getMemU32Ptr(0x5D4594, 1047928)) {
		if (!nox_xxx_wndPointInWnd_46AAB0(win, (unsigned short)pos, pos >> 16)) {
			nox_client_buildTrap_45E040();
		}
		nox_xxx_wndClearCaptureMain_46ADE0(win);
		dword_5d4594_1049532 = 0;
		*getMemU32Ptr(0x5D4594, 1047928) = 0;
		return 1;
	}
	sub_46AE10(dword_5d4594_1049504, 1);
	nox_playerInfo* player = (nox_playerInfo*)dword_8531A0_2576;
	if (!player) {
		return 1;
	}
	if (player->info.playerClass == 1 && sub_45F890()) {
		nox_xxx_wndSetCaptureMain_46ADC0(win);
		dword_5d4594_1049532 = win;
		*getMemU32Ptr(0x5D4594, 1047928) = 1;
	} else {
		nox_client_buildTrap_45E040();
	}
	return 1;
}

//----- (0045F890) --------------------------------------------------------
int sub_45F890() {
	unsigned char* row = nox_quickbar_trap_selected_row();
	if (!row) {
		return 0;
	}
	for (int slot = 0; slot < 3; ++slot) {
		uint32_t spell = *(uint32_t*)(row + 8 * slot);
		if (spell && (nox_xxx_spellFlags_424A70(spell) & 8) == 8) {
			return 1;
		}
	}
	return 0;
}

//----- (0045F8D0) --------------------------------------------------------
int nox_xxx_quickbar_45F8D0(nox_window* win, int event, uintptr_t a3, uintptr_t a4) {
	(void)win;
	(void)a3;
	(void)a4;
	return event != 8 && event != 12 && event != 16;
}

//----- (0045F8F0) --------------------------------------------------------
int sub_45F8F0() {
	sub_45F900();
	return 1;
}

//----- (0045F900) --------------------------------------------------------
void sub_45F900() {
	unsigned char* data = nox_xxx_aClosewoodengat_587000_133480;
	if (!data) {
		return;
	}
	int sequence = nox_xxx_bookGet_430B40_get_mouse_prev_seq();
	if (dword_5d4594_1049496) {
		if (*getMemU32Ptr(0x5D4594, 1049492) == (uint32_t)sequence) {
			return;
		}
		dword_5d4594_1049496 = 0;
	} else {
		if (*getMemU32Ptr(0x5D4594, 1049492) != (uint32_t)sequence) {
			return;
		}
		dword_5d4594_1049496 = 1;
	}
	unsigned char* row = nox_quickbar_selected_row(data);
	if (!row) {
		return;
	}
	for (int i = 0; i < 5; ++i) {
		uint32_t spell = *(uint32_t*)(row + 8 * i);
		if (spell && !nox_xxx_spellHasFlags_424A50(spell, 0x200400)) {
			row[8 * i + 4] ^= 1u;
			nox_xxx_updateSpellIconDir_45DE10(i, data);
		}
	}
	nox_xxx_updateSpellIcons_45DDF0(data);
	nox_xxx_clientPlaySoundSpecial_452D80(921, 100);
}

//----- (0045FB90) --------------------------------------------------------
int nox_xxx_quickbarTrapProc_45FB90(nox_window* win, unsigned int event, uintptr_t a3, uintptr_t a4) {
	(void)a3;
	(void)a4;
	if (event >= 5) {
		if (event <= 6) {
			return 1;
		}
		if (event == 7) {
			if (win && !wndIsShown_nox_xxx_wndIsShown_46ACC0(win)) {
				sub_461060();
			}
			return 1;
		}
	}
	return 0;
}

//----- (00460000) --------------------------------------------------------
int nox_xxx_quickbarDrawFn_460000(nox_window* win, nox_window_data* draw_data) {
	(void)win;
	(void)draw_data;
	if (!dword_5d4594_1049504) {
		return 0;
	}
	int y = (int)dword_5d4594_1049504->off_y;
	int delta = y - (int)dword_5d4594_1049536;
	if (delta >= 0) {
		if (delta <= 0) {
			if (dword_5d4594_1049536 > nox_win_height) {
				sub_460070();
				dword_5d4594_1049536 = nox_win_height - 74;
			}
		} else {
			nox_window_setPos_46A9B0(dword_5d4594_1049504, (int)dword_5d4594_1049504->off_x, y - 1);
		}
	} else {
		nox_window_setPos_46A9B0(dword_5d4594_1049504, (int)dword_5d4594_1049504->off_x, y + 1);
	}
	return 1;
}

typedef struct nox_quickbar_window_set {
	void* data;
	nox_window* root;
	nox_window* buttons[5];
	nox_window* nuggets[5];
	nox_window* nugget_children[5];
} nox_quickbar_window_set;

static nox_quickbar_window_set nox_quickbar_window_sets[8];

static nox_quickbar_window_set* nox_quickbar_windows(void* data, int create) {
	nox_quickbar_window_set* empty = 0;
	for (size_t i = 0; i < sizeof(nox_quickbar_window_sets) / sizeof(nox_quickbar_window_sets[0]); ++i) {
		nox_quickbar_window_set* set = &nox_quickbar_window_sets[i];
		if (set->data == data) {
			return set;
		}
		if (!set->data && !empty) {
			empty = set;
		}
	}
	if (create && empty) {
		empty->data = data;
		return empty;
	}
	return 0;
}

nox_window* nox_quickbar_root(void* data) {
	nox_quickbar_window_set* set = nox_quickbar_windows(data, 0);
	return set ? set->root : 0;
}

nox_window* nox_quickbar_button(void* data, int index) {
	nox_quickbar_window_set* set = nox_quickbar_windows(data, 0);
	return set && index >= 0 && index < 5 ? set->buttons[index] : 0;
}

nox_window* nox_quickbar_nugget(void* data, int index) {
	nox_quickbar_window_set* set = nox_quickbar_windows(data, 0);
	return set && index >= 0 && index < 5 ? set->nuggets[index] : 0;
}

nox_window* nox_quickbar_nugget_child(void* data, int index) {
	nox_quickbar_window_set* set = nox_quickbar_windows(data, 0);
	return set && index >= 0 && index < 5 ? set->nugget_children[index] : 0;
}

void nox_quickbar_set_nugget(void* data, int index, nox_window* win) {
	nox_quickbar_window_set* set = nox_quickbar_windows(data, 1);
	if (set && index >= 0 && index < 5) {
		set->nuggets[index] = win;
	}
}

void nox_quickbar_set_nugget_child(void* data, int index, nox_window* win) {
	nox_quickbar_window_set* set = nox_quickbar_windows(data, 1);
	if (set && index >= 0 && index < 5) {
		set->nugget_children[index] = win;
	}
}

void* nox_quickbar_data_for_window(nox_window* win) {
	for (size_t i = 0; i < sizeof(nox_quickbar_window_sets) / sizeof(nox_quickbar_window_sets[0]); ++i) {
		nox_quickbar_window_set* set = &nox_quickbar_window_sets[i];
		if (set->root == win) {
			return set->data;
		}
		for (size_t j = 0; j < sizeof(set->buttons) / sizeof(set->buttons[0]); ++j) {
			if (set->buttons[j] == win || set->nuggets[j] == win || set->nugget_children[j] == win) {
				return set->data;
			}
		}
	}
	return 0;
}

void nox_quickbar_windows_clear(void* data) {
	nox_quickbar_window_set* set = nox_quickbar_windows(data, 0);
	if (set) {
		memset(set, 0, sizeof(*set));
	}
}

void nox_quickbar_windows_destroy_all(void) {
	for (size_t i = 0; i < sizeof(nox_quickbar_window_sets) / sizeof(nox_quickbar_window_sets[0]); ++i) {
		nox_quickbar_window_set* set = &nox_quickbar_window_sets[i];
		nox_xxx_windowDestroyMB_46C4E0(set->root);
		for (size_t j = 0; j < sizeof(set->buttons) / sizeof(set->buttons[0]); ++j) {
			nox_xxx_windowDestroyMB_46C4E0(set->buttons[j]);
			nox_xxx_windowDestroyMB_46C4E0(set->nuggets[j]);
			nox_xxx_windowDestroyMB_46C4E0(set->nugget_children[j]);
		}
		memset(set, 0, sizeof(*set));
	}
}

//----- (004601F0) --------------------------------------------------------
int nox_xxx_quickBarInitWindow_4601F0(void* data, int x, int y, int count, int spacing, void* proc, void* draw) {
	nox_quickbar_window_set* set = nox_quickbar_windows(data, 1);
	if (!data || !set || count < 0 || count > 5) {
		return 0;
	}
	set->root = nox_window_new(0, 1224, x - 10, y - 15, 199, 59, 0);
	if (!set->root) {
		nox_quickbar_windows_clear(data);
		return 0;
	}
	nox_xxx_wndSetIcon_46AE60(set->root, nox_xxx_gLoadImg_42F970("QuickBarBase"));
	nox_xxx_wndSetOffsetMB_46AE40(set->root, -61, -18);
	nox_xxx_wndSetWindowProc_46B300(set->root, nox_xxx_quickbar_45F8D0);
	int button_x = spacing + 10;
	unsigned char* widths = getMemAt(0x587000, 133488);
	for (int i = 0; i < count; ++i) {
		set->buttons[i] = nox_window_new(set->root, 1032, button_x, 15, 34, 34, 0);
		if (!set->buttons[i]) {
			return 0;
		}
		nox_window_set_all_funcs(set->buttons[i], (int (*)(int, int, int, int))proc,
							 (int (*)(nox_window*, void*))draw, 0);
		if (sizeof(void*) == 4) {
			*(nox_window**)((unsigned char*)data + 212 + 4 * i) = set->buttons[i];
			set->buttons[i]->field_92 = (uint32_t)(uintptr_t)data;
		}
		button_x += *(uint32_t*)widths;
		widths += 4;
	}
	if (sizeof(void*) == 4) {
		*(nox_window**)((unsigned char*)data + 208) = set->root;
		*(void**)((unsigned char*)data + 204) = (unsigned char*)data + 40 * *((unsigned char*)data + 200);
	}
	return *((unsigned char*)data + 200);
}

//----- (004602F0) --------------------------------------------------------
void* sub_4602F0() {
	int v0;
	unsigned char* v1;
	unsigned char* v6;

	nox_xxx_quickBarClose_4606B0();
	unsigned char* main_data = nox_xxx_aClosewoodengat_587000_133480;
	if (!main_data) {
		return 0;
	}
	v0 = 0;
	v1 = getMemAt(0x5D4594, 1048196);
	do {
		unsigned char* dst = main_data + v0;
		for (int i = 0; i < 5; ++i) {
			*(uint32_t*)dst = 0;
			dst[4] = 0;
			*(uint32_t*)v1 = 0;
			v1[4] = 0;
			dst += 8;
			v1 += 8;
		}
		v1 += 216;
		v0 += 40;
	} while (v1 < (unsigned char*)getMemAt(0x5D4594, 1049476));
	v6 = getMemAt(0x5D4594, 1047940);
	do {
		unsigned char* dst = v6;
		for (int i = 0; i < 3; ++i) {
			*(uint32_t*)dst = 0;
			dst[4] = 0;
			dst += 8;
		}
		v6 += 40;
	} while (v6 < (unsigned char*)getMemAt(0x5D4594, 1048060));
	// Window pointers live in the native-width sidecar; the packed +232 slot is
	// only a PE32 compatibility mirror.
	nox_xxx_updateSpellIcons_45DDF0(main_data);
	return main_data;
}

//----- (00460380) --------------------------------------------------------
void* sub_460380() {
	unsigned char* v0; // eax

	v0 = getMemAt(0x5D4594, 1047764 + 24*1 + 16);
	do {
		*(uint32_t*)v0 = 0;
		v0 += 24;
	} while (v0 < (unsigned char*)getMemAt(0x5D4594, 1047924));
	return sub_4602F0();
}

//----- (004603A0) --------------------------------------------------------
void nox_client_trapSetNext_4603A0() {
	if (getMemByte(0x5D4594, 1048140) == 2) {
		*getMemU8Ptr(0x5D4594, 1048140) = 0;
	} else {
		++*getMemU8Ptr(0x5D4594, 1048140);
	}
	if (sizeof(void*) == 4) {
		*getMemU32Ptr(0x5D4594, 1048144) =
			(uint32_t)(uintptr_t)nox_quickbar_trap_selected_row();
	}
	nox_xxx_clientPlaySoundSpecial_452D80(798, 100);
}

//----- (004603F0) --------------------------------------------------------
void nox_client_trapSetPrev_4603F0() {
	if (getMemByte(0x5D4594, 1048140)) {
		--*getMemU8Ptr(0x5D4594, 1048140);
	} else {
		*getMemU8Ptr(0x5D4594, 1048140) = 2;
	}
	if (sizeof(void*) == 4) {
		*getMemU32Ptr(0x5D4594, 1048144) =
			(uint32_t)(uintptr_t)nox_quickbar_trap_selected_row();
	}
	nox_xxx_clientPlaySoundSpecial_452D80(798, 100);
}

//----- (004604B0) --------------------------------------------------------
int nox_client_trapSetSelect_4604B0(int a1) {
	int result; // eax

	result = a1;
	if (a1 >= 0 && a1 < 3) {
		*getMemU8Ptr(0x5D4594, 1048140) = a1;
		result = 5 * (unsigned char)a1;
		if (sizeof(void*) == 4) {
			*getMemU32Ptr(0x5D4594, 1048144) =
				(uint32_t)(uintptr_t)nox_quickbar_trap_selected_row();
		}
	}
	return result;
}

//----- (004604E0) --------------------------------------------------------
int sub_4604E0() { return getMemByte(0x5D4594, 1048140); }

//----- (004604F0) --------------------------------------------------------
int nox_client_spellSetNext_4604F0() {
	int result; // eax
	int v1;     // eax

	result = *getMemU32Ptr(0x5D4594, 1049476);
	if (!*getMemU32Ptr(0x5D4594, 1049476)) {
		result = dword_5d4594_1049496;
		if (!dword_5d4594_1049496) {
			result = nox_xxx_get_57AF20();
			if (!result) {
				nox_xxx_clientStoreLastButton_45DAD0(-1);
				v1 = nox_xxx_buttonsGetSelectedRow_45E180() + 1;
				if (v1 > 4) {
					v1 = 0;
				}
				result = nox_xxx_clientUpdateButtonRow_45E110(v1);
			}
		}
	}
	return result;
}

//----- (00460540) --------------------------------------------------------
int nox_client_spellSetPrev_460540() {
	int result; // eax
	int v1;     // eax

	result = *getMemU32Ptr(0x5D4594, 1049476);
	if (!*getMemU32Ptr(0x5D4594, 1049476)) {
		result = dword_5d4594_1049496;
		if (!dword_5d4594_1049496) {
			result = nox_xxx_get_57AF20();
			if (!result) {
				nox_xxx_clientStoreLastButton_45DAD0(-1);
				v1 = nox_xxx_buttonsGetSelectedRow_45E180() - 1;
				if (v1 < 0) {
					v1 = 4;
				}
				result = nox_xxx_clientUpdateButtonRow_45E110(v1);
			}
		}
	}
	return result;
}

//----- (00460590) --------------------------------------------------------
void nox_client_spellSetSelect_460590() {
	int result; // eax

	result = *getMemU32Ptr(0x5D4594, 1049476);
	if (!*getMemU32Ptr(0x5D4594, 1049476)) {
		result = dword_5d4594_1049496;
		if (!dword_5d4594_1049496) {
			unsigned char* data = nox_xxx_aClosewoodengat_587000_133480;
			if (!data) {
				return;
			}
			if (nox_xxx_checkKeybTimeout_4160F0(7u, gameFPS() >> 1)) {
				LOBYTE(result) = 0;
				*getMemU32Ptr(0x5D4594, 1049712) = 0;
			} else {
				result = ++*getMemU32Ptr(0x5D4594, 1049712);
				if (*getMemIntPtr(0x5D4594, 1049712) >= 5) {
					result = 4;
				}
			}
			data[200] = (uint8_t)result;
			// Preserve the packed PE32 selected-row slot only on 32-bit builds.
			// Native builds derive the row address from its index at +200.
			if (sizeof(void*) == 4) {
				*(uint32_t*)(data + 204) = (uint32_t)(uintptr_t)(data + 40 * data[200]);
			}
			nox_xxx_setKeybTimeout_4160D0(7);
			nox_xxx_clientPlaySoundSpecial_452D80(798, 100);
			nox_xxx_updateSpellIcons_45DDF0(data);
		}
	}
}

//----- (00460630) --------------------------------------------------------
void sub_460630() {
	if (!*getMemU32Ptr(0x5D4594, 1049476)) {
		int result = nox_xxx_bookGet_430B40_get_mouse_prev_seq();
		*getMemU32Ptr(0x5D4594, 1049492) = result;
	}
}

//----- (00460650) --------------------------------------------------------
int nox_xxx_guiSpell_460650() { return *getMemU32Ptr(0x5D4594, 1047928); }

//----- (00460660) --------------------------------------------------------
int sub_460660() {
	if (dword_5d4594_1049532 || *getMemU32Ptr(0x5D4594, 1047928) || dword_5d4594_1047932) {
		nox_window* capture = dword_5d4594_1049532;
		if (!capture) {
			capture = nox_xxx_wndGetCaptureMain_46AE00();
		}
		if (capture) {
			nox_xxx_wndClearCaptureMain_46ADE0(capture);
		}
		dword_5d4594_1049532 = 0;
		*getMemU32Ptr(0x5D4594, 1047928) = 0;
		dword_5d4594_1047932 = 0;
		return 1;
	}
	return 0;
}

//----- (004606B0) --------------------------------------------------------
int nox_xxx_quickBarClose_4606B0() {
	if (*getMemU32Ptr(0x5D4594, 1049476) != 1) {
		return 0;
	}
	unsigned char* main_data = nox_xxx_aClosewoodengat_587000_133480;
	if (!main_data) {
		return 0;
	}
	for (int row = 0; row < 5; ++row) {
		unsigned char* expanded = getMemAt(0x5D4594, 1048196 + 256 * row);
		for (int slot = 0; slot < 5; ++slot) {
			unsigned char* src = expanded + 8 * slot;
			unsigned char* dst = main_data + 40 * row + 8 * slot;
			memcpy(dst, src, sizeof(uint32_t));
			dst[4] = src[4];
			nox_window_set_hidden(nox_quickbar_nugget(expanded, slot), 1);
		}
		nox_window_set_hidden(nox_quickbar_root(expanded), 1);
	}
	// Opening stores the original row at 1047912; restore that row on close.
	uint32_t selected = *getMemU32Ptr(0x5D4594, 1047912);
	main_data[200] = selected < 5 ? selected : 0;
	if (sizeof(void*) == 4) {
		*(uint32_t*)(main_data + 204) = (uint32_t)(uintptr_t)(main_data + 40 * main_data[200]);
	}
	nox_xxx_updateSpellIcons_45DDF0(main_data);
	nox_window_set_hidden(dword_5d4594_1049512, 0);
	nox_xxx_clientPlaySoundSpecial_452D80(800, 100);
	*getMemU32Ptr(0x5D4594, 1049476) = 0;
	return 1;
}

//----- (00460920) --------------------------------------------------------
void sub_460920() {
	if (*getMemU32Ptr(0x5D4594, 1049476)) {
		nox_xxx_quickBarClose_4606B0();
		return;
	}
	if (dword_5d4594_1049496) {
		return;
	}
	if (nox_xxx_get_57AF20()) {
		return;
	}
	if (dword_5d4594_1049484 == 1) {
		sub_461010();
	}
	unsigned char* main_data = nox_xxx_aClosewoodengat_587000_133480;
	if (!main_data) {
		return;
	}
	for (int row = 0; row < 5; ++row) {
		unsigned char* expanded = getMemAt(0x5D4594, 1048196 + 256 * row);
		for (int slot = 0; slot < 5; ++slot) {
			unsigned char* src = main_data + 40 * row + 8 * slot;
			unsigned char* dst = expanded + 8 * slot;
			memcpy(dst, src, sizeof(uint32_t));
			dst[4] = src[4];
			nox_window_set_hidden(nox_quickbar_nugget(expanded, slot), 0);
		}
		nox_window_set_hidden(nox_quickbar_root(expanded), 0);
		nox_xxx_updateSpellIcons_45DDF0(expanded);
	}
	*getMemU32Ptr(0x5D4594, 1047912) = main_data[200];
	main_data[200] = 4;
	if (sizeof(void*) == 4) {
		*(uint32_t*)(main_data + 204) = (uint32_t)(uintptr_t)(main_data + 40 * main_data[200]);
	}
	nox_xxx_updateSpellIcons_45DDF0(main_data);
	nox_window_set_hidden(dword_5d4594_1049512, 1);
	nox_xxx_clientPlaySoundSpecial_452D80(799, 100);
	*getMemU32Ptr(0x5D4594, 1049476) = 1;
}

//----- (00460940) --------------------------------------------------------
int sub_460940(void* this) {
	int result; // eax
	uint8_t data_kind;

	(void)this;
	if (!nox_crypt_IsReadOnly() && sub_461450() == 1) {
		sub_461400();
		sub_461440(0);
	}
	nox_playerInfo* player = (nox_playerInfo*)dword_8531A0_2576;
	if (player) {
		data_kind = player->info.playerClass;
	} else {
		data_kind = nox_xxx_getHostInfoPtr_431770()[66];
	}
	nox_xxx_fileReadWrite_426AC0_file3_fread(&data_kind, 1u);
	if (!data_kind) {
		result = sub_460A10(nox_xxx_aClosewoodengat_587000_133480, 5, 5, 0);
		if (!result) {
			return result;
		}
		return 1;
	}
	result = sub_460A10(nox_xxx_aClosewoodengat_587000_133480, 5, 5, (char)data_kind);
	if (result) {
		result = sub_460A10(getMemAt(0x5D4594, 1047940), 3, 3, (char)data_kind);
		if (result) {
			if (nox_quickbar_nugget(nox_xxx_aClosewoodengat_587000_133480, 0)) {
				nox_xxx_updateSpellIcons_45DDF0(nox_xxx_aClosewoodengat_587000_133480);
			}
			return 1;
		}
	}
	return result;
}

//----- (00460A10) --------------------------------------------------------
int sub_460A10(void* a1, int a2, int a3, char a4) {
	uint8_t* v4;   // esi
	int v5;        // ebp
	char* v6;      // eax
	bool v7;       // zf
	size_t v9;     // [esp-14h] [ebp-120h]
	int v10;       // [esp+0h] [ebp-10Ch]
	uint8_t* v11;  // [esp+4h] [ebp-108h]
	int v12;       // [esp+8h] [ebp-104h]
	char v13[256]; // [esp+Ch] [ebp-100h]

	if (a2 <= 0) {
		return 1;
	}
	v4 = a1;
	v11 = a1;
	v12 = a2;
	do {
		v5 = a3;
		if (a3 > 0) {
			do {
				if (nox_crypt_IsReadOnly() == 1) {
					nox_xxx_fileReadWrite_426AC0_file3_fread(&v10, 1u);
					nox_xxx_fileReadWrite_426AC0_file3_fread(v13, (unsigned char)v10);
					v13[(unsigned char)v10] = 0;
					if (a4) {
						*(uint32_t*)v4 = nox_xxx_spellNameToN_4243F0(v13);
					} else {
						*(uint32_t*)v4 = nox_xxx_abilityNameToN_424D80(v13);
					}
				} else {
					if (a4) {
						LOBYTE(v10) = strlen(nox_xxx_spellNameByN_424870(*(uint32_t*)v4));
						nox_xxx_fileReadWrite_426AC0_file3_fread(&v10, 1u);
						v9 = (unsigned char)v10;
						v6 = nox_xxx_spellNameByN_424870(*(uint32_t*)v4);
					} else {
						LOBYTE(v10) = strlen(nox_xxx_abilityGetName_425250(*(uint32_t*)v4));
						nox_xxx_fileReadWrite_426AC0_file3_fread(&v10, 1u);
						v9 = (unsigned char)v10;
						v6 = nox_xxx_abilityGetName_425250(*(uint32_t*)v4);
					}
					nox_xxx_fileReadWrite_426AC0_file3_fread(v6, v9);
				}
				nox_xxx_fileReadWrite_426AC0_file3_fread((uint8_t*)(v4 + 4), 1u);
				v4 += 8;
				--v5;
			} while (v5);
		}
		v4 = v11 + 40;
		v7 = v12 == 1;
		v11 += 40;
		--v12;
	} while (!v7);
	return 1;
}
// 460A10: using guessed type char var_100[256];

//----- (00460B90) --------------------------------------------------------
int sub_460B90(int a1) {
	if (!a1 && *getMemU32Ptr(0x5D4594, 1049476)) {
		sub_460920();
	}
	int hidden = a1 ? 0 : 1;
	nox_window_set_hidden(dword_5d4594_1049500, hidden);
	nox_window_set_hidden(dword_5d4594_1049504, hidden);
	nox_window_set_hidden(dword_5d4594_1049520, hidden);
	nox_window_set_hidden(dword_5d4594_1049508, hidden);
	nox_window_set_hidden(dword_5d4594_1049512, hidden);
	void* main_data = nox_xxx_aClosewoodengat_587000_133480;
	nox_window_set_hidden(nox_quickbar_root(main_data), hidden);
	for (int i = 0; i < 5; ++i) {
		nox_window_set_hidden(nox_quickbar_button(main_data, i), hidden);
		nox_window_set_hidden(nox_quickbar_nugget(main_data, i), hidden);
	}
	void* trap_data = getMemAt(0x5D4594, 1047940);
	int trap_hidden = !a1 || !dword_5d4594_1049484;
	nox_window_set_hidden(nox_quickbar_root(trap_data), trap_hidden);
	for (int i = 0; i < 3; ++i) {
		nox_window_set_hidden(nox_quickbar_button(trap_data, i), trap_hidden);
		nox_window_set_hidden(nox_quickbar_nugget(trap_data, i), trap_hidden);
	}
	if (a1 && dword_5d4594_1049484) {
		nox_window_set_hidden(dword_5d4594_1049512, 1);
	}
	return 1;
}
