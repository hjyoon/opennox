...
typedef int32_t (*sentry_xfer_fn_4F5E50)(nox_object_t*);

_Static_assert(CHAR_BIT == 8, "serialized bytes must remain eight bits");
_Static_assert(sizeof(void*) == 4 || sizeof(void*) == 8,
	"unsupported pointer width");
_Static_assert(sizeof(int32_t) == 4,
	"SentryXfer result must remain int32");
_Static_assert(
	_Generic(&nox_xxx_XFerSentry_4F5E50,
		sentry_xfer_fn_4F5E50: 1, default: 0),
	"SentryXfer must preserve one native pointer and exact int32 result");
...
int main(void) {
	...
	sentry_xfer_fn_4F5E50 const transfer = nox_xxx_XFerSentry_4F5E50;
