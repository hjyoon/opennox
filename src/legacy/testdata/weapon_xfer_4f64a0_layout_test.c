...
typedef int32_t (*weapon_xfer_fn_4F64A0)(nox_object_t*);

_Static_assert(CHAR_BIT == 8, "serialized bytes must remain eight bits");
_Static_assert(sizeof(void*) == 4 || sizeof(void*) == 8,
	"unsupported pointer width");
_Static_assert(sizeof(int32_t) == 4,
	"WeaponXfer result must remain int32");
_Static_assert(
	_Generic(&nox_xxx_XFerWeapon_4F64A0,
		weapon_xfer_fn_4F64A0: 1, default: 0),
	"WeaponXfer must preserve one native pointer and exact int32 result");
...
int main(void) {
	...
	weapon_xfer_fn_4F64A0 const transfer = nox_xxx_XFerWeapon_4F64A0;
