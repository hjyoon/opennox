...
typedef int32_t (*monster_generator_xfer_fn_4F7130)(nox_object_t*);

_Static_assert(CHAR_BIT == 8, "serialized bytes must remain eight bits");
_Static_assert(sizeof(void*) == 4 || sizeof(void*) == 8,
	"unsupported pointer width");
_Static_assert(sizeof(int32_t) == 4,
	"MonsterGeneratorXfer result must remain int32");
_Static_assert(
	_Generic(&nox_xxx_XFerMonsterGen_4F7130,
		monster_generator_xfer_fn_4F7130: 1, default: 0),
	"MonsterGeneratorXfer must preserve one native pointer and exact int32 result");
...
int main(void) {
	...
	monster_generator_xfer_fn_4F7130 const transfer =
