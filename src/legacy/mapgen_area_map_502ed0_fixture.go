...
typedef struct nox_test_mapgen_rewrite_502ED0_result {
	int result;
	uint32_t count;
	int file_closed;
	...
	char first_name[64];
	char second_name[64];
} nox_test_mapgen_rewrite_502ED0_result;

static nox_test_mapgen_rewrite_502ED0_result nox_test_mapgen_rewrite_502ED0(
	const char* source, const char* directory, const char* name) {
	nox_test_mapgen_rewrite_502ED0_result out = {0};
	char first[0x800] = {0};
	char second[0x800] = {0};
	char records[0x26000] = {0};
	if (!source || !directory || !name || strlen(source) >= sizeof(first) ||
	...
	out.result = sub_502ED0((char*)name);
...
type mapgenRewriteViaCResult502ED0 struct {
	result         int
	count          uint32
	fileClosed     bool
...
func mapgenRewriteViaC502ED0(source, directory, name string) mapgenRewriteViaCResult502ED0 {
	...
	v := C.nox_test_mapgen_rewrite_502ED0(csource, cdirectory, cname)
	return mapgenRewriteViaCResult502ED0{
		result:         int(v.result),
