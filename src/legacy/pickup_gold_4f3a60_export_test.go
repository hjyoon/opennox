...
func TestPickupGoldExport4F3A60PreservesNativePointersFourArgumentsAndResult(t *testing.T) {
	...
	old := Nox_xxx_pickupGold_4F3A60
	t.Cleanup(func() { Nox_xxx_pickupGold_4F3A60 = old })
	calls := 0
	Nox_xxx_pickupGold_4F3A60 = func(gotOwner, gotItem *server.Object, arg3, arg4 int32) int32 {
		calls++
	...
	if got := pickupGoldExportCall4F3A60(owner, item, math.MinInt32, math.MaxInt32); got != math.MinInt32 {
		t.Fatalf("first result = %d, want %d", got, int32(math.MinInt32))
	}
	if got := pickupGoldExportCall4F3A60(owner, item, -17, -23); got != math.MaxInt32 {
