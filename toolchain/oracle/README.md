...
`game-exe-functions.json`은 포팅한 원본 함수의 VA, 정확한 함수 길이와 범위 SHA-256을 기록한다. 검증기는 전체 트리 매니페스트와 오라클 ID 및 `GAME.EXE` 해시를 교차 확인하고, PE32/I386 형식, 이미지 베이스 `0x00400000`, 함수와 jump table이 실행 섹션에 있는지, 상수 테이블이 비실행 섹션에 있는지와 각 범위 해시를 검사한다. 현재는 `0x004D6A20`, `0x004D8270`, `0x004D9EB0`, `0x004E44F0..0x004E9488`, `0x004EADE0`, `0x00536E10..0x00536E4F`, `0x0054E170..0x0054E45D`의 145개 함수와 `0x004E4C90`·`0x004E6CE0`·`0x004E89F4`·`0x004E8DB8`이 사용하는 네 dispatch table 범위를 합계 149개 코드 범위로 봉인했다. `004E6E50`의 입력인 256개 정수 방향 벡터·임계값·9×16 반환표, `004E7410`의 float32 `85.0` 상수, `004E7470`의 두 드롭 표·타입 문자열, `004E8910/004E8A10`의 네 Door 열쇠 이름과 SilverKey 조회 문자열, `004E8AC0`의 float32 `34.0`과 일곱 메시지 문자열, `004E8E60`의 두 NUL 종료 문자열, `004E9090`의 세 literal과 ExitCollide 등록 레코드, `004E9430/00536E10`의 DamageCollide 등록 레코드, WeaponDie/ArmorDie 등록 레코드, `004E8460`의 wall-grid inverse float32는 열네 데이터 범위로 별도 봉인했다. `004E8E50`이 반환하는 `0x007531F8`은 PE의 초기화 데이터 범위 밖 BSS이므로 별도 데이터 해시 범위를 만들지 않는다. 함수 하나를 포팅할 때마다 어셈블리에서 경계를 확인한 뒤 이 목록과 의미 설명을 확장한다.
