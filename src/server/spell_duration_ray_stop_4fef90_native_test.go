...
func TestDurationRayStopNative4FEF90Layout(t *testing.T) {
...
func TestDurationRayStopNative4FEF90PreservesHighPointersAndLiveReloads(t *testing.T) {
	caster := &Object{Direction1: 0xcd}
	caster2 := &Object{Direction1: 0xee}
	caster3 := &Object{Direction1: 0xff}
	who := &Object{}
	record := &DurSpell{Spell: 59, Caster16: caster}

	var pin runtime.Pinner
	for _, pointer := range []any{caster, caster2, caster3, who, record} {
		pin.Pin(pointer)
	}
	defer pin.Unpin()
	if unsafe.Sizeof(uintptr(0)) == 8 {
		for name, pointer := range map[string]uintptr{
			...
			"who":     uintptr(unsafe.Pointer(who)),
	...
	var gotRemove int32
	var unmarked []*Object
	durationRayStopNative4FEF90(record, who, durationRayStopNativeDeps4FEF90{
...
func TestDurationRayStopNative4FEF90ChainUsesLiveLinks(t *testing.T) {
	...
	var packets [][7]byte
	durationRayStopNative4FEF90(parent, targetA, durationRayStopNativeDeps4FEF90{
		unitCode: func(object *Object) uint32 {
			return codes[object]
		},
	...
	want := [][7]byte{
		{0x9e, 10, 1, 0x01, 0x20, 0x01, 0x10},
		{0x9e, 11, 3, 0x03, 0x20, 0x02, 0x10},
	}
