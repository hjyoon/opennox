...
typedef void (*disengage_callback)(
	void*, nox_object_t*, const nox_object_t*);
typedef void (*item_apply_disengage_fn)(const nox_object_t*, nox_object_t*);

struct modifier_effect {
	uintptr_t marker;
	disengage_callback disengage;
};
...
_Static_assert(sizeof(disengage_callback) == sizeof(void*), "disengage callbacks must use native pointer width");
_Static_assert(
	_Generic(&nox_xxx_itemApplyDisengageEffect_4F3030,
		item_apply_disengage_fn: 1, default: 0),
	"item disengage dispatch must return void and receive two native object pointers");
...
static void replacement_disengage(
		void* effect,
		nox_object_t* owner,
		const nox_object_t* item) {
...
// Freestanding semantic companion for GAME.EXE 004F3030. Production uses the
...
void nox_xxx_itemApplyDisengageEffect_4F3030(
	...
	for (size_t slot = 2; slot < 4; ++slot) {
		...
		if (modifier == NULL)
			continue;
		disengage_callback const disengage = modifier->disengage;
		if (disengage != NULL)
			disengage(modifier, owner, item);
	}
}
...
int main(void) {
	...
	modifier_effect replacement = {
		...
		.disengage = replacement_disengage,
	};
	modifier_data data = {
	...
	item_apply_disengage_fn const apply = nox_xxx_itemApplyDisengageEffect_4F3030;
