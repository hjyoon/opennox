...
_Static_assert(
	_Generic(&sub_4FC950, fixed_rng_seed_fn: 1, default: 0),
	"fourth fixed RNG seed wrapper must take no arguments and return void");
_Static_assert(
	_Generic(&sub_4FE840, fixed_rng_seed_fn: 1, default: 0),
	"fifth fixed RNG seed wrapper must take no arguments and return void");

static unsigned int observed_seeds[10];
...
int main(void) {
	...
	fixed_rng_seed_fn const fourth = sub_4FC950;
	fixed_rng_seed_fn const fifth = sub_4FE840;
	...
	assert(observed_calls == 10);
	assert(observed_seeds[0] == UINT32_C(0x143D));
	assert(observed_seeds[1] == UINT32_C(0x22EA));
	assert(observed_seeds[2] == UINT32_C(0x22EB));
	assert(observed_seeds[3] == UINT32_C(0x1446));
	assert(observed_seeds[4] == UINT32_C(0x1447));
	assert(observed_seeds[5] == UINT32_C(0x143D));
	assert(observed_seeds[6] == UINT32_C(0x22EA));
	assert(observed_seeds[7] == UINT32_C(0x22EB));
	assert(observed_seeds[8] == UINT32_C(0x1446));
	assert(observed_seeds[9] == UINT32_C(0x1447));
	return 0;
}
