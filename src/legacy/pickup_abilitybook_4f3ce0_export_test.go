...
func TestPickupAbilityBookExport4F3CE0PreservesNativePointersFourArgumentsAndResult(t *testing.T) {
	...
	old := Nox_xxx_pickupAbilitybook_4F3CE0
	t.Cleanup(func() { Nox_xxx_pickupAbilitybook_4F3CE0 = old })
	calls := 0
	Nox_xxx_pickupAbilitybook_4F3CE0 = func(gotOwner, gotItem *server.Object, arg3, arg4 int32) int32 {
		calls++
	...
	if got := pickupAbilityBookExportCall4F3CE0(owner, item, math.MinInt32, math.MaxInt32); got != math.MinInt32 {
		t.Fatalf("first result = %d, want %d", got, int32(math.MinInt32))
	}
	if got := pickupAbilityBookExportCall4F3CE0(owner, item, -17, -23); got != math.MaxInt32 {
