...
typedef short (*unit_get_old_mana_fn)(nox_object_t*);

_Static_assert(CHAR_BIT == 8, "mana bytes must remain eight bits");
_Static_assert(sizeof(short) == 2, "current-mana return must remain 16-bit");
_Static_assert(SHRT_MIN == -32768 && SHRT_MAX == 32767, "C short must be exact signed 16-bit");
_Static_assert(sizeof(void*) == 4 || sizeof(void*) == 8, "unsupported pointer width");
_Static_assert(
	_Generic(&nox_xxx_unitGetOldMana_4EEC80, unit_get_old_mana_fn: 1, default: 0),
	"current-mana getter must use one native object pointer and return a 16-bit C short");

static nox_object_t* observed_unit;
static short next_result;

short nox_xxx_unitGetOldMana_4EEC80(nox_object_t* unit) {
	observed_unit = unit;
	return next_result;
}

static void check_word(unit_get_old_mana_fn get_mana, nox_object_t* unit, short value, uint16_t bits) {
	next_result = value;
...
int main(void) {
	...
	unit_get_old_mana_fn const get_mana = nox_xxx_unitGetOldMana_4EEC80;
