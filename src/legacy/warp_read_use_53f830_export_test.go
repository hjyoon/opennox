...
type warpReadUseLegacyServer53F830 struct {
	Server
	srv *server.Server
}

func (s *warpReadUseLegacyServer53F830) S() *server.Server {
	return s.srv
}

func installWarpReadUseLegacyServer53F830(t *testing.T, srv *server.Server) {
	t.Helper()
	oldGetServer := GetServer
	GetServer = func() Server {
		return &warpReadUseLegacyServer53F830{srv: srv}
	}
	t.Cleanup(func() {
...
func TestWarpReadUseExport53F830PreservesNativePointers(t *testing.T) {
	installWarpReadUseLegacyServer53F830(t, &server.Server{})
	...
	if got := warpReadUseExportCall53F830(owner, readable); got != 1 {
...
func TestWarpReadUseRegistration53F830DispatchesNativeImplementation(t *testing.T) {
	srv := &server.Server{}
	srv.SetTickRate(20)
	srv.SetFrame(61)
	installWarpReadUseLegacyServer53F830(t, srv)
	...
	use := server.UseFuncPtr{Ptr: Get_sub_53F830()}.Get()
	if use == nil {
		t.Fatal("registered WarpReadUse callback is nil")
	}
	if !use(owner, readable) {
		t.Fatal("registered WarpReadUse result = false, want true")
	}
