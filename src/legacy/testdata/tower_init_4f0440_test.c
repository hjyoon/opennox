...
typedef void (*tower_init_fn)(nox_object_t*);

_Static_assert(CHAR_BIT == 8, "bytes must remain eight bits");
_Static_assert(sizeof(void*) == 4 || sizeof(void*) == 8, "unsupported pointer width");
_Static_assert(
	_Generic(&nox_xxx_unitTowerInit_4F0440, tower_init_fn: 1, default: 0),
	"TowerInit must use one native object pointer and return void");

static nox_object_t* observed_unit;
static unsigned int observed_calls;

void nox_xxx_unitTowerInit_4F0440(nox_object_t* unit) {
	observed_unit = unit;
	++observed_calls;
}

static int check_call(tower_init_fn init, nox_object_t* unit) {
	init(unit);
...
int main(void) {
	...
	tower_init_fn const init = nox_xxx_unitTowerInit_4F0440;
	int line;
