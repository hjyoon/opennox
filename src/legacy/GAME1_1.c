...
//----- (00418830) --------------------------------------------------------
void sub_418830(nox_team_t* team, int id) {
	if (team) {
		team->field_60 = id;
	}
}
...
//----- (004189D0) --------------------------------------------------------
nox_team_t* sub_4189D0(void) {
	nox_team_t* result = NULL;
	unsigned char minimum = 32;
	for (nox_team_t* team = nox_server_teamFirst_418B10(); team; team = nox_server_teamNext_418B60(team)) {
		unsigned char count = sub_418BC0(team);
		if (count < minimum) {
			minimum = count;
			result = team;
		}
...
//----- (00418A10) --------------------------------------------------------
nox_team_t* sub_418A10(void) {
	nox_team_t* result = nox_server_teamFirst_418B10();
	if (!result) {
		return nox_xxx_teamCreate_4186D0(0);
	}
	while (result->field_60) {
...
//----- (00418A40) --------------------------------------------------------
nox_team_t* sub_418A40(wchar2_t* name) {
	if (!name) {
		return NULL;
	}
	for (nox_team_t* team = nox_server_teamFirst_418B10(); team; team = nox_server_teamNext_418B60(team)) {
		if (!_nox_wcsicmp(team->name, name)) {
			return team;
		}
	}
	return NULL;
...
//----- (00418CD0) --------------------------------------------------------
void nox_xxx_teamRenameMB_418CD0(nox_team_t* team, wchar2_t* name) {
	if (!team || !name) {
		return;
	}
	sub_457010(team, name);
	team->field_68 = 0;
	if (nox_common_gameFlags_check_40A5C0(1)) {
		char packet[46] = {0};
		*(uint16_t*)packet = 1220;
		*(uint32_t*)&packet[2] = team->field_57;
		nox_wcscpy((wchar2_t*)&packet[6], name);
		nox_xxx_netSendPacket1_4E5390(159, packet, sizeof(packet), 0, 1);
	}
	nox_wcscpy(team->name, name);
}
...
//----- (00419090) --------------------------------------------------------
void nox_xxx_netChangeTeamID_419090(nox_team_t* team, int lessons) {
	if (!team) {
		return;
	}
	...
	if (nox_common_gameFlags_check_40A5C0(1)) {
		...
		packet[1] = 8;
		*(uint32_t*)&packet[2] = team->field_57;
		*(uint32_t*)&packet[6] = team->lessons;
		nox_xxx_netSendPacket1_4E5390(159, packet, sizeof(packet), 0, 1);
	}
}
...
int sub_4196D0(void* a1p, void* a2p, int a3, int a4) {
	nox_object_team_t* value = a1p;
	nox_team_t* team = a2p;
	if (!value || !team || !nox_xxx_teamCompare2_419180(value, value->id)) {
		return 0;
	}
	...
	if (nox_common_gameFlags_check_40A5C0(1) && nox_common_gameFlags_check_40A5C0(0x2000)) {
		...
		packet[1] = 3;
		*(uint32_t*)&packet[2] = team->field_57;
		*(uint16_t*)&packet[6] = a3;
		nox_xxx_netSendPacket1_4E5390(159, packet, sizeof(packet), 0, 1);
		sub_4571A0(a3, team->field_57);
	}
	nox_team_t* old_team = nox_xxx_getTeamByID_418AB0(value->id);
	if (!old_team) {
		return 0;
	}
	if (old_team->field_48 > 0) {
		old_team->field_48--;
	}
	sub_418E40(old_team, value);
	nox_xxx_createAtImpl_4191D0(team->field_57, value, 0, a3, a4);
	if (a3 == nox_player_netCode_85319C) {
		sub_455E70(team->field_57);
	}
	return 1;
}
...
//----- (00419900) --------------------------------------------------------
char sub_419900(nox_team_t* team, nox_object_team_t* value, short net_code) {
	char result = 0;
	if (team && value) {
		result = team->field_57;
		if (value->id != (uint8_t)result) {
			char packet[10] = {0};
			*(uint32_t*)&packet[2] = team->field_57;
			packet[0] = -60;
			packet[1] = 10;
			*(uint16_t*)&packet[6] = net_code;
			result = nox_xxx_netClientSend2_4E53C0(31, packet, sizeof(packet), 0, 1);
		}
	}
	return result;
}
...
//----- (00419960) --------------------------------------------------------
char sub_419960(nox_team_t* team, nox_object_team_t* value, short net_code) {
	char result = 0;
	if (team && value) {
		result = team->field_57;
		if (value->id != (uint8_t)result) {
	...
	if (result) {
		...
		if ((uint8_t)a1) {
			...
			if (nox_crypt_IsReadOnly() == 1) {
				...
				nox_xxx_playerManaRefresh_4EECF0((nox_object_t*)(uintptr_t)(uint32_t)v1);
			}
...
int nox_xxx_countObserverPlayers_425BF0() {
	...
	if (nox_common_gameFlags_check_40A5C0(0x8000)) {
		for (nox_playerInfo* player = nox_common_playerInfoGetFirst_416EA0(); player;
			...
			if (flags & 1 && !(flags & 0x20) && player->playerInd != 31) {
				++count;
			}
		}
	}
...
//----- (00425E90) --------------------------------------------------------
int sub_425E90(nox_playerInfo* player, char a2) {
	int result; // eax
...
//----- (00425ED0) --------------------------------------------------------
int sub_425ED0(nox_playerInfo* player, char a2) {
	int result; // eax
...
void sub_425F10(nox_playerInfo* pl) {
	if (!nox_common_gameFlags_check_40A5C0(0x2000) || nox_common_gameFlags_check_40A5C0(4096) || !pl ||
		pl->field_4648 != -1) {
		return;
	}
	int index = (dword_5d4594_608316)++;
	int offset = 32 * index;
	memcpy(getMemAt(0x5D4594, 600124 + offset), pl->field_2096, strlen(pl->field_2096) + 1);
	int connection = pl->playerInd == 31 ? 0 : pl->playerInd + 1;
	*getMemU32Ptr(0x5D4594, 600136 + offset) = htonl(nox_xxx_net_getIP_554200(connection));
	if ((pl->playerInd == 31 && nox_common_getEngineFlag(NOX_ENGINE_FLAG_DISABLE_GRAPHICS_RENDERING)) ||
		(pl->field_3680 & 1 && !(pl->field_3680 & 0x20))) {
		*getMemU8Ptr(0x5D4594, 600152 + offset) = 0;
	} else {
		*getMemU8Ptr(0x5D4594, 600152 + offset) = 1;
	}
	*getMemU32Ptr(0x5D4594, 600140 + offset) = pl->field_2068;
	*getMemU8Ptr(0x5D4594, 600144 + offset) = pl->info.playerClass;
	*getMemU32Ptr(0x5D4594, 600148 + offset) = time(0);
	*getMemU8Ptr(0x5D4594, 600145 + offset) = 1;
	pl->field_4648 = index;
}
