...
func (c *CtrlEventHandler) nox_xxx_clientControl_42D6B0_B() {
	...
	for i := indStart; i < indEnd; i++ {
		...
		case player.CCServerOptions:
			...
			ce.Active = false
		case player.CCLaugh:
			nox_xxx_netClientSendSocial(server.HostPlayerIndex, 1, nil, 1)
		case player.CCTaunt:
			nox_xxx_netClientSendSocial(server.HostPlayerIndex, 2, nil, 1)
		case player.CCPoint:
			nox_xxx_netClientSendSocial(server.HostPlayerIndex, 4, nil, 1)
