// Freestanding C11 matrix companion for GAME.EXE 004F3C60. Production uses
// the Go restoration; this file independently locks the exact four-argument
// ABI, native pointer width, live flag/subclass reloads, delayed argument
// loads, full signed DefaultPickup result, and 826/828 audio selection.
...
typedef int32_t (*pickup_spellbook_fn)(
	nox_object_t*, nox_object_t*, int32_t, int32_t);

struct nox_object_t {
	uintptr_t marker;
	uint32_t object_subclass;
...
_Static_assert(
	_Generic(&nox_xxx_pickupSpellbook_4F3C60,
		pickup_spellbook_fn: 1, default: 0),
	"SpellBookPickup must retain two native pointers, two int32_t values, and int32_t result");
...
static uint32_t expected_sound;
...
static int32_t default_pickup(
	...
	if (default_result == INT32_MIN)
		item->object_subclass |= UINT32_C(1);
	return default_result;
}

static uint8_t load_subclass_low(nox_object_t* item) {
	event(7U);
	return (uint8_t)item->object_subclass;
}
...
int32_t nox_xxx_pickupSpellbook_4F3C60(
	...
	if (result != INT32_C(0)) {
		uint32_t sound = UINT32_C(828);
		if ((load_subclass_low(item) & UINT8_C(1)) != UINT8_C(0))
			sound = UINT32_C(826);
		audio(sound, owner, INT32_C(0), UINT32_C(0));
	}
	return result;
}
...
int main(void) {
	...
	expected_sound = UINT32_C(826);
	assert(nox_xxx_pickupSpellbook_4F3C60(
		&owner, &item, INT32_MIN, INT32_MAX) == INT32_MIN);
	{
		const unsigned int want[] = {1U, 2U, 3U, 4U, 5U, 6U, 7U, 8U};
		assert_trace(want, sizeof(want) / sizeof(want[0]));
	}

	item.object_flags = UINT32_C(0);
	item.object_subclass = UINT32_C(0x80);
	reset_case(&owner, &item);
	game_flags_result = INT32_C(1);
	...
	default_result = INT32_C(7);
	expected_sound = UINT32_C(828);
	assert(nox_xxx_pickupSpellbook_4F3C60(
		&owner, &item, INT32_C(9), INT32_C(10)) == INT32_C(7));
	{
		const unsigned int want[] = {1U, 3U, 4U, 5U, 6U, 7U, 8U};
		assert_trace(want, sizeof(want) / sizeof(want[0]));
	}
