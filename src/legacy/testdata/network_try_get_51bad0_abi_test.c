...
typedef int (*network_try_get_fn)(unsigned char*, nox_object_t*, void*);

_Static_assert(CHAR_BIT == 8, "bytes must remain eight bits");
_Static_assert(sizeof(void*) == 4 || sizeof(void*) == 8,
	"unsupported pointer width");
_Static_assert(
	_Generic(&nox_server_netTryGet_51BAD0,
		network_try_get_fn: 1, default: 0),
	"MSG_TRY_GET must preserve all pointer-width arguments");
...
int nox_server_netTryGet_51BAD0(
	...
	if (data != expected_data || unit != expected_unit ||
		update != expected_update || data[0] != UINT8_C(0x73))
		return -1;
	return 3;
}

int main(void) {
	unsigned char packet[3] = {UINT8_C(0x73), UINT8_C(0x34), UINT8_C(0x12)};
	nox_object_t unit = {.marker = UINTPTR_MAX};
	uintptr_t update = (uintptr_t)0x1234U;
	network_try_get_fn const try_get = nox_server_netTryGet_51BAD0;
	expected_data = packet;
	expected_unit = &unit;
	expected_update = &update;
	if (try_get(packet, &unit, &update) != 3)
		return __LINE__;
