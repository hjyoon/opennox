...
func TestDefaultXferNativeWrite4F49A0PreservesPointersAndWire(t *testing.T) {
	object, freeObject := alloc.New(server.Object{})
	defer freeObject()
	item, freeItem := alloc.New(server.Object{})
	defer freeItem()
	id, freeID := alloc.CString("default-xfer")
	defer freeID()

	assertObjectMapNativePointer4F4530(t, "object", unsafe.Pointer(object))
	assertObjectMapNativePointer4F4530(t, "inventory item", unsafe.Pointer(item))
	...
	writeU16(defaultXferCurrentVersion4F49A0)
	...
	want.WriteByte(uint8(len("default-xfer")))
	want.WriteString("default-xfer")
	want.WriteByte(7)
	want.WriteByte(1)
	writeU16(0)
	...
	if object.IDPtr != unsafe.Pointer(id) || object.InvFirstItem != item {
		t.Errorf("native pointers changed: ID=%p item=%p", object.IDPtr, object.InvFirstItem)
	}
}

func TestDefaultXferNativeRead4F49A0RestoresEntryField34AfterSerializer(t *testing.T) {
	const (
	...
	path := filepath.Join(t.TempDir(), "default-xfer-read.bin")
