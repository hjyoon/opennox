...
type goldXferNativeInventoryCall4F6EC0 struct {
	version uint16
	object  *server.Object
	count   int32
}

func TestGoldXferNativeLayout4F6EC0(t *testing.T) {
	type field struct {
		name string
		got  uintptr
		pe32 uintptr
	...
	fixed := []struct {
		...
		{name: "GoldInitData size", got: unsafe.Sizeof(server.GoldInitData{}), want: 4},
		{name: "GoldInitData.Amount", got: unsafe.Offsetof(server.GoldInitData{}.Amount), want: 0},
	}
...
func TestGoldXferNativeWrite4F6EC0PreservesPointerAndExactWire(t *testing.T) {
	object, freeObject := alloc.New(server.Object{})
	defer freeObject()
	data, freeData := alloc.New(server.GoldInitData{Amount: 0xdecafbad})
	defer freeData()
	*data = server.GoldInitData{Amount: 0xdecafbad}
	id, freeID := alloc.CString("gold-native")
	defer freeID()

	for name, pointer := range map[string]unsafe.Pointer{
		"object":    unsafe.Pointer(object),
		"init data": unsafe.Pointer(data),
	...
	object.ObjClass = objectlib.ClassFood | objectlib.ClassPickup | objectlib.ClassClientPersist
	...
	writeU16(goldXferCurrentVersion4F6EC0)
	...
	want.WriteByte(uint8(len("gold-native")))
	want.WriteString("gold-native")
	want.WriteByte(7)
	want.WriteByte(0)
	writeU16(0)
	...
	if object.Field34 != objectFrame || object.InitData != unsafe.Pointer(data) {
		t.Fatalf("native records changed: Field34=%#x init=%p", object.Field34, object.InitData)
	}
	if data.Amount != 0xdecafbad {
		t.Fatalf("gold amount = %#x, want %#x", data.Amount, uint32(0xdecafbad))
	}
}

func TestGoldXferNativeRead4F6EC0TransfersAmountAndRestoresCount(t *testing.T) {
	const (
	...
	path := filepath.Join(t.TempDir(), "gold-read.bin")
	...
	object, freeObject := alloc.New(server.Object{})
	defer freeObject()
	data, freeData := alloc.New(server.GoldInitData{Amount: 1})
	defer freeData()
	*data = server.GoldInitData{Amount: 1}
	...
	var inventoryCalls []goldXferNativeInventoryCall4F6EC0
	deps := goldXferRuntimeDeps4F6EC0()
	deps.transferInventory = func(version uint16, gotObject *server.Object, count int32) int32 {
		inventoryCalls = append(inventoryCalls, goldXferNativeInventoryCall4F6EC0{
			version: version,
			object:  gotObject,
			count:   count,
		})
	...
	if got := goldXferNative4F6EC0(cf, object, deps); got != 1 {
		t.Fatalf("result = %d, want 1", got)
	}
	if data.Amount != amount {
		t.Fatalf("gold amount = %#x, want %#x", data.Amount, amount)
	}
	if len(inventoryCalls) != 1 || inventoryCalls[0] != (goldXferNativeInventoryCall4F6EC0{
	...
	if object.IDPtr != unsafe.Pointer(oldID) || object.InitData != unsafe.Pointer(data) {
...
func TestGoldXferExport4F6EC0PreservesNativePointerAndResult(t *testing.T) {
	object, freeObject := alloc.New(server.Object{})
	defer freeObject()
	assertObjectMapNativePointer4F4530(t, "object", unsafe.Pointer(object))

	old := goldXferCall4F6EC0
	t.Cleanup(func() { goldXferCall4F6EC0 = old })
	calls := 0
	goldXferCall4F6EC0 = func(_ *cryptfile.CryptFile, gotObject *server.Object) int32 {
		calls++
		switch calls {
		case 1:
			if gotObject != object {
	...
	if got := goldXferExportCall4F6EC0(object); got != math.MinInt32 {
		t.Fatalf("first result = %d, want %d", got, int32(math.MinInt32))
	}
	if got := goldXferExportCall4F6EC0(nil); got != math.MaxInt32 {
