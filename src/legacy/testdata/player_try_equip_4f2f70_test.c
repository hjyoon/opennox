...
typedef int32_t (*player_try_equip_fn)(nox_object_t*, nox_object_t*);
typedef int32_t (*equip_lower_fn)(
	nox_object_t*, nox_object_t*, int32_t, int32_t);

_Static_assert(CHAR_BIT == 8, "bytes must remain eight bits");
_Static_assert(sizeof(int32_t) == 4, "equip result and flags must be exact dwords");
_Static_assert(sizeof(void*) == 4 || sizeof(void*) == 8, "unsupported pointer width");
_Static_assert(
	_Generic(&nox_xxx_playerTryEquip_4F2F70,
		player_try_equip_fn: 1, default: 0),
	"player equip must use native object pointers and an exact int32 result");
...
// Freestanding semantic companion for GAME.EXE 004F2F70. Production uses the
// Go restoration; this definition lets every target frontend independently
// check the exact public declaration and original callback order.
int32_t nox_xxx_playerTryEquip_4F2F70(
		nox_object_t* owner,
		nox_object_t* item) {
	equip_lower_fn const weapon = try_weapon;
	equip_lower_fn const armor = try_armor;
...
static int check_case(
	...
	player_try_equip_fn const try_equip = nox_xxx_playerTryEquip_4F2F70;
	...
	call_sequence = 0U;
	failure_line = 0;
	if (try_equip(owner, item) != expected_result)
		return __LINE__;
	if (failure_line != 0)
		return failure_line;
...
int main(void) {
	...
	nox_object_t item = {.marker = UINT32_C(0x55555555)};
	int line;
