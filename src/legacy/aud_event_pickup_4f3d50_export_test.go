...
func TestAudEventPickupExport4F3D50PreservesNativePointersFourArgumentsAndResult(t *testing.T) {
	...
	old := Nox_objectPickupAudEvent_4F3D50
	t.Cleanup(func() { Nox_objectPickupAudEvent_4F3D50 = old })
	calls := 0
	Nox_objectPickupAudEvent_4F3D50 = func(gotOwner, gotItem *server.Object, arg3, arg4 int32) int32 {
		calls++
	...
	if got := audEventPickupExportCall4F3D50(owner, item, math.MinInt32, math.MaxInt32); got != math.MinInt32 {
		t.Fatalf("first result = %d, want %d", got, int32(math.MinInt32))
	}
	if got := audEventPickupExportCall4F3D50(owner, item, -17, -23); got != math.MaxInt32 {
