...
func TestServerNetSendBallStatus4D95F0ExactPacketAndReturn(t *testing.T) {
	s := &Server{}
	wantResult := int(math.MinInt32 + 17)
	called := 0
	s.NetSendPacketXxx = func(recipient int, payload []byte, related *Object, removeIfDisconnected, sequenceEnabled int) int {
		...
		wantPayload := []byte{0xd9, 0xfe, 0xcd, 0xab}
		if !reflect.DeepEqual(payload, wantPayload) {
			t.Fatalf("payload = % x, want % x", payload, wantPayload)
		}
		return wantResult
	}
	if got := s.Nox_xxx_netSendBallStatus_4D95F0(255, 0xfe, 0xabcd); got != int32(wantResult) {
